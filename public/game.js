...
(() => {
  ...
  function buildGround() {
    ...
    const holes=[[-47,-40,-37,-8.5],[16.5,27.5,47,40.5]];
  ...
  function buildRabbitTunnel(){
    ...
    // V19: superficies internas sem contorno por segmento. Os EdgesGeometry de cada
    // bloco curto criavam o efeito de "molduras" repetidas dentro da toca.
    // Mantemos segmentos curtos para acompanhar a rampa, mas sem arestas desenhadas
    // em cada emenda; visualmente o corredor passa a parecer continuo.
    const tunnelBox=(w,h,d,material,x,y,z)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),material);m.position.set(x,y,z);m.castShadow=true;m.receiveShadow=true;scene.add(m);return m;};
    let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],pieces=Math.max(1,Math.ceil(len/2.6));for(let j=0;j<pieces;j++){const u0=j/pieces,u1=(j+1)/pieces,um=(u0+u1)/2;const x0=ax+(bx-ax)*u0,z0=az+(bz-az)*u0,x1=ax+(bx-ax)*u1,z1=az+(bz-az)*u1,mx=(x0+x1)/2,mz=(z0+z1)/2,segLen=Math.hypot(x1-x0,z1-z0),t0=(acc+len*u0)/total,t1=(acc+len*u1)/total,tm=(t0+t1)/2,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(x1-x0,z1-z0),pitch=Math.atan2(y1-y0,segLen);
      const floorMat=mat(wallLight);floorMat.side=THREE.DoubleSide;const floor=tunnelBox(5.55,.10,segLen+.18,floorMat,mx,ym,mz);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;floor.userData.rabbitFloor=true;
      for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.76,wz=mz-Math.sin(yaw)*side*2.76;const wallMat=mat(wall);wallMat.side=THREE.DoubleSide;const extra=Math.abs(y1-y0);const wallSeg=tunnelBox(.30,3.35+extra,segLen+.34,wallMat,wx,ym+1.58,wz);wallSeg.rotation.y=yaw;wallSeg.userData.rabbitWall=true;const ex=Math.abs(Math.cos(yaw))*.15+Math.abs(Math.sin(yaw))*(segLen+.34)/2,ez=Math.abs(Math.sin(yaw))*.15+Math.abs(Math.cos(yaw))*(segLen+.34)/2;colliders.push({minX:wx-ex,maxX:wx+ex,minZ:wz-ez,maxZ:wz+ez,minY:ym-.12,height:ym+3.35+extra});}
      if(window.__inkRabbitTunnelY(tm)<-1.75&&tm>.16&&tm<.84){const roofMat=mat(0xe8e2d5);roofMat.side=THREE.DoubleSide;const roof=tunnelBox(5.90,.24,segLen+.31,roofMat,mx,ym+3.18,mz);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;}
    }acc+=len;}
