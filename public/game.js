...
(() => {
  ...
  function addInkPotMagazine(v,kind){
    ...
    const body=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.34,.44,.31),mat(0xf2eee3)),2.4); body.userData.keepViewColor=true; pot.add(body);
    const rim=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.36,.070,.33),mat(0x252a31)),2.1); rim.userData.keepViewColor=true; rim.position.y=.215; pot.add(rim);
    // Manchas e pingos na face voltada para a câmera: tinta vermelha realmente legível.
    for(const x of [-.085,-.015,.070]){const d=new THREE.Mesh(new THREE.CapsuleGeometry(.022,.13+(x<0?.055:0),3,6),mat(0xb51f2e));d.userData.keepViewColor=true;d.position.set(x,-.07,.148);pot.add(d);}
    const stain=new THREE.Mesh(new THREE.SphereGeometry(.060,7,5),mat(0xb51f2e)); stain.userData.keepViewColor=true; stain.scale.set(1.4,.65,.30); stain.position.set(.065,.08,.150); pot.add(stain);
    const longDrip=new THREE.Mesh(new THREE.CapsuleGeometry(.026,.20,3,6),mat(0xb51f2e)); longDrip.userData.keepViewColor=true; longDrip.position.set(-.10,-.12,.165); pot.add(longDrip);
    // O pote fica pendurado sob o receptor, ligeiramente para o lado da câmera para não sumir atrás da arma.
    const pos={rifle:[.40,-.67,-.82],sniper:[.40,-.64,-.98],smg:[.42,-.69,-.88],shotgun:[.41,-.62,-.88]}[kind]||[.40,-.67,-.86];
    pot.position.set(...pos); pot.rotation.set(.10,0,-.06); v.add(pot);
  }
  for(const [kind,v] of Object.entries(weaponVariants))if(kind!=='knife')addInkPotMagazine(v,kind);
  // V56.34: mãos desenhadas como palma + polegar, presas ao próprio viewmodel.
  for(const [kind,v] of Object.entries(weaponVariants)){
    if(kind==='knife')continue;
    const handMat=mat(0xfffbef), sleeveMat=mat(0x315f9d);
    const grips={rifle:{rear:[.40,-.57,-.57],front:[.40,-.43,-1.35]},sniper:{rear:[.40,-.57,-.72],front:[.40,-.43,-1.55]},smg:{rear:[.41,-.58,-.60],front:[.41,-.44,-1.18]},shotgun:{rear:[.42,-.58,-.58],front:[.42,-.44,-1.62]},launcher:{rear:[.40,-.58,-.84],front:[.40,-.43,-1.62]}}[kind]||{rear:[.40,-.57,-.60],front:[.40,-.43,-1.30]};
    const makeGripHand=(x,y,z,front=false)=>{
      const hg=new THREE.Group();
      const sleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.105,.30,4,7),sleeveMat),2.2); sleeve.rotation.z=front?.52:-.42; sleeve.position.set(.12,-.16,.12); hg.add(sleeve);
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.26,.16,.25),handMat),2.3); palm.userData.keepViewColor=true; palm.rotation.z=front?.12:-.12; hg.add(palm); const fingers=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.21,.09,.20),handMat),2.0); fingers.userData.keepViewColor=true; fingers.position.set(0,-.09,-.025); hg.add(fingers);
      const thumb=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.045,.13,3,6),handMat),2.0); thumb.userData.keepViewColor=true; thumb.rotation.z=front?-1.0:.95; thumb.position.set(front?-.11:.11,-.01,-.05); hg.add(thumb);
      hg.position.set(x,y,z); v.add(hg); return hg;
    };
    makeGripHand(...grips.rear,false);
    makeGripHand(...grips.front,true);
  }
  ...
  function reloadWeapon(){
    ...
    if(!reloadCartridge){
      ...
      const reloadSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.11,.34,4,7),mat(0x315f9d)),2.2); reloadSleeve.rotation.z=-.45; reloadSleeve.position.set(-.44,-.25,.08); reloadCartridge.add(reloadSleeve);
  ...
  function makeRemotePlayer(state){
    ...
    body.scale.set(1.19,1.03,.74); body.position.set(0,1.18,0); // V56.36: cintura/tronco levemente afinados group.add(body);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.125,.17,.34,8),paperMat);
    neck.position.set(0,1.715,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.133,.178,.342,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    ...
    const shoulderL=new THREE.Vector3(-.52,1.47,-.005), shoulderR=new THREE.Vector3(.52,1.47,-.005);
    const elbowL=new THREE.Vector3(-.31,1.30,-.34), elbowR=new THREE.Vector3(.34,1.25,-.27);
    const handLPos=new THREE.Vector3(.105,1.245,-.91), handRPos=new THREE.Vector3(.155,1.175,-.56);
