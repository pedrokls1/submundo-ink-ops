...
(() => {
  ...
  function localTeamSpawn(){
    ...
    return {x:xs[Math.min(slot,xs.length-1)],z:team==='vermelho'?-36:36,yaw:team==='vermelho'?Math.PI:0};
  }
  ...
  // V49 — introdução "Livro do Reino das Copas".
  ...
  function makeMatchIntro(){
    ...
    const root=document.createElement('div');root.id='matchIntro';root.style.cssText='position:fixed;inset:0;z-index:120;overflow:hidden;background:#120d09;pointer-events:auto;perspective:1200px';
  ...
  function beginMatchIntro(myTeam){
    ...
    const hide=[document.getElementById('hud'),document.querySelector('.controls'),centerEl,rifleScopeEl,layoutBtn,returnLobbyBtn,voiceBtn,fullscreenBtn].filter(Boolean);
    ...
    camera.up.set(0,0,-1);camera.position.set(0,82,0.01);camera.lookAt(0,0,0);camera.fov=58;camera.updateProjectionMatrix();
    matchIntro={root,start:performance.now(),oldFog,hide,oldVis,spawnX,spawnZ,spawnYaw};
  ...
  function updateMatchIntro(now){
    ...
    if(t>2.85)I.root.classList.add('map-fly');
    if(t>=2.85){const u=Math.min(1,(t-2.85)/2.15),e=u*u*(3-2*u);const target=new THREE.Vector3(I.spawnX,1.7,I.spawnZ);const high=new THREE.Vector3(0,82,0.01);const approach=new THREE.Vector3(I.spawnX,4.7,I.spawnZ+(I.spawnZ>0?8:-8));camera.position.lerpVectors(high,approach,e);const look=new THREE.Vector3().lerpVectors(new THREE.Vector3(0,0,0),new THREE.Vector3(I.spawnX,1.7,I.spawnZ+(I.spawnZ>0?-12:12)),e);camera.lookAt(look);}
    if(t>=5){I.root.remove();I.hide.forEach((el,i)=>el.style.visibility=I.oldVis[i]);scene.fog=I.oldFog;camera.up.set(0,1,0);camera.fov=NORMAL_FOV;camera.updateProjectionMatrix();player.pos.set(I.spawnX,1.7,I.spawnZ);player.yaw=I.spawnYaw;player.pitch=0;matchIntro=null;last=performance.now();applyControlMode();if(isDesktopMode())setTimeout(()=>canvas.requestPointerLock?.(),0);return false;}
    return true;
  }
  ...
  function startGame(){
    ...
    updateHud(); matchEndOverlay.style.display='none';startEl.classList.add('hidden');overEl.classList.add('hidden');layoutBtn.style.display='block';returnLobbyBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';voiceBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';running=true;last=performance.now();clock.start();beginMatchIntro(myTeam);requestAnimationFrame(loop);
