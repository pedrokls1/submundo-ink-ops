...
(() => {
  ...
  function buildRabbitTunnel(){
    ...
    // V28: acabamento das duas bocas no nivel do terreno.
    // Sao apenas saias verticais curtas na BORDA EXTERNA da abertura; nao acompanham o tunel
    // e por isso nao criam blocos/linhas sobre o chao. Fecham a visao lateral obliqua para dentro.
    const mouthMat=mat(wall); mouthMat.side=THREE.DoubleSide;
    const mouthPanels=[
      // entrada esquerda: duas laterais, da superficie ate a parede da rampa
      [-44.82,-34.05,.22,3.0,5.7],[-39.18,-34.05,.22,3.0,5.7],
      // entrada direita: laterais da boca final (segmento leste-oeste)
      [42.05,30.82,5.7,3.0,.22],[42.05,36.45,5.7,3.0,.22]
    ];
    for(const [x,z,w,h,d] of mouthPanels){const m=box(w,h,d,mouthMat,x,-1.48,z,false);m.userData.rabbitWall=true;}
