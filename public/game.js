...
(() => {
  ...
  function buildRabbitTunnel(){
    ...
    let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],pieces=Math.max(1,Math.ceil(len/2.6));for(let j=0;j<pieces;j++){const u0=j/pieces,u1=(j+1)/pieces,um=(u0+u1)/2;const x0=ax+(bx-ax)*u0,z0=az+(bz-az)*u0,x1=ax+(bx-ax)*u1,z1=az+(bz-az)*u1,mx=(x0+x1)/2,mz=(z0+z1)/2,segLen=Math.hypot(x1-x0,z1-z0),t0=(acc+len*u0)/total,t1=(acc+len*u1)/total,tm=(t0+t1)/2,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(x1-x0,z1-z0),pitch=Math.atan2(y1-y0,segLen);
      ...
      for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.76,wz=mz-Math.sin(yaw)*side*2.76;const wallMat=mat(wall);wallMat.side=THREE.DoubleSide;const extra=Math.abs(y1-y0);const wallH=3.35+extra,wallCY=ym+1.58;const wallSeg=box(.30,wallH,segLen+.28,wallMat,wx,wallCY,wz,false);wallSeg.rotation.y=yaw;wallSeg.userData.rabbitWall=true;const ex=Math.abs(Math.cos(yaw))*.15+Math.abs(Math.sin(yaw))*(segLen+.28)/2,ez=Math.abs(Math.sin(yaw))*.15+Math.abs(Math.cos(yaw))*(segLen+.28)/2;colliders.push({minX:wx-ex,maxX:wx+ex,minZ:wz-ez,maxZ:wz+ez,minY:ym-.12,height:ym+wallH});
        // V27: saia vertical somente entre o topo da parede subterranea e o nivel do terreno.
        // Fecha a visao lateral da toca sem criar blocos acima do mapa (problema da V26).
        const wallTop=wallCY+wallH/2,groundTop=.04;
        if(wallTop<groundTop-.03&&tm>.12&&tm<.88){const skirtH=groundTop-wallTop;const skirtMat=mat(wall);skirtMat.side=THREE.DoubleSide;const skirt=box(.34,skirtH,segLen+.34,skirtMat,wx,wallTop+skirtH/2,wz,false);skirt.rotation.y=yaw;skirt.userData.rabbitWall=true;skirt.userData.rabbitOuterSkirt=true;colliders.push({minX:wx-ex-.03,maxX:wx+ex+.03,minZ:wz-ez-.03,maxZ:wz+ez+.03,minY:wallTop-.03,height:groundTop+.03});}
      }
