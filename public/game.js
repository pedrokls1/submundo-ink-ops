...
(() => {
  ...
  function beginMatchIntro(myTeam){
    ...
    const hide=[document.getElementById('hud'),document.querySelector('.controls'),centerEl,rifleScopeEl,layoutBtn,returnLobbyBtn,voiceBtn,fullscreenBtn,matchClockEl,onlineScoreEl,ammoCompactEl,kdaBtn].filter(Boolean);
  ...
  function startGame(){
    ...
    updateHud(); matchEndOverlay.style.display='none';startEl.classList.add('hidden');overEl.classList.add('hidden');layoutBtn.style.display='block';returnLobbyBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';voiceBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';running=true;document.body.classList.add('ink-playing');last=performance.now();clock.start();beginMatchIntro(myTeam);requestAnimationFrame(loop);
  ...
  const suppressIOSHold=e=>{if(!running)return;const t=e.target;if(t&&(t.tagName==='INPUT'||t.tagName==='TEXTAREA'))return;e.preventDefault();};
