...
(() => {
  ...
  function makeRemotePlayer(state){
    ...
    // Boca curta e inclinada para dar expressão mais agressiva sem poluir a cabeça.
    // Meio-sorriso torto/malicioso: um canto sobe e o outro fica mais baixo.
    const mouth=new THREE.Mesh(new THREE.TorusGeometry(.12,.024,5,12,Math.PI*.72),darkMat);
    mouth.position.set(.015,1.885,-.423); mouth.rotation.z=-.38; mouth.scale.y=.72; group.add(mouth);
    ...
    for(const side of [-1,1]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.17,8,6),accentMat),2.6);
      shoulder.position.set(side*.46,1.43,-.05); group.add(shoulder);
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.105,.42,4,7),accentMat),2.6);
      arm.position.set(side*.39,1.20,-.28); arm.rotation.x=-1.02; arm.rotation.z=side*.10; group.add(arm); arms.push(arm);
      const hand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.12,8,6),paperMat),2.3);
      hand.position.set(side<0?-.12:.27,1.12,side<0?-.67:-.56); group.add(hand);
    }
    ...
    addHatching(gun,.11,.10,.43,3,true);
    const gunAccent=new THREE.Mesh(new THREE.BoxGeometry(.175,.045,.28),mat(0xb51f2e));
    gunAccent.position.set(.16,1.31,-.58); gunGroup.add(gunAccent); group.add(gunGroup);
  ...
  function update(dt){
    ...
    const viewScale={rifle:.62,sniper:.55,smg:.62,shotgun:.60,launcher:.58,knife:.78}[weaponMode]||.62;
    activeWeapon.scale.multiplyScalar(viewScale);
    if(weaponMode!=='knife'){activeWeapon.position.x+=.16;activeWeapon.position.y-=.10;activeWeapon.position.z+=.10;}
