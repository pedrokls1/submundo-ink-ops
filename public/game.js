...
(() => {
  ...
  let mouseLook = false;
  let fireHeld = false;
  let damageFlash=0,damageShake=0,rabbitTunnelCooldown=0;
  ...
  function launchRocket(from,dir,distance,impactNormal=null){
    const group=new THREE.Group();
    ...
    rocketProjectiles.push({group,dir:dir.clone(),remaining:distance,speed:15.5,life:4.2,puffTimer:0,soundTimer:.04,soundStep:0,caption,impactNormal});
  }
  ...
  function buildSeasonalReinoDasCopas(){
    const heartPts=[];for(let i=0;i<28;i++){const t=i/28*TAU;const x=5.4*Math.sin(t)**3;const z=-(4.4*Math.cos(t)-1.8*Math.cos(2*t)-.8*Math.cos(3*t)-.35*Math.cos(4*t));heartPts.push(new THREE.Vector3(x*.72,.10,z*.72));}heartPts.push(heartPts[0].clone());scene.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(heartPts),new THREE.LineBasicMaterial({color:red,transparent:true,opacity:.9})));
    const throne=box(2.2,2.8,.65,mat(wall),0,1.4,6.4,true);addHatching(throne,1.7,2.1,.34,5,true);const crown=new THREE.Mesh(new THREE.ConeGeometry(.72,.75,5),mat(red));crown.position.set(0,3.15,6.4);crown.rotation.y=.3;addOutline(crown);scene.add(crown);
    for(const [x,z] of [[-6,5.5],[6,5.5],[-6,-5.5],[6,-5.5]]){const b=box(2.2,.9,.48,mat(woodLight),x,.45,z,true);addHatching(b,1.65,.55,.25,3,false);}
    for(const [x,z] of [[-8,2],[8,2],[-8,-2],[8,-2]]){box(.85,2.2,.18,mat(wall),x,1.1,z,true);box(.24,.24,.20,mat(red),x,1.25,z-.1,false);}
    const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(3.1,3.1,.34,12),mat(wallLight)));table.position.set(20,.72,20);scene.add(table);cyl(.50,1.6,mat(wood),20,.8,20,10,true);for(let i=0;i<6;i++){const a=i/6*TAU;const chair=box(.9,1.0,.9,mat(i%2?woodLight:wall),20+Math.cos(a)*4,.5,20+Math.sin(a)*4,true);chair.rotation.y=-a;}const teaPot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.72,8,6),mat(wall)));teaPot.position.set(20,1.25,20);scene.add(teaPot);
    const hedgeMat=mat(0xf7f7f2);const hedge=(x,z,w,d)=>{const h=box(w,1.75,d,hedgeMat,x,.88,z,true);addHatching(h,Math.max(.4,w*.7),1.15,Math.max(.08,d*.35),5,true);return h;};hedge(-24,-18,11,.75);hedge(-29,-23,.75,10);hedge(-20,-24,8,.75);hedge(-19,-18,.75,7);hedge(-25,-14,9,.75);hedge(-26,-21,5,.70);hedge(-22,-17,.70,4);hedge(-27,-16,3.5,.70);
    const rabbitPortals=[];function rabbitHole(x,z,team){const gg=new THREE.Group();gg.position.set(x,0,z);const arch=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.45,.30,7,18,Math.PI),mat(wall)));arch.rotation.z=Math.PI;arch.position.y=1.3;gg.add(arch);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.18,16),new THREE.MeshBasicMaterial({color:0x171510}));dark.position.set(0,1.18,.08);gg.add(dark);scene.add(gg);rabbitPortals.push({x,z,team});}
    rabbitHole(-18,30,'azul');rabbitHole(18,-30,'vermelho');window.__inkRabbitPortals=rabbitPortals;
  ...
  function shoot(){
    ...
    if(hit){
      ...
      if(remoteToken){
        const headshot=hit.object.userData?.hitZone==='head';
        const damage=isSniper?(headshot?100:60):(weaponMode==='smg'?(headshot?21:13):(headshot?34:22));
  ...
  function applyRemoteStates(states){
    ...
    for(const state of states||[]){
      ...
      if(!remote){remote=makeRemotePlayer(state);multiplayer.remotePlayers.set(state.token,remote);if(voiceEnabled&&String(multiplayer.token)<String(state.token))ensureVoicePeer(state.token,true);}
  ...
  const voiceBtn=document.createElement('button');voiceBtn.textContent='🎙 VOZ';voiceBtn.style.cssText='position:fixed;left:12px;top:48px;z-index:24;padding:9px 12px;border:3px solid #171510;border-radius:11px;background:rgba(252,251,247,.92);font:900 13px Georgia;color:#171510;display:none;pointer-events:auto';document.body.appendChild(voiceBtn);
  ...
  window.__inkRealtimeVoice=async msg=>{if(!voiceEnabled)return;const from=msg.from,s=msg.signal;if(!from||!s)return;const entry=await ensureVoicePeer(from,false);if(!entry)return;try{if(s.description){await entry.pc.setRemoteDescription(s.description);if(s.description.type==='offer'){const ans=await entry.pc.createAnswer();await entry.pc.setLocalDescription(ans);await window.__inkRealtimeSendVoice?.(from,{description:entry.pc.localDescription});}}else if(s.candidate)await entry.pc.addIceCandidate(s.candidate);}catch(_){}};
  async function setVoiceEnabled(on){if(on){try{voiceStream=voiceStream||await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false});voiceEnabled=true;voiceBtn.textContent='🎙 VOZ: ON';voiceBtn.style.background='#dff3e8';try{await audioCtx?.resume?.();}catch(_){}for(const token of multiplayer.remotePlayers.keys())if(String(multiplayer.token)<String(token))ensureVoicePeer(token,true);}catch(_){voiceEnabled=false;voiceBtn.textContent='🎙 SEM MICROFONE';}}else{voiceEnabled=false;voiceBtn.textContent='🎙 VOZ';for(const token of [...voicePeers.keys()])closeVoicePeer(token);voiceStream?.getTracks().forEach(t=>t.stop());voiceStream=null;}}
  ...
  function update(dt){
    if(controlsEditing)return;
    jumpPadCooldown=Math.max(0,jumpPadCooldown-dt);rabbitTunnelCooldown=Math.max(0,rabbitTunnelCooldown-dt);if(rabbitTunnelCooldown<=0&&window.__inkRabbitPortals?.length===2){const [a,b]=window.__inkRabbitPortals;if(Math.hypot(player.pos.x-a.x,player.pos.z-a.z)<1.75){player.pos.set(b.x,1.7,b.z+2.4);rabbitTunnelCooldown=2;hitNoticeEl.textContent='TOCA DO COELHO • VOCÊ SAIU DO OUTRO LADO!';hitNoticeEl.classList.add('show');hitNoticeTimer=1;}else if(Math.hypot(player.pos.x-b.x,player.pos.z-b.z)<1.75){player.pos.set(a.x,1.7,a.z-2.4);rabbitTunnelCooldown=2;hitNoticeEl.textContent='TOCA DO COELHO • VOCÊ SAIU DO OUTRO LADO!';hitNoticeEl.classList.add('show');hitNoticeTimer=1;}}
    ...
    for(let i=rocketProjectiles.length-1;i>=0;i--){
      ...
      if(rocket.remaining<=.01||rocket.life<=0){
        ...
        rocketProjectiles.splice(i,1);detonateRocket(impact,rocket.impactNormal);
      }
    }
