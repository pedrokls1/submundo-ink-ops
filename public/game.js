...
(() => {
  ...
  function weaponWallPush(){
    if(matchIntro)return 0;
    const dir=new THREE.Vector3(0,0,-1).applyEuler(new THREE.Euler(player.pitch,player.yaw,0,'YXZ')).normalize();
    raycaster.set(camera.position,dir);raycaster.far=1.15;
    const candidates=[];scene.traverse(o=>{if(!o.isMesh||o.userData.noBulletMark)return;let own=false;for(const v of Object.values(weaponVariants))v.traverse(w=>{if(w===o)own=true;});if(!own)candidates.push(o);});
    const h=raycaster.intersectObjects(candidates,false)[0];
    return h?Math.max(0,Math.min(1,(1.15-h.distance)/.72)):0;
  }
  ...
  function update(dt){
    ...
    if(weaponMode==='rifle'){
      const wallPush=weaponWallPush();
      weapon.position.set(-.08*ease+runSwayX,-player.recoil*.35-.02*ease-weaponKick*.035-runSwayY-wallPush*.22,-.10*ease+weaponKick*.045+wallPush*.78);
      weapon.rotation.x=player.recoil*.18-.012*ease+weaponKick*.08-wallPush*.28;
