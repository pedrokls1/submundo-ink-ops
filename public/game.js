...
(() => {
  ...
  for(const v of Object.values(weaponVariants))v.traverse(o=>{if(o.isLine||o.isLineSegments){o.visible=true;o.frustumCulled=true;if(o.material){o.material.color?.setHex(0x596273);o.material.opacity=.34;o.material.transparent=true;}}});
  // Detalhes vermelhos adicionais sem transformar a arma num bloco colorido.
  for(const [kind,v] of Object.entries(weaponVariants)){if(kind==='knife')continue;const accent=new THREE.Mesh(new THREE.BoxGeometry(.055,.055,kind==='sniper'?.72:.48),mat(0xb51f2e));accent.position.set(.405,-.115,kind==='sniper'?-1.36:-1.04);v.add(accent);}
  ...
  function makeRemotePlayer(state){
    ...
    // V56.30: sorriso maléfico assimétrico, claramente ABAIXO dos olhos.
    // Dois traços formam um meio-sorriso torto sem subir para a região do olho.
    const mouthA=new THREE.Mesh(new THREE.BoxGeometry(.13,.026,.035),darkMat);
    mouthA.position.set(-.045,1.855,-.425); mouthA.rotation.z=-.16; group.add(mouthA);
    const mouthB=new THREE.Mesh(new THREE.BoxGeometry(.105,.026,.035),darkMat);
    mouthB.position.set(.065,1.875,-.425); mouthB.rotation.z=.34; group.add(mouthB);
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.15,.18,.16,8),darkMat);
    neck.position.set(0,1.62,0); group.add(neck);

    const arms=[];
    // V56.29: braços convergem para a arma em vez de ficarem pendurados ao lado do tronco.
    for(const side of [-1,1]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.145,8,6),accentMat),2.6);
      shoulder.position.set(side*.43,1.42,-.06); group.add(shoulder);
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.10,.40,4,7),accentMat),2.6);
      arm.position.set(side<0?-.31:.32,1.22,side<0?-.30:-.27); arm.rotation.x=-1.12; arm.rotation.z=side<0?-.20:.15; group.add(arm); arms.push(arm);
      const hand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.115,8,6),paperMat),2.3);
      // Mão esquerda pega a frente/guarda; direita fica junto ao punho/gatilho.
      hand.position.set(side<0?-.01:.18,side<0?1.22:1.16,side<0?-.88:-.64); group.add(hand);
    }
    ...
    addHatching(gun,.12,.105,.48,5,true);
    const gunAccent=new THREE.Mesh(new THREE.BoxGeometry(.175,.045,.28),mat(0xb51f2e));
    gunAccent.position.set(.16,1.31,-.58); gunGroup.add(gunAccent); const gunAccent2=new THREE.Mesh(new THREE.BoxGeometry(.05,.16,.22),mat(0xb51f2e)); gunAccent2.position.set(.16,1.17,-.74); gunGroup.add(gunAccent2); group.add(gunGroup);
