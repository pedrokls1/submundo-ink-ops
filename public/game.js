// SUBMUNDO INK OPS V11 ESTABILIDADE
...
(() => {
  ...
  function getPlayerHeight(x,z){
    let best=1.7;if(window.__inkClimbPlatforms){for(const p of window.__inkClimbPlatforms){if(x>p.minX&&x<p.maxX&&z>p.minZ&&z<p.maxZ&&(p.minCurrentY==null||player.pos.y>=p.minCurrentY))best=Math.max(best,1.7+p.top);}}if(best>1.7)return best;
    if(window.__inkRabbitTunnelInfo&&window.__inkRabbitTunnelY){const ri=window.__inkRabbitTunnelInfo(x,z);if(ri.d<2.18){const edge=Math.min(ri.t,1-ri.t);if(edge<.18||player.pos.y<.80)return window.__inkRabbitTunnelY(ri.t);}}
    if(Math.abs(x)<2.6||Math.abs(x-30)<2.6||Math.abs(x+30)<2.6){const az=Math.abs(z);if(az<=3.5)return 2.20;if(az<5.9)return 1.70+(5.9-az)/2.4*.50;}return 1.7;
  }
  ...
  function spawnMushroomSmoke(m){for(let i=0;i<84;i++){const puff=new THREE.Mesh(new THREE.SphereGeometry(.68+Math.random()*.92,7,6),new THREE.MeshBasicMaterial({color:i%5===0?0x090d0b:(i%3===0?0x101713:0x18221d),transparent:true,opacity:.98,depthWrite:true}));const ang=Math.random()*TAU,r=Math.random()*5.0;puff.position.set(m.x+Math.cos(ang)*r,.55+Math.random()*4.5,m.z+Math.sin(ang)*r);puff.userData.noBulletMark=true;puff.userData.mushroomSmokeParticle=true;puff.raycast=()=>{};scene.add(puff);mushroomSmoke.push({mesh:puff,vx:(Math.random()-.5)*.28,vy:.04+Math.random()*.16,vz:(Math.random()-.5)*.28,life:7+Math.random()*1.6,max:8.6});}}
  ...
  function buildTeaParty(){const tx=27,tz=-25;const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(7.0,7.0,.70,16),mat(wallLight)));table.position.set(tx,2.2,tz);scene.add(table);cyl(1.0,4.4,mat(wood),tx,2.0,tz,10,true);for(let i=0;i<8;i++){if(i===4)continue;const a=i/8*TAU;const chair=box(2.2,4.0,2.0,mat(i%2?red:blue),tx+Math.cos(a)*10,2.0,tz+Math.sin(a)*9.2,true);chair.rotation.y=-a;}const pot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(2.0,10,7),mat(wall)));pot.position.set(tx,3.35,tz);scene.add(pot);cyl(.75,.35,mat(red),tx,5.0,tz,10,false);for(const [dx,dz] of [[-3.2,2.1],[3.0,-2.0],[1.8,3.7],[-1.2,-3.6]]){cyl(.85,1.3,mat(wall),tx+dx,3.0,tz+dz,10,false);const tea=cyl(.65,.05,mat(red),tx+dx,3.68,tz+dz,10,false);tea.userData.noBulletMark=true;}for(let i=0;i<6;i++){const top=.25+i*.34,w=4.8,d=2.7,xx=16.5+i*1.15,zz=-25;box(w,.22,d,mat(i===5?red:woodLight),xx,top-.11,zz,false);window.__inkClimbPlatforms.push({minX:xx-w/2,maxX:xx+w/2,minZ:zz-d/2,maxZ:zz+d/2,top,minCurrentY:i===0?1.65:1.7+(top-.34)-.12});}window.__inkClimbPlatforms.push({minX:20,maxX:34,minZ:-32,maxZ:-18,top:2.55,minCurrentY:3.05});makeWorldSign('CHÁ MALUCO',27,-37.5,0);}
  function buildRabbitTunnel(){
    const pts=[[-40,-31],[-40,-22],[-40,-8],[-40,12],[-35,27],[-20,34],[20,34],[35,27],[40,12],[40,22],[40,31]];
    ...
    window.__inkRabbitTunnelY=t=>{const ramp=.18;if(t<ramp)return 1.7-(t/ramp)*4.25;if(t>1-ramp)return 1.7-((1-t)/ramp)*4.25;return -2.55;};
    function entrance(x,z,rot){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const ring=addOutline(new THREE.Mesh(new THREE.TorusGeometry(2.15,.40,8,24,Math.PI),mat(wall)));ring.rotation.z=Math.PI;ring.position.y=1.72;gg.add(ring);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.72,20),new THREE.MeshBasicMaterial({color:0x141710,side:THREE.DoubleSide}));dark.position.set(0,1.50,.10);dark.userData.noBulletMark=true;gg.add(dark);scene.add(gg);makeWorldSign('TOCA DO COELHO',x+(x<0?4:-4),z+(z<0?-2.8:2.8),rot);}
    entrance(-40,-31,0);entrance(40,31,Math.PI);let acc=0;
    for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(y1-y0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floor=box(5.5,.30,len+1.7,mat(wallLight),mx,ym,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.66,wz=mz-Math.sin(yaw)*side*2.66;const wallSeg=box(.34,3.65,len+1.8,mat(wall),wx,ym+1.82,wz,false);wallSeg.rotation.y=yaw;}const roof=box(5.9,.36,len+1.9,mat(0xded8ca),mx,ym+3.62,mz,false);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;acc+=len;}
    let a2=0;for(let i=1;i<pts.length-1;i++){a2+=lens[i-1];const [x,z]=pts[i],t=a2/total,y=window.__inkRabbitTunnelY(t)-1.7;const cap=box(6.0,.38,6.0,mat(0xded8ca),x,y+3.62,z,false);cap.userData.rabbitRoof=true;}
  }
  ...
  function update(dt){
    ...
    if(grounded&&jumpPadCooldown<=0){
      ...
      if(onJumpPad){
        verticalVelocity=11.0;grounded=false;jumpPadCooldown=1.35;footstepSound(.22,190);
