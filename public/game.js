...
(() => {
  ...
  function makeRemotePlayer(state){
    ...
    const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.44,12,9),paperMat),3.2);
    ...
    const mouth=new THREE.Mesh(new THREE.TorusGeometry(.115,.025,5,10,Math.PI*.82),darkMat);
    mouth.position.set(0,1.88,-.414); mouth.rotation.z=Math.PI*1.10; group.add(mouth);
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.15,.18,.16,8),darkMat);
    neck.position.set(0,1.62,0); group.add(neck);

    const arms=[];
    for(const side of [-1,1]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.18,7,6),accentMat),2.8);
      shoulder.position.set(side*.48,1.42,-.02); group.add(shoulder);
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.115,.45,4,6),accentMat),2.8);
      arm.position.set(side*.50,1.17,-.12); arm.rotation.z=side*.14; arm.rotation.x=-.72; group.add(arm); arms.push(arm);
      const hand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.13,7,6),paperMat),2.5);
      hand.position.set(side*.43,.94,-.39); group.add(hand);
    }

    // Arma do personagem: preta, fina e curta o bastante para não esconder a silhueta.
    const gunGroup=new THREE.Group();
    const gun=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.16,.15,.88),darkMat),2.5);
    ...
    barrel.rotation.x=Math.PI/2; barrel.position.set(.16,1.25,-1.20); gunGroup.add(barrel);
  ...
  function update(dt){
    ...
    activeWeapon.scale.multiplyScalar(weaponMode==='knife'?.94:.88);if(weaponWallPush>.001){activeWeapon.position.z+=weaponWallPush*.72;activeWeapon.position.y-=weaponWallPush*.12;activeWeapon.rotation.x-=weaponWallPush*.42;activeWeapon.rotation.y+=weaponWallPush*.10;activeWeapon.scale.multiplyScalar(1-weaponWallPush*.06);}
