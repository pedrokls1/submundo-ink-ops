...
(() => {
  ...
  function makeRemotePlayer(state){
    ...
    head.position.y=2.00; group.add(head);
    ...
    headOutline.position.y=2.00; group.add(headOutline);
    const browMat=darkMat;
    for(const side of [-1,1]){
      const eye=new THREE.Mesh(new THREE.BoxGeometry(.17,.075,.045),browMat);
      eye.position.set(side*.15,2.02,-.405); eye.rotation.z=side*.20; group.add(eye);
    }
    ...
    mouth.position.set(0,1.855,-.425); mouth.rotation.z=.10; group.add(mouth);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.16,.205,.64,8),paperMat);
    neck.position.set(0,1.665,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.169,.214,.642,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    ...
    const shoulderL=new THREE.Vector3(-.55,1.39,-.005), shoulderR=new THREE.Vector3(.55,1.39,-.005);
