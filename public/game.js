...
(() => {
  ...
  function buildGround() {
    ...
    // V18: os recortes acompanham TODO o trecho inclinado, nao apenas a boca externa.
    // A rampa esquerda continua ate z~-12.7; a direita comeca em x~20.7.
    const holes=[[-45,-38,-39,-18.6],[27.0,30.5,45,37.5]];
    ...
    const ground=new THREE.Mesh(g,mat(paper));ground.rotation.x=-Math.PI/2;ground.position.y=-.06;ground.receiveShadow=true;scene.add(ground);
  ...
  function buildTeaParty(){
    ...
    const steps=14;for(let i=0;i<steps;i++){const top=.18+i*((tableTop-.02-.18)/(steps-1)),w=5.25,d=2.55,xx=13.6+i*.94,zz=-25;box(w,.16,d,mat(i===steps-1?red:woodLight),xx,top-.08,zz,false);}window.__inkTeaRamp={minX:11.0,maxX:27.15,minZ:-26.65,maxZ:-23.35,startX:11.0,endX:27.15,startTop:.08,endTop:tableTop};
  ...
  function buildRabbitTunnel(){
    ...
    let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],pieces=Math.max(1,Math.ceil(len/2.6));for(let j=0;j<pieces;j++){const u0=j/pieces,u1=(j+1)/pieces,um=(u0+u1)/2;const x0=ax+(bx-ax)*u0,z0=az+(bz-az)*u0,x1=ax+(bx-ax)*u1,z1=az+(bz-az)*u1,mx=(x0+x1)/2,mz=(z0+z1)/2,segLen=Math.hypot(x1-x0,z1-z0),t0=(acc+len*u0)/total,t1=(acc+len*u1)/total,tm=(t0+t1)/2,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(x1-x0,z1-z0),pitch=Math.atan2(y1-y0,segLen);
      const floorMat=mat(wallLight);floorMat.side=THREE.DoubleSide;const floor=box(5.55,.10,segLen+.12,floorMat,mx,ym,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;
      ...
      if(window.__inkRabbitTunnelY(tm)<-1.75&&tm>.16&&tm<.84){const roofMat=mat(0xe8e2d5);roofMat.side=THREE.DoubleSide;const roof=box(5.90,.24,segLen+.25,roofMat,mx,ym+3.18,mz,false);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;}
    }acc+=len;}
    // V26: casca externa continua da toca. As paredes internas segmentadas continuam intactas,
    // mas por fora nao e mais possivel enxergar a sequencia de segmentos do tunel.
    // A casca fica ligeiramente PARA FORA das paredes existentes, portanto nao invade o corredor.
    const shellMat=mat(wall); shellMat.side=THREE.DoubleSide;
    const addTunnelShell=(ax,az,bx,bz)=>{
      const dx=bx-ax,dz=bz-az,len=Math.hypot(dx,dz); if(len<.01)return;
      const mx=(ax+bx)/2,mz=(az+bz)/2,yaw=Math.atan2(dx,dz);
      // Apenas o trecho realmente subterraneo: a boca/rampa permanece aberta como na V25.
      const samples=28;
      for(let k=0;k<samples;k++){
        const u0=k/samples,u1=(k+1)/samples,um=(u0+u1)/2;
        const x=ax+dx*um,z=az+dz*um,ri=window.__inkRabbitTunnelInfo(x,z);
        if(!ri || ri.y>-1.70)continue;
        const seg=len/samples+.10;
        const floorY=ri.y-1.7;
        // sobe ate pouco acima do nivel do terreno para esconder a lateral vista da superficie.
        const topY=.18,h=Math.max(3.35,topY-floorY);
        for(const side of [-1,1]){
          const wx=x+Math.cos(yaw)*side*2.94,wz=z-Math.sin(yaw)*side*2.94;
          const panel=box(.24,h,seg,shellMat,wx,floorY+h/2,wz,false);
          panel.rotation.y=yaw; panel.userData.rabbitOuterShell=true;
        }
      }
    };
    // Segmentos principais da toca; a casca acompanha o caminho sem criar parede no miolo.
    for(let si=0;si<pts.length-1;si++) addTunnelShell(pts[si][0],pts[si][1],pts[si+1][0],pts[si+1][1]);
    ...
    for(let i=1;i<pts.length-1;i++){
      ...
      const outerX=cx + (sz1||sz2)*-2.76;
      const outerZ=cz + (sx1||sx2)* 2.76;
      const a=box(.30,h,5.8,cornerMat,outerX,cy+1.62,cz,false);a.userData.rabbitWall=true;
      const b=box(5.8,h,.30,cornerMat,cx,cy+1.62,outerZ,false);b.userData.rabbitWall=true;
      colliders.push({minX:outerX-.18,maxX:outerX+.18,minZ:cz-2.9,maxZ:cz+2.9,minY:cy-.15,height:cy+h});
      colliders.push({minX:cx-2.9,maxX:cx+2.9,minZ:outerZ-.18,maxZ:outerZ+.18,minY:cy-.15,height:cy+h});
    }
  ...
  function update(dt){
    ...
    activeWeapon.scale.multiplyScalar(weaponMode==='knife'?.96:.94);if(weaponWallPush>0){activeWeapon.position.z+=weaponWallPush*.72;activeWeapon.position.y-=weaponWallPush*.12;activeWeapon.rotation.x-=weaponWallPush*.52;activeWeapon.rotation.y+=weaponWallPush*.16;activeWeapon.scale.multiplyScalar(1-weaponWallPush*.08);}
