...
(() => {
  ...
  function buildTeaParty(){
    ...
    const steps=12;for(let i=0;i<steps;i++){const top=.18+i*((tableTop-.10-.18)/(steps-1)),w=5.1,d=2.35,xx=13.9+i*.92,zz=-25;box(w,.16,d,mat(i===steps-1?red:woodLight),xx,top-.08,zz,false);}window.__inkTeaRamp={minX:11.4,maxX:26.5,minZ:-26.55,maxZ:-23.45,startX:11.4,endX:26.5,startTop:.08,endTop:tableTop};
  ...
  function update(dt){
    ...
    activeWeapon.scale.multiplyScalar(weaponMode==='knife'?.90:.78);if(weaponWallPush>0){activeWeapon.position.z+=weaponWallPush*1.30;activeWeapon.position.y-=weaponWallPush*.30;activeWeapon.rotation.x-=weaponWallPush*.38;activeWeapon.scale.multiplyScalar(1-weaponWallPush*.38);if(weaponWallPush>.88)activeWeapon.visible=false;}
