...
(() => {
  ...
  function buildPortMap(){
    ...
    const hangar=(x,z,color,flip)=>{box(18,.25,16,mat(0xe8e5dc),x,.12,z,false);box(18,6,.55,mat(0xbfc2c0),x,3,z+flip*7.8,true);for(const px of [-6,-2,2,6])box(.18,5.2,.18,mat(color),x+px,2.6,z+flip*7.45,false);makeMapLabel('HANGAR '+(flip<0?'AZUL':'VERMELHO'),x,4.8,z+flip*7.35,1.8);};
    ...
    container(-39,-14,0,0x6b86a4,true,true);container(39,14,0,0xa75c50,true,true);
    ...
    for(const x of [-15,15])box(.7,14,.7,mat(0xd2a84c),x,7,0,true);box(31,.65,.8,mat(0xd2a84c),0,13.6,0,false);portCraneTrolley=new THREE.Group();const trolley=addOutline(new THREE.Mesh(new THREE.BoxGeometry(2.4,.8,1.5),mat(0xd2a84c)));trolley.position.y=13.15;portCraneTrolley.add(trolley);const cable=new THREE.Mesh(new THREE.CylinderGeometry(.10,.10,5.0,6),mat(black));cable.position.y=10.25;portCraneTrolley.add(cable);const hook=addOutline(new THREE.Mesh(new THREE.TorusGeometry(.65,.18,6,10,Math.PI*1.35),mat(black)));hook.position.y=7.55;hook.rotation.z=.2;portCraneTrolley.add(hook);scene.add(portCraneTrolley);
    ...
    portObjects=scene.children.filter(o=>!before.has(o));for(let i=c0;i<colliders.length;i++){colliders[i].map='porto';portColliders.push(colliders[i]);}
  }
  buildPortMap();
  ...
  function shoot(){
    ...
    const worldMeshes=[];
    scene.traverse(o=>{
      if(!o.isMesh || o.userData.noBulletMark || o.userData.mushroomInactive || o.userData.mushroomSmokeParticle || (activeMap==='quarto'&&(o.userData.mushroomId||o.userData.cheshire))) return;
