...
(() => {
  ...
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const grips={
      rifle:{rear:[.39,-.56,-.60],front:[.40,-.43,-1.43],rearRoot:[.94,-1.18,.28],frontRoot:[-.82,-1.16,.12]},
      sniper:{rear:[.39,-.58,-.83],front:[.40,-.43,-1.67],rearRoot:[.96,-1.20,.34],frontRoot:[-.86,-1.18,.08]},
      smg:{rear:[.40,-.58,-.68],front:[.41,-.43,-1.28],rearRoot:[.92,-1.18,.32],frontRoot:[-.78,-1.15,.15]},
      shotgun:{rear:[.41,-.58,-.68],front:[.42,-.43,-1.76],rearRoot:[.96,-1.20,.30],frontRoot:[-.88,-1.18,.04]},
      launcher:{rear:[.39,-.58,-.95],front:[.40,-.43,-1.72],rearRoot:[.98,-1.20,.38],frontRoot:[-.88,-1.18,.06]}
    }[kind];
    const addViewLimb=(rootArr,ptArr,front=false)=>{
      ...
      arm.position.copy(mid); arm.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir.normalize()); arm.userData.keepViewColor=true; v.add(arm);
      const cuff=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.16,.145,.18,8),sleeveMat),2.1);
      cuff.position.copy(pt).add(new THREE.Vector3(front?-.035:.035,-.085,.055)); cuff.rotation.z=front?.18:-.18; cuff.userData.keepViewColor=true; v.add(cuff);
      const hg=new THREE.Group(); hg.position.copy(pt);
  ...
  function makeRemotePlayer(state){
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.14,.185,.42,8),paperMat);
    neck.position.set(0,1.715,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.148,.194,.422,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    ...
    const armBetween=(a,b)=>{
      ...
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.116,Math.max(.10,len-.22),4,8),accentMat),2.4);
    ...
    const shoulderL=new THREE.Vector3(-.53,1.47,-.005), shoulderR=new THREE.Vector3(.53,1.47,-.005);
    // V56.39: cotovelos realmente abertos para que os dois braços apareçam na silhueta frontal.
    const elbowL=new THREE.Vector3(-.72,1.24,-.30), elbowR=new THREE.Vector3(.74,1.22,-.17);
    const handLPos=new THREE.Vector3(.08,1.20,-.96), handRPos=new THREE.Vector3(.15,1.18,-.53);
