...
(() => {
  ...
  function buildRabbitTunnel(){
    ...
    // V20: acabamento independente das duas bocas da toca. O terreno V18 permanece intacto;
    // estas pecas apenas definem visualmente a entrada e escondem a borda do recorte sem criar um piso falso.
    function rabbitEntranceFrame(x,z,yaw){
      const gm=mat(wall);gm.side=THREE.DoubleSide;
      const lipMat=mat(wallLight);lipMat.side=THREE.DoubleSide;
      // laterais curtas, fora do vao caminhavel
      for(const side of [-1,1]){
        const lx=x+Math.cos(yaw)*side*2.86,lz=z-Math.sin(yaw)*side*2.86;
        const w=box(.34,1.45,3.6,gm,lx,.67,lz,false);w.rotation.y=yaw;w.userData.rabbitWall=true;
      }
      // soleira fina somente na borda externa; nao cobre a rampa nem o personagem
      const sx=x-Math.sin(yaw)*1.58,sz=z-Math.cos(yaw)*1.58;
      const lip=box(5.55,.10,.30,lipMat,sx,.015,sz,false);lip.rotation.y=yaw;
    }
    rabbitEntranceFrame(-42,-34,0);rabbitEntranceFrame(42,34,Math.PI);
  ...
  function applyRemoteStates(states){
    ...
    for(const state of states||[]){
      ...
      remote.alive=alive;/* V20 TOCA: jogadores continuam visiveis durante toda a transicao superficie/subterraneo. A regra antiga escondia o boneco assim que um jogador cruzava y=0. */remote.group.visible=alive;
    }
