...
(() => {
  ...
  let bedroomObjects=[], kingdomObjects=[], bedroomColliders=[], outletCooldown=0, phonePhase=0, bedroomLightOff=0, bedroomMatchElapsed=0, bedroomPhoneRingMask=0, duckBounceCooldown=0;
  ...
  function buildGiantBedroom(){
    ...
    const sw=box(.28,3.0,2.4,mat(0xf7f0df),48.72,2.4,7,false);sw.userData.lightSwitch=true;const lever=box(.34,.75,.65,mat(black),48.50,2.45,7,false);lever.userData.lightSwitch=true;
  ...
  function shoot(){
    ...
    if(activeMap==='quarto'&&hit?.object?.userData?.lightSwitch){bedroomLightOff=5;hemi.intensity=.42;sun.intensity=.32;setTimeout(()=>{if(bedroomLightOff<=0){hemi.intensity=2.1;sun.intensity=2.6;}},5100);hitNoticeEl.textContent='CLICK! • LUZES APAGADAS';hitNoticeEl.classList.add('show');hitNoticeTimer=1.1;}
  ...
  function update(dt){
    ...
    outletCooldown=Math.max(0,outletCooldown-dt);bedroomLightOff=Math.max(0,bedroomLightOff-dt);duckBounceCooldown=Math.max(0,duckBounceCooldown-dt);
    if(activeMap==='quarto'){
      ...
      if(grounded&&duckBounceCooldown<=0&&onDuck){verticalVelocity=12.8;grounded=false;duckBounceCooldown=1.15;player.pos.z+=2.0;duckQuackSound();hitNoticeEl.textContent='QUACK!  BOING!';hitNoticeEl.classList.add('show');hitNoticeTimer=.75;}
