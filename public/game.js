...
(() => {
  ...
  const bedroomOutlets=[[-48.15,-24],[-48.15,18],[48.15,-20],[48.15,24],[-24,-39.15],[20,-39.15],[-22,39.15],[24,39.15]];
  ...
  // V56.1 — QUARTO GIGANTE: quarto mais alto, cama lateral, centro com coberturas e 8 tomadas.
  function buildGiantBedroom(){
    ...
    // Proteção na frente dos spawns: os times não se enxergam imediatamente ao nascer.
    box(16,4.5,2.8,softBlue,0,2.25,29,true);box(16,4.5,2.8,softRed,0,2.25,-29,true);
    // Livros/degraus para chegar à cama.
    const steps=[[-22,-9,1.1],[-24,-6,2.1],[-26,-3,3.1],[-28,0,4.15]];for(let i=0;i<steps.length;i++){const [x,z,top]=steps[i];box(5.5,top,4.5,mat(i%2?0xd8b46c:0x9eb0c8),x,top/2,z,true);}
    const pencil=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.48,.48,20,8),mat(0xe5c74f)));pencil.rotation.z=Math.PI/2;pencil.position.set(-20,6.15,7);scene.add(pencil);
    ...
    // Slinky junto da cama.
    const spring=new THREE.Group();for(let i=0;i<14;i++){const ring=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.45,.10,6,20),mat(i%2?red:toyYellow)));ring.rotation.x=Math.PI/2;ring.position.y=.22+i*.16;spring.add(ring);}spring.position.set(-20,0,-2);scene.add(spring);makeMapLabel('BOING!',-20,2.8,-2,.85);
  ...
  function applyMapVisibility(){
    kingdomObjects.forEach(o=>o.visible=activeMap==='reino');bedroomObjects.forEach(o=>o.visible=activeMap==='quarto');
  ...
  function update(dt){
    ...
    if(activeMap==='quarto'){
      // Slinky: lança para a cama lateral.
      if(grounded&&jumpPadCooldown<=0&&Math.hypot(player.pos.x+20,player.pos.z+2)<1.8){verticalVelocity=13.8;grounded=false;jumpPadCooldown=1.4;player.pos.x-=1.4;hitNoticeEl.textContent='BOOOING!';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;}
