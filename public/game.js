...
(() => {
  ...
  function makeWorldSign(text,x,z,rot=0){const group=new THREE.Group();group.position.set(x,0,z);group.rotation.y=rot;for(const sx of [-1,1]){const post=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.10,.12,2.15,7),mat(black)));post.position.set(sx*1.45,1.05,0);group.add(post);}const board=addOutline(new THREE.Mesh(new THREE.BoxGeometry(3.9,1.15,.18),mat(wallLight)));board.position.set(0,2.05,0);group.add(board);const c=document.createElement('canvas');c.width=512;c.height=160;const ctx=c.getContext('2d');ctx.fillStyle='#fffdf7';ctx.fillRect(0,0,512,160);ctx.strokeStyle='#171510';ctx.lineWidth=10;ctx.strokeRect(6,6,500,148);ctx.fillStyle='#171510';ctx.font='900 42px Georgia';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(String(text).slice(0,24),256,82);const tx=new THREE.CanvasTexture(c);tx.colorSpace=THREE.SRGBColorSpace;const face=new THREE.Mesh(new THREE.PlaneGeometry(3.58,.92),new THREE.MeshBasicMaterial({map:tx,side:THREE.DoubleSide,polygonOffset:true,polygonOffsetFactor:-3}));face.position.set(0,2.05,-.105);group.add(face);scene.add(group);return group;}
  ...
  function makeTreePropLegacy(x,z,s=1){return null;}
  ...
  function buildRabbitTunnel(){const pts=[[-40,-31],[-45,-20],[-45,-5],[-40,10],[-30,23],[-15,32],[3,36],[22,35],[36,31],[40,31]];window.__inkRabbitPath=pts;window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0},total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz);if(d<best.d)best={d,t:(acc+lens[i]*u)/total};acc+=lens[i];}return best;};window.__inkRabbitTunnelY=t=>{const edge=Math.min(t,1-t);return edge<.07?1.7-(edge/.07)*4.8:-3.10;};function entrance(x,z,rot,signX,signZ){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const arch=addOutline(new THREE.Mesh(new THREE.TorusGeometry(2,.42,8,24,Math.PI),mat(wall)));arch.rotation.z=Math.PI;arch.position.y=1.72;gg.add(arch);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.68,20),new THREE.MeshBasicMaterial({color:black,side:THREE.DoubleSide}));dark.position.set(0,1.50,.11);gg.add(dark);for(const sx of [-1,1]){const ear=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.24,.92,4,6),mat(wallLight)));ear.position.set(sx*.52,3.08,0);ear.rotation.z=sx*.16;gg.add(ear);}scene.add(gg);makeWorldSign('TOCA DO COELHO',signX,signZ,rot);return {x,z};}window.__inkRabbitEntrances=[entrance(-40,-31,0,-35.5,-34.5),entrance(40,31,Math.PI,35.5,34.5)];let total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,tm=(t0+t1)/2,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(y1-y0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floor=box(5,.22,len+1.25,mat(wallLight),mx,ym,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.48,wz=mz-Math.sin(yaw)*side*2.48;const wallSeg=box(.32,3.55,len+1.35,mat(wall),wx,ym+1.72,wz,false);wallSeg.rotation.y=yaw;}if(i>0&&i<pts.length-2){const roof=box(5.25,.30,len+1.45,mat(0xe3ded2),mx,ym+3.48,mz,false);roof.rotation.y=yaw;roof.userData.rabbitRoof=true;}acc+=len;}for(let i=1;i<pts.length-1;i++){const [x,z]=pts[i],t=i/(pts.length-1),y=window.__inkRabbitTunnelY(t)-1.7;const cap=box(5.35,.32,5.35,mat(0xe3ded2),x,y+3.48,z,false);cap.rotation.y=.15*i;cap.userData.rabbitRoof=true;for(const side of [-1,1])cyl(.32,3.6,mat(wall),x+side*2.48,y+1.75,z,8,false);}}
