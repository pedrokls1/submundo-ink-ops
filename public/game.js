...
(() => {
  ...
  function detonateRocket(position,impactNormal=null){
    ...
    for(const [token,remote] of multiplayer.remotePlayers){const d=remote.group.position.distanceTo(position);if(d<=4.8)reportRemoteHit(token,d<=3.2?100:48,false);}if(activeMap==='reino')for(const m of destructibleMushrooms.values())if(m.active&&Math.hypot(m.x-position.x,m.z-position.z)<=5.2)damageMushroom(m.id,999);
  ...
  function getPlayerHeight(x,z){if(activeMap==='porto'){let h=1.7;const decks=[[-41,-20,-30,-8,4.45],[30,8,41,20,4.45],[-13,-6,-4,6,4.45],[4,-6,13,6,4.45]];for(const [minX,minZ,maxX,maxZ,top] of decks)if(x>minX&&x<maxX&&z>minZ&&z<maxZ&&player.pos.y>top-.8)h=Math.max(h,1.7+top);return h;}if(activeMap==='quarto'){let h=1.7;const onBed=x>-46&&x<-24&&z>-17&&z<17;if(onBed&&player.pos.y>5.05)h=7.35;const onPillow=x>-44&&x<-25&&z>8&&z<14;if(onPillow&&player.pos.y>6.0)h=8.10;const onRedTable=x>29.7&&x<48.2&&z>-38&&z<-28;if(onRedTable&&player.pos.y>6.65)h=9.40;const onBlueTable=x>29.7&&x<48.2&&z>28&&z<38;if(onBlueTable&&player.pos.y>6.65)h=9.40;const onPhone=x>36.4&&x<41.6&&z>-36.8&&z<-29.2;if(onPhone&&player.pos.y>8.75)h=9.93;return h;}let best=1.7;for(const m of destructibleMushrooms.values()){if(!m.active)continue;const d=Math.hypot(x-m.x,z-m.z);const top=m.top||3.72*(m.s||1);if(d<(m.standRadius||1.72*(m.s||1))&&player.pos.y>=1.7+top-.72)best=Math.max(best,1.7+top);}if(window.__inkTeaRamp){const r=window.__inkTeaRamp;if(x>=r.minX&&x<=r.maxX&&z>=r.minZ&&z<=r.maxZ){const u=Math.max(0,Math.min(1,(x-r.startX)/(r.endX-r.startX)));best=Math.max(best,1.7+r.startTop+(r.endTop-r.startTop)*u);}}if(window.__inkClimbPlatforms){for(const p of window.__inkClimbPlatforms){const inside=p.radius!=null?Math.hypot(x-p.cx,z-p.cz)<p.radius:(x>p.minX&&x<p.maxX&&z>p.minZ&&z<p.maxZ);if(inside&&(p.minCurrentY==null||player.pos.y>=p.minCurrentY))best=Math.max(best,1.7+p.top);}}if(best>1.7)return best;if(window.__inkRabbitTunnelInfo&&window.__inkRabbitTunnelY){const ri=window.__inkRabbitTunnelInfo(x,z);const edge=Math.min(ri.t,1-ri.t);if(ri.d<2.22&&(edge<.105||player.pos.y<.75))return ri.y??window.__inkRabbitTunnelY(ri.t);}if(Math.abs(x)<2.6||Math.abs(x-32)<2.6||Math.abs(x+32)<2.6){const az=Math.abs(z);if(az<=4.2)return 2.20;if(az<6.9)return 1.70+(6.9-az)/2.7*.50;}return 1.7;}
  ...
  function buildPortMap(){
    ...
    const asphalt=box(100,.16,82,mat(0xd9d7cf),0,-.08,0,false); addHatching(asphalt,88,.05,70,18,false);
    // Limites: galpões nas laterais e muralhas/grades nas pontas.
    box(100,5,.7,mat(0xc9c7bf),0,2.5,-41,true); box(100,5,.7,mat(0xc9c7bf),0,2.5,41,true);
    box(.7,5,82,mat(0xc9c7bf),-50,2.5,0,true); box(.7,5,82,mat(0xc9c7bf),50,2.5,0,true);
    ...
    const ramp=(x,z,dir,color)=>{for(let i=0;i<5;i++)box(4.4,.55+i*.75,3.0,mat(color),x,z==0?(.28+i*.38):(.28+i*.38),z+dir*(i*2.45),true);};
    ramp(-39,-1,-1,0xb8c7d8);ramp(39,1,1,0xd9bbb2);
    // Guindaste central como marco visual; pernas deixam passagem por baixo.
    for(const x of [-15,15])box(.7,10,.7,mat(0xd2a84c),x,5,0,true);box(31,.65,.8,mat(0xd2a84c),0,9.6,0,false);box(.35,6,.35,mat(black),0,6.4,0,false);const hook=addOutline(new THREE.Mesh(new THREE.TorusGeometry(.65,.18,6,10,Math.PI*1.35),mat(black)));hook.position.set(0,3.1,0);hook.rotation.z=.2;scene.add(hook);
    ...
    for(const [z,color,sgn] of [[35,blue,-1],[-35,red,1]]){const ring=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(5.2,5.2,.08,24),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.20})));ring.position.set(0,.05,z);scene.add(ring);box(7,1.65,1.0,mat(0xc8a85c),0,.82,z+sgn*6,true);box(2.2,2.4,2.2,mat(color),-6,1.2,z+sgn*4,true);box(2.2,2.4,2.2,mat(color),6,1.2,z+sgn*4,true);}
