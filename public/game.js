...
(() => {
  ...
  function buildKnife(){
    ...
    {
      const pts=[]; for(let i=0;i<5;i++){const t=(i+1)/6, y=-.23+t*.46;pts.push(new THREE.Vector3(-.045,y,.047),new THREE.Vector3(.045,y+.08,.047));}
      const hatch=new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(pts),new THREE.LineBasicMaterial({color:hatchInk,transparent:true,opacity:.48}));
      blade.add(hatch);
    }
  ...
  function addInkPotMagazine(v,kind){
    ...
    const pot=new THREE.Group(); pot.userData.inkPotMagazine=true;
    ...
    const pos={rifle:[.40,-.70,-.90],sniper:[.40,-.69,-1.15],smg:[.42,-.72,-1.18],shotgun:[.42,-.67,-1.14]}[kind]||[.40,-.70,-.92];
  ...
  // V56.38: braços/antebraços mais visíveis e conectados às mãos no viewmodel.
  // V56.37: braços + mãos fazem parte do viewmodel. Antes aparecia quase só a mão em algumas armas.
  // V56.39: braços completos na primeira pessoa. Cada arma recebe uma pegada própria;
  // o membro entra pela borda inferior da câmera, passa pelo antebraço/punho e termina na mão.
  // Isso evita a antiga impressão de uma mão flutuando ao lado da arma.
  for(const [kind,v] of Object.entries(weaponVariants)){
    if(kind==='knife')continue;
    const handMat=mat(0xfffbef), sleeveMat=mat(0x315f9d);
    const grips={
      rifle:{rear:[.39,-.56,-.60],front:[.40,-.43,-1.43],rearRoot:[.94,-1.30,.28],frontRoot:[-.86,-1.28,.14]},
      sniper:{rear:[.39,-.58,-.83],front:[.40,-.43,-1.67],rearRoot:[.96,-1.32,.34],frontRoot:[-.90,-1.30,.10]},
      smg:{rear:[.40,-.58,-.68],front:[.41,-.43,-1.28],rearRoot:[.92,-1.29,.32],frontRoot:[-.82,-1.26,.17]},
      shotgun:{rear:[.41,-.58,-.68],front:[.42,-.43,-1.76],rearRoot:[.96,-1.32,.30],frontRoot:[-.91,-1.30,.08]},
      launcher:{rear:[.39,-.58,-.95],front:[.40,-.43,-1.72],rearRoot:[.98,-1.32,.38],frontRoot:[-.91,-1.30,.09]}
    }[kind];
    const addViewLimb=(rootArr,ptArr,front=false)=>{
      const root=new THREE.Vector3(...rootArr), pt=new THREE.Vector3(...ptArr);
      const dir=pt.clone().sub(root), len=dir.length(), mid=root.clone().add(pt).multiplyScalar(.5);
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.16,Math.max(.24,len-.25),4,8),sleeveMat),2.35);
      arm.position.copy(mid); arm.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir.normalize()); arm.userData.keepViewColor=true; arm.userData.fpLimb=true; arm.userData.fpSupport=front; v.add(arm);
      const cuff=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.16,.145,.18,8),sleeveMat),2.1);
      cuff.position.copy(pt).add(new THREE.Vector3(front?-.035:.035,-.085,.055)); cuff.rotation.z=front?.18:-.18; cuff.userData.keepViewColor=true; cuff.userData.fpLimb=true; cuff.userData.fpSupport=front; v.add(cuff);
      const hg=new THREE.Group(); hg.position.copy(pt); hg.userData.fpLimb=true; hg.userData.fpSupport=front;
    ...
    addViewLimb(grips.rearRoot,grips.rear,false);
    addViewLimb(grips.frontRoot,grips.front,true);
  }
  // V56.41: impede hachuras das armas em 1ª pessoa de ultrapassarem a própria peça.
  // A função global addHatching continua intacta para cenário/mapas.
  for(const v of Object.values(weaponVariants)){
    v.traverse(o=>{
      if(!o.isLineSegments||!o.geometry?.attributes?.position)return;
      const a=o.geometry.attributes.position;
      for(let i=0;i<a.count;i++){
        a.setX(i,Math.max(-.30,Math.min(.30,a.getX(i))));
        a.setY(i,Math.max(-.42,Math.min(.42,a.getY(i))));
      }
  ...
  function setWeaponMode(mode){
    ...
    for(const key of weaponModes) setSupportViewArmVisible(key,true);
  ...
  function setSupportViewArmVisible(kind,visible){
    const v=weaponVariants[kind]; if(!v)return;
    v.children.forEach(o=>{if(o.userData?.fpSupport===true)o.visible=visible;});
  }
  ...
  function reloadWeapon(){
    ...
    reloadBtn.textContent='↻...';
    setSupportViewArmVisible(reloadMode,false);
    if(!reloadCartridge){
      ...
      // V56.40: o pote NÃO cria uma terceira mão. A mão/braço de apoio normal sai da arma durante a recarga.
      // Braço de apoio em movimento: substitui temporariamente o braço normal durante a recarga.
      const movingArm=new THREE.Group(); movingArm.userData.reloadSupportArm=true;
      const rSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.145,.78,4,8),mat(0x315f9d)),2.3);
      rSleeve.rotation.z=-.52; rSleeve.position.set(-.55,-.42,.12); rSleeve.userData.keepViewColor=true; movingArm.add(rSleeve);
      const rHand=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.25,.19,.27),mat(0xfffbef)),2.35);
      rHand.position.set(-.23,-.10,.06); rHand.rotation.z=-.18; rHand.userData.keepViewColor=true; movingArm.add(rHand);
      const rFingers=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.20,.10,.23),mat(0xfffbef)),2.0);
      rFingers.position.set(-.22,-.18,.035); rFingers.userData.keepViewColor=true; movingArm.add(rFingers);
      reloadCartridge.add(movingArm);
      reloadCartridge.scale.setScalar(.70); reloadCartridge.position.set(-.30,-.66,-.78); reloadCartridge.rotation.set(-.15,.12,-.22); camera.add(reloadCartridge);
    }
    ...
    setTimeout(()=>{
      ...
      ammo+=take; reserveAmmo[reloadMode]-=take; reloadTimer=0;if(reloadCartridge)reloadCartridge.visible=false;setSupportViewArmVisible(reloadMode,true);
  ...
  function makeRemotePlayer(state){
    ...
    for(const x of [-.25,.25]){
      const leg=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.165,.50,4,7),accentMat),3.0);
      leg.position.set(x,.42,0); leg.rotation.z=x<0?.055:-.055; group.add(leg); legs.push(leg);
      const boot=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.31,.17,.46),darkMat),2.7);
    ...
    body.scale.set(1.20,1.04,.80); body.position.set(0,1.18,0); group.add(body); // V56.37: corpo voltou a ser realmente adicionado ao grupo; fecha o vazado do tronco.
    ...
    head.position.y=2.16; group.add(head);
    ...
    headOutline.position.y=2.16; group.add(headOutline);
    const browMat=darkMat;
    for(const side of [-1,1]){
      const eye=new THREE.Mesh(new THREE.BoxGeometry(.17,.075,.045),browMat);
      eye.position.set(side*.15,2.18,-.405); eye.rotation.z=side*.20; group.add(eye);
    }
    ...
    mouth.position.set(0,2.015,-.425); mouth.rotation.z=.10; group.add(mouth);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.155,.195,.58,8),paperMat);
    neck.position.set(0,1.74,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.164,.204,.582,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    ...
    trapezius.position.set(0,1.37,0); trapezius.scale.set(1.04,.66,.76); group.add(trapezius);
    ...
    const armBetween=(a,b)=>{
      ...
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.121,Math.max(.10,len-.23),4,8),accentMat),2.4);
    ...
    const shoulderL=new THREE.Vector3(-.50,1.43,-.005), shoulderR=new THREE.Vector3(.50,1.43,-.005);
    // V56.39: cotovelos realmente abertos para que os dois braços apareçam na silhueta frontal.
    const elbowL=new THREE.Vector3(-.68,1.12,-.30), elbowR=new THREE.Vector3(.69,1.10,-.17);
    const handLPos=new THREE.Vector3(.08,1.17,-.96), handRPos=new THREE.Vector3(.15,1.15,-.53);
    for(const pos of [shoulderL,shoulderR]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.102,8,6),accentMat),2.2);
    ...
    // V56.42: pescoço é deliberadamente BODY. Só a esfera real da cabeça/rosto pode gerar HEADSHOT.
    neck.userData.hitZone='body'; neckOutline.userData.hitZone='body'; trapezius.userData.hitZone='body';
    // V56.38: headshot usa SOMENTE a geometria real da cabeça/rosto.
    // Pescoço, trapézio, ombros e tórax nunca viram HEAD por estarem acima de uma altura arbitrária.
    head.userData.hitZone='head'; headOutline.userData.hitZone='head';
    for(const o of [mouth]) o.userData.hitZone='head';
    // Os dois olhos continuam visualmente idênticos; por estarem colados à esfera, recebem a zona da cabeça.
    for(const o of group.children){if(o.isMesh && o.geometry?.type==='BoxGeometry' && Math.abs(o.position.y-2.02)<.03 && Math.abs(o.position.z+.405)<.06)o.userData.hitZone='head';}
    neck.userData.hitZone='body'; neckOutline.userData.hitZone='body'; trapezius.userData.hitZone='body';
  ...
  function update(dt){
    ...
    if(reloadCartridge?.visible){reloadCartridge.userData.reloadLife=Math.max(0,(reloadCartridge.userData.reloadLife||0)-dt);const total=Math.max(.01,reloadCartridge.userData.reloadTotal||.95),p=1-Math.min(1,reloadCartridge.userData.reloadLife/total),insert=Math.max(0,Math.min(1,(p-.38)/.42));reloadCartridge.rotation.y=-.28+Math.sin(p*Math.PI)*.34;reloadCartridge.rotation.z=-.16+.10*Math.sin(p*Math.PI*2);reloadCartridge.position.y=-.66+Math.sin(Math.min(1,p/.78)*Math.PI)*.31+insert*.13;reloadCartridge.position.x=-.30+insert*.56;reloadCartridge.position.z=-.78-insert*.35;}
