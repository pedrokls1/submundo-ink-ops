...
(() => {
  ...
  function buildPortMap(){
    ...
    const hangar=(x,z,color,flip)=>{box(18,.25,16,mat(0xe8e5dc),x,.12,z,false);box(18,6,.55,mat(0xbfc2c0),x,3,z+flip*7.8,true);box(.55,6,16,mat(0xbfc2c0),x-8.8,3,z,true);box(.55,6,16,mat(0xbfc2c0),x+8.8,3,z,true);for(const px of [-6,-2,2,6])box(.18,5.2,.18,mat(color),x+px,2.6,z+flip*7.45,false);makeMapLabel('HANGAR '+(flip<0?'AZUL':'VERMELHO'),x,4.8,z+flip*7.35,1.8);};
    hangar(0,31,blue,1);hangar(0,-31,red,-1);
    // Contêineres grandes: poucos, fortes e coloridos, formando três corredores.
    const container=(x,z,rot,color,high=false)=>{const g=new THREE.Group();const body=addOutline(new THREE.Mesh(new THREE.BoxGeometry(9,3.2,3.4),mat(color)));body.position.y=1.6;body.castShadow=true;body.receiveShadow=true;g.add(body);for(let i=-3;i<=3;i++){const rib=new THREE.Mesh(new THREE.BoxGeometry(.10,2.7,3.48),mat(0x30343a));rib.position.set(i*1.05,1.6,0);g.add(rib);}g.position.set(x,high?3.25:0,z);g.rotation.y=rot;scene.add(g);const swap=Math.abs(Math.sin(rot))>.5,w=swap?3.4:9,d=swap?9:3.4;colliders.push({minX:x-w/2,maxX:x+w/2,minZ:z-d/2,maxZ:z+d/2,height:(high?6.45:3.2)});return g;};
    ...
    container(-35,-14,0,0x6b86a4,true);container(35,14,0,0xa75c50,true);
    // Rampas largas para os dois mirantes laterais.
    const portPlatforms=[];window.__inkPortPlatforms=portPlatforms;const ramp=(x,z,dir,color)=>{for(let i=0;i<5;i++){const h=.55+i*.75,zz=z+dir*(i*2.45),step=box(4.4,h,3.0,mat(color),x,h/2,zz,false);step.userData.portSurface=true;portPlatforms.push({minX:x-2.2,maxX:x+2.2,minZ:zz-1.5,maxZ:zz+1.5,top:h});}};
    ramp(-39,-1,-1,0xb8c7d8);ramp(39,1,1,0xd9bbb2);
    // Guindaste central como marco visual; pernas deixam passagem por baixo.
    for(const x of [-15,15])box(.7,10,.7,mat(0xd2a84c),x,5,0,true);box(31,.65,.8,mat(0xd2a84c),0,9.6,0,false);portCraneTrolley=new THREE.Group();const trolley=addOutline(new THREE.Mesh(new THREE.BoxGeometry(2.4,.8,1.5),mat(0xd2a84c)));trolley.position.y=9.15;portCraneTrolley.add(trolley);const cable=new THREE.Mesh(new THREE.CylinderGeometry(.10,.10,5.6,6),mat(black));cable.position.y=6.0;portCraneTrolley.add(cable);const hook=addOutline(new THREE.Mesh(new THREE.TorusGeometry(.65,.18,6,10,Math.PI*1.35),mat(black)));hook.position.y=3.05;hook.rotation.z=.2;portCraneTrolley.add(hook);scene.add(portCraneTrolley);
