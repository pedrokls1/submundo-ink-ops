...
(() => {
  ...
  function buildGround() {
    ...
    const holes=[[-44.82,-34.55,-39.18,-18.25],[27.55,30.82,42.55,36.45]];
  ...
  function buildRabbitTunnel(){
    ...
    // V29: a toca deve ficar enterrada; somente as bocas ficam abertas.
    // Remove os paineis verticais da V28 (eles criavam a linha/parede atravessando o chao).
    // Fecha no proprio nivel do terreno a parte interna dos recortes que ja esta funda o bastante.
    // Assim a rampa continua acessivel pela boca, mas o corredor nao fica visivel pela lateral/de cima.
    const coverMat=mat(paper); coverMat.side=THREE.DoubleSide;
    const tunnelSurfaceCovers=[
      // esquerda: a boca permanece aberta de z=-34 ate aproximadamente -26.3
      [-42.0,-22.25,5.66,.12,8.10],
      // direita: a boca permanece aberta de x=34.4 ate x=42.5
      [30.98,33.635,6.86,.12,5.66]
    ];
    for(const [x,z,w,h,d] of tunnelSurfaceCovers){
      const c=box(w,h,d,coverMat,x,-.12,z,false);
      c.receiveShadow=true; c.userData.rabbitSurfaceCover=true;
      // cobertura fisica e balistica: nao deixa tiro atravessar o solo sobre a toca
      colliders.push({minX:x-w/2,maxX:x+w/2,minZ:z-d/2,maxZ:z+d/2,minY:-.25,height:.02});
    }
