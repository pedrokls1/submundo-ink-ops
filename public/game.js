...
(() => {
  ...
  // V56.32: mãos claramente visíveis segurando as armas em primeira pessoa.
  // Elas ficam ligeiramente para fora do corpo da arma para não serem escondidas pelo receiver.
  for(const [kind,v] of Object.entries(weaponVariants)){
    if(kind==='knife')continue;
    const handMat=mat(0xfffbef), sleeveMat=mat(0x315f9d);
    const rearSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.105,.28,4,7),sleeveMat),2.2);
    rearSleeve.rotation.z=-.30; rearSleeve.position.set(.64,-.76,kind==='sniper'?-1.00:-.78); v.add(rearSleeve);
    const rearHand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.19,8,6),handMat),2.4);
    rearHand.scale.set(1.0,.78,1.20); rearHand.position.set(.55,-.63,kind==='sniper'?-1.08:-.82); v.add(rearHand);
    const frontSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.10,.30,4,7),sleeveMat),2.2);
    frontSleeve.rotation.z=.42; frontSleeve.position.set(.67,-.58,kind==='launcher'?-1.54:(kind==='sniper'?-1.95:-1.50)); v.add(frontSleeve);
    const frontHand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.18,8,6),handMat),2.4);
    frontHand.scale.set(1.0,.76,1.20); frontHand.position.set(.55,-.43,kind==='launcher'?-1.62:(kind==='sniper'?-2.03:-1.58)); v.add(frontHand);
  }
  ...
  function makeRemotePlayer(state){
    ...
    const vest=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.58,.43,.30),darkMat),3.0);
    vest.position.set(0,1.22,.015); group.add(vest);
    // V56.32: colete mais rente ao tórax; deixa os braços passarem pela frente sem atravessar um bloco grosso.
    const vestPlate=new THREE.Mesh(new THREE.BoxGeometry(.46,.27,.025),mat(0x343941)); vestPlate.position.set(0,1.23,-.145); group.add(vestPlate);
    const chestMark=new THREE.Mesh(new THREE.BoxGeometry(.40,.12,.025),accentMat);
    chestMark.position.set(0,1.30,-.33); group.add(chestMark);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.15,.18,.16,8),darkMat);
    ...
    const shoulderL=new THREE.Vector3(-.325,1.43,-.015), shoulderR=new THREE.Vector3(.325,1.43,-.015);
    const elbowL=new THREE.Vector3(-.29,1.245,-.39), elbowR=new THREE.Vector3(.30,1.20,-.31);
    const handLPos=new THREE.Vector3(.145,1.235,-.94), handRPos=new THREE.Vector3(.155,1.145,-.61);
    for(const pos of [shoulderL,shoulderR]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.105,8,6),accentMat),2.4);
      shoulder.scale.set(1,.92,.88); shoulder.position.copy(pos); group.add(shoulder);
    }
    ...
    addHatching(gun,.12,.105,.48,7,true);
