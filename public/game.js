...
(() => {
  ...
  function addInkPotMagazine(v,kind){
    ...
    const pos={rifle:[.48,-.58,-.82],sniper:[.49,-.58,-1.02],smg:[.49,-.59,-1.04],shotgun:[.49,-.57,-1.02]}[kind]||[.40,-.70,-.92];
  ...
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const grips={rifle:{rear:[.58,-.50,-.48],front:[.20,-.40,-1.24]},sniper:{rear:[.59,-.51,-.68],front:[.20,-.40,-1.45]},smg:{rear:[.58,-.50,-.54],front:[.21,-.40,-1.08]},shotgun:{rear:[.59,-.50,-.54],front:[.21,-.40,-1.52]},launcher:{rear:[.58,-.50,-.78],front:[.21,-.40,-1.48]}}[kind];
    const makeArmHand=(pt,front=false)=>{
      ...
      const sleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.115,.48,4,7),sleeveMat),2.3);
      sleeve.rotation.z=front?.62:-.58; sleeve.rotation.x=front?.12:-.08; sleeve.position.set(front?-.22:.23,-.27,.18); hg.add(sleeve);
  ...
  function reloadWeapon(){
    ...
    if(!reloadCartridge){
      ...
      const reloadSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.12,.52,4,7),mat(0x315f9d)),2.2); reloadSleeve.rotation.z=-.45; reloadSleeve.position.set(-.48,-.31,.10); reloadCartridge.add(reloadSleeve);
      reloadCartridge.scale.setScalar(.70); reloadCartridge.position.set(-1.15,-.74,-.58); reloadCartridge.rotation.set(-.15,.12,-.22); camera.add(reloadCartridge);
    }
  ...
  function makeRemotePlayer(state){
    ...
    headOutline.position.y=2.08; group.add(headOutline);
    const browMat=darkMat;
    for(const side of [-1,1]){
      const eye=new THREE.Mesh(new THREE.BoxGeometry(.17,.075,.045),browMat);
      eye.position.set(side*.15,2.10,-.405); eye.rotation.z=side*.20; group.add(eye);
    }
    ...
    mouth.position.set(0,1.935,-.425); mouth.rotation.z=.10; group.add(mouth);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.13,.175,.44,8),paperMat);
    neck.position.set(0,1.78,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.138,.183,.442,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    ...
    for(const o of group.children){if(o!==head&&o.isMesh&&o.position.y>=1.78)o.userData.hitZone='head';}
    neck.userData.hitZone='head'; neckOutline.userData.hitZone='head'; trapezius.userData.hitZone='body';
