...
(() => {
  ...
  function addInkPotMagazine(v,kind){
    ...
    const body=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.24,.34,.24),mat(0xf2eee3)),2.2); pot.add(body);
    const rim=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.27,.055,.27),mat(0x252a31)),2.0); rim.position.y=.18; pot.add(rim);
    for(const x of [-.065,0,.06]){const d=new THREE.Mesh(new THREE.CapsuleGeometry(.018,.10+(x===0?.05:0),3,5),mat(0xb51f2e));d.position.set(x,-.075,-.128);pot.add(d);}
    const pos={rifle:[.43,-.68,-1.16],sniper:[.40,-.55,-1.34],smg:[.42,-.69,-1.20],shotgun:[.42,-.58,-1.15]}[kind]||[.42,-.66,-1.15];
    pot.position.set(...pos); pot.rotation.x=.10; v.add(pot);
  }
  ...
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const makeGripHand=(x,y,z,front=false)=>{
      ...
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.22,.17,.27),handMat),2.3); palm.rotation.z=front?.12:-.12; hg.add(palm);
      const thumb=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.045,.13,3,6),handMat),2.0); thumb.rotation.z=front?-1.0:.95; thumb.position.set(front?-.11:.11,-.01,-.05); hg.add(thumb);
      hg.position.set(x,y,z); v.add(hg); return hg;
    };
    makeGripHand(.60,-.57,rearZ,false);
    makeGripHand(.59,-.42,frontZ,true);
  }
  // V56.28: armas grafite/preto hachurado, mantendo pequenos detalhes vermelhos.
  for(const v of Object.values(weaponVariants))v.traverse(o=>{if(o.isMesh&&!o.userData.sniperLens&&o.material?.color){const original=o.material.color.getHex();if(original===red||original===0xff2038||original===0xd73535)o.material.color.setHex(0xb51f2e);else if(original!==0xfffbef)o.material.color.setHex(0x17191d);if(o.material.emissive)o.material.emissive.setHex(0x000000);}});
  ...
  function reloadWeapon(){
    ...
    if(!reloadCartridge){
      ...
      for(const x of [-.10,-.025,.075]){const drip=new THREE.Mesh(new THREE.CapsuleGeometry(.025,.13+(x<0?.06:0),3,6),mat(0xb51f2e));drip.position.set(x,-.04,-.165);reloadCartridge.add(drip);}
      const stain=new THREE.Mesh(new THREE.SphereGeometry(.055,7,5),mat(0xb51f2e)); stain.scale.set(1.35,.65,.35); stain.position.set(.075,.09,-.17); reloadCartridge.add(stain);
      const handGroup=new THREE.Group();
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.25,.19,.28),mat(0xfffbef)),2.4); handGroup.add(palm);
  ...
  function makeRemotePlayer(state){
    ...
    body.scale.set(1.16,1.0,.72); body.position.set(0,1.18,0); group.add(body);
    ...
    const vest=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.62,.38,.075),mat(0x252a31)),2.4);
    vest.position.set(0,1.23,-.285); group.add(vest);
    const vestBack=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.62,.38,.075),mat(0x252a31)),2.4);
    vestBack.position.set(0,1.23,.285); group.add(vestBack);
    const vestPlate=new THREE.Mesh(new THREE.BoxGeometry(.48,.24,.018),mat(0x3e4652)); vestPlate.position.set(0,1.23,-.328); group.add(vestPlate);
    const vestBackPlate=new THREE.Mesh(new THREE.BoxGeometry(.48,.24,.018),mat(0x3e4652)); vestBackPlate.position.set(0,1.23,.328); group.add(vestBackPlate);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.12,.16,.20,8),paperMat);
    neck.position.set(0,1.66,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.128,.168,.202,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    neckOutline.position.copy(neck.position); group.add(neckOutline);
    const trapezius=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.76,.18,.36),accentMat),2.5);
    trapezius.position.set(0,1.50,0); trapezius.scale.set(1,.78,.74); group.add(trapezius);
    ...
    const shoulderL=new THREE.Vector3(-.43,1.46,-.005), shoulderR=new THREE.Vector3(.43,1.46,-.005);
    ...
    for(const [pos,scale] of [[handLPos,[.88,.68,1.00]],[handRPos,[.88,.68,.98]]]){
      const hand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.10,8,6),paperMat),2.2);
      hand.scale.set(...scale); hand.position.copy(pos); group.add(hand);
    }
  ...
  function update(dt){
    ...
    if(reloadTimer>0&&weaponMode!=='knife'){const rt=Math.max(.01,reloadCartridge?.userData.reloadTotal||reloadTimer),rp=1-Math.min(1,reloadTimer/rt),lift=Math.sin(Math.min(1,rp)*Math.PI);activeWeapon.position.y+=.16*lift;activeWeapon.position.z+=.08*lift;activeWeapon.rotation.x-=.22*lift;activeWeapon.rotation.z+=.07*lift;}
