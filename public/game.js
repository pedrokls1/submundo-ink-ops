...
(() => {
  ...
  let bedroomObjects=[], kingdomObjects=[], bedroomColliders=[], outletCooldown=0, phonePhase=0, bedroomLightOff=0, bedroomLightLock=0, bedroomMatchElapsed=0, bedroomPhoneRingMask=0, duckBounceCooldown=0;
  const bedroomOutlets=[[-48.70,-24],[-48.70,18],[48.70,-20],[48.70,24],[-24,-39.70],[20,-39.70],[-22,39.70],[24,39.70]];
  ...
  function buildGiantBedroom(){
    ...
    const makeCornerTable=(z,color)=>{box(18,1.0,10,mat(color),39,7.2,z,false);for(const x of [32,46])for(const zz of [z-3.8,z+3.8])box(1,7,1,mat(0x8f6f55),x,3.5,zz,true);};
    ...
    bedroomOutlets.forEach(([x,z],i)=>{const side=Math.abs(x)>47;if(side){const wallX=x<0?-48.80:48.80,face=x<0?-48.58:48.58,triggerX=x<0?-47.0:47.0;const plate=box(.18,2.5,3.1,mat(0xf8f3e7),wallX,1.45,z,false);plate.userData.outlet=i;const dark=box(.20,.55,.78,mat(black),face,1.45,z,false);dark.userData.outlet=i;makeBolt(face,3.35,z,'x');bedroomOutlets[i][0]=triggerX;}else{const wallZ=z<0?-39.80:39.80,face=z<0?-39.58:39.58,triggerZ=z<0?-38.0:38.0;const plate=box(3.1,2.5,.18,mat(0xf8f3e7),x,1.45,wallZ,false);plate.userData.outlet=i;const dark=box(.78,.55,.20,mat(black),x,1.45,face,false);dark.userData.outlet=i;makeBolt(x,3.35,face,'z');bedroomOutlets[i][1]=triggerZ;}});
    ...
    box(8,10,13,mat(0xd7a6a0),38.5,5.0,2,true);
    const dollRoof=new THREE.Mesh(new THREE.ConeGeometry(6.2,4.0,4),mat(0x8fa8bd));dollRoof.position.set(38.5,12.0,2);dollRoof.rotation.y=Math.PI/4;addOutline(dollRoof);scene.add(dollRoof);
    box(2.2,4.5,.18,mat(0x5f4a42),38.5,2.25,-4.58,false);box(1.4,1.6,.20,mat(0xcfe0e4),35.8,6.0,-4.60,false);box(1.4,1.6,.20,mat(0xcfe0e4),41.2,6.0,-4.60,false);
    makeMapLabel('CASINHA',38.5,14.6,2,.72);
  ...
  function update(dt){
    ...
    if(activeMap==='quarto'){
      ...
      if(outletCooldown<=0){const oi=bedroomOutlets.findIndex(([x,z])=>Math.hypot(player.pos.x-x,player.pos.z-z)<1.55);if(oi>=0){const choices=bedroomOutlets.map((_,i)=>i).filter(i=>i!==oi),to=choices[Math.floor(Math.random()*choices.length)],p=bedroomOutlets[to];spawnOutletZap(player.pos.clone());electricZapSound();outletCooldown=5;let tx=p[0],tz=p[1];if(Math.abs(tx)>=46.5)tx+=tx<0?1.0:-1.0;else if(Math.abs(tz)>=37.5)tz+=tz<0?1.0:-1.0;player.pos.set(tx,1.7,tz);verticalVelocity=0;grounded=true;setTimeout(()=>{if(activeMap==='quarto')spawnOutletZap(player.pos.clone());},35);hitNoticeEl.textContent='⚡ TRZZZT!  PUFT!';hitNoticeEl.classList.add('show');hitNoticeTimer=1.0;}}
