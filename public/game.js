...
(() => {
  ...
  const bedroomOutlets=[[-43,-24],[43,-20],[-43,23],[43,25]];
  ...
  // V56 — QUARTO GIGANTE, primeira versão jogável. Geometria simples para manter o Ink legível e leve.
  function buildGiantBedroom(){
    ...
    // paredes desenhadas, sem excesso de detalhe
    box(98,8,.35,mat(0xf2eadb),0,4,-40,true);box(98,8,.35,mat(0xf2eadb),0,4,40,true);box(.35,8,80,mat(0xf2eadb),-49,4,0,true);box(.35,8,80,mat(0xf2eadb),49,4,0,true);
    // cama gigante: espaço livre embaixo + colchão acessível por mola/livros
    for(const x of [-18,18])for(const z of [-5,17])box(1.1,5.2,1.1,mat(0x8f6f55),x,2.6,z,true);
    const bed=box(38,1.0,24,softBlue,0,5.15,6,false); bed.userData.bedroomPlatform=true;
    box(38,1.2,1.0,mat(0x8f6f55),0,5.2,18,true);
    // travesseiros coloridos
    box(9,.8,5,mat(0xf4eee2),-10,6.0,13,false);box(9,.8,5,mat(0xf4eee2),10,6.0,13,false);
    // escrivaninha e criado-mudo
    box(22,1.0,10,mat(0xb99a72),31,7.2,-22,false);for(const x of [22,40])for(const z of [-26,-18])box(1,7,1,mat(0x8f6f55),x,3.5,z,true);
    box(11,1,10,mat(0xb99a72),-36,5.2,-22,false);for(const x of [-40,-32])for(const z of [-26,-18])box(.9,5,.9,mat(0x8f6f55),x,2.5,z,true);
    // caixa de brinquedos como cobertura grande
    box(14,4,10,softRed,-32,2,20,true);makeMapLabel('BRINQUEDOS',-32,4.5,20,1.1);
    // livros/caixas em degraus para chegar na cama sem depender da mola
    const steps=[[-25,-2,1.1],[-22,0,2.1],[-20,2,3.1],[-18,4,4.15]];
    for(let i=0;i<steps.length;i++){const [x,z,top]=steps[i];box(6,top,5,mat(i%2?0xd8b46c:0x9eb0c8),x,top/2,z,true);}
    // lápis gigante: ponte elevada simples entre cama e mesa lateral
    const pencil=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.48,.48,24,8),mat(0xe5c74f)));pencil.rotation.z=Math.PI/2;pencil.position.set(24,6.15,5);scene.add(pencil);
    // quatro tomadas aleatórias
    bedroomOutlets.forEach(([x,z],i)=>{const onSide=Math.abs(x)>40;const plate=box(onSide?.22:3.1,2.5,onSide?3.1:.22,mat(0xf8f3e7),x,1.45,z,false);plate.userData.outlet=i;const dark=box(onSide?.24:.32,.48,onSide?.75:.24,mat(black),x+(x<0?.13:x>0?-.13:0),1.55,z,false);dark.userData.outlet=i;makeMapLabel('⚡',x,3.0,z,.7);});
    // Slinky/mola — visual + zona de lançamento para a cama
    const spring=new THREE.Group();for(let i=0;i<14;i++){const ring=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.45,.10,6,20),mat(i%2?red:toyYellow)));ring.rotation.x=Math.PI/2;ring.position.y=.22+i*.16;spring.add(ring);}spring.position.set(0,0,-8);scene.add(spring);makeMapLabel('BOING!',0,2.8,-8,.85);
    // telefone gigante: plataforma/objeto visual que vibra levemente
    ...
    // interruptor — nesta versão já reage a tiro no cliente
  ...
  function update(dt){
    ...
    if(activeMap==='quarto'){
      // Slinky: lança para a cama.
      if(grounded&&jumpPadCooldown<=0&&Math.hypot(player.pos.x,player.pos.z+8)<1.8){verticalVelocity=13.8;grounded=false;jumpPadCooldown=1.4;hitNoticeEl.textContent='BOOOING!';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;}
      // Tomadas: choque rápido e saída aleatória em outra das quatro.
      if(outletCooldown<=0){const oi=bedroomOutlets.findIndex(([x,z])=>Math.hypot(player.pos.x-x,player.pos.z-z)<1.25);if(oi>=0){const choices=[0,1,2,3].filter(i=>i!==oi),to=choices[Math.floor(Math.random()*choices.length)],p=bedroomOutlets[to];outletCooldown=5;player.pos.set(p[0]+(p[0]<0?1.7:-1.7),1.7,p[1]);verticalVelocity=0;grounded=true;hitNoticeEl.textContent='⚡ TRZZZT!  PUFT!';hitNoticeEl.classList.add('show');hitNoticeTimer=1.0;}}
