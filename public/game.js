...
(() => {
  ...
  window.__inkRealtimeSnapshot = payload => {
    ...
    if(payload.self){const wasAlive=multiplayer.selfAlive;multiplayer.selfAlive=payload.self.alive;if(!payload.self.alive){hp=0;createLocalDeathPuddle();showDeathScreen(payload.self.respawnSeconds);fireHeld=false;scopeMode=false;}else if(!wasAlive){hp=100;armor=100;reserveAmmo={...RESERVE_AMMO};ammo=weaponMode==='knife'?0:(MAGAZINES[weaponMode]||0);clearLocalDeathPuddle();hideDeathScreen();const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;player.pos.set(0,1.7,myTeam==='vermelho'?-36:36);player.yaw=myTeam==='vermelho'?0:Math.PI;updateHud();}else{hp=Math.min(100,payload.self.health);hideDeathScreen();updateHud();}}
    if(payload.scores){
