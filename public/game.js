...
(() => {
  ...
  function getPlayerHeight(x,z){if(window.__inkClimbPlatforms){let best=1.7;for(const p of window.__inkClimbPlatforms){if(x>p.minX&&x<p.maxX&&z>p.minZ&&z<p.maxZ)best=Math.max(best,1.7+p.top);}if(best>1.7)return best;}
  ...
  function spawnMushroomSmoke(m){for(let i=0;i<72;i++){const puff=new THREE.Mesh(new THREE.SphereGeometry(.68+Math.random()*.92,7,6),new THREE.MeshBasicMaterial({color:i%5===0?0x090d0b:(i%3===0?0x101713:0x18221d),transparent:true,opacity:.94,depthWrite:false}));const ang=Math.random()*TAU,r=Math.random()*4.25;puff.position.set(m.x+Math.cos(ang)*r,.55+Math.random()*4.5,m.z+Math.sin(ang)*r);puff.userData.noBulletMark=true;puff.raycast=()=>{};scene.add(puff);mushroomSmoke.push({mesh:puff,vx:(Math.random()-.5)*.28,vy:.04+Math.random()*.16,vz:(Math.random()-.5)*.28,life:7+Math.random()*1.6,max:8.6});}}
  ...
  function buildRabbitTunnel(){const pts=[[-36,-27],[-36,-10],[-32,8],[-18,24],[4,30],[24,30],[36,27]];window.__inkRabbitPath=pts;window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0},total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz);if(d<best.d)best={d,t:(acc+lens[i]*u)/total};acc+=lens[i];}return best;};window.__inkRabbitTunnelY=()=>-2.55;function portal(x,z,rot,targetX,targetZ,exitX,exitZ){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const ring=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.9,.38,8,24,Math.PI),mat(wall)));ring.rotation.z=Math.PI;ring.position.y=1.65;gg.add(ring);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.62,20),new THREE.MeshBasicMaterial({color:black,side:THREE.DoubleSide}));dark.position.set(0,1.48,.08);gg.add(dark);scene.add(gg);makeWorldSign('TOCA DO COELHO',x+(x<0?4:-4),z+(z<0?-2.8:2.8),rot);return {x,z,targetX,targetZ,targetY:-2.55,exitX,exitZ};}window.__inkRabbitEntrances=[portal(-40,-31,0,-36,-27,-40,-31),portal(40,31,Math.PI,36,27,40,31)];for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=Math.hypot(bx-ax,bz-az),yaw=Math.atan2(bx-ax,bz-az),mx=(ax+bx)/2,mz=(az+bz)/2,y=-4.25;const floor=box(5.4,.28,len+1.1,mat(wallLight),mx,y,mz,false);floor.rotation.y=yaw;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.62,wz=mz-Math.sin(yaw)*side*2.62;const wallSeg=box(.36,3.55,len+1.25,mat(wall),wx,y+1.78,wz,false);wallSeg.rotation.y=yaw;}const roof=box(5.75,.34,len+1.35,mat(0xded8ca),mx,y+3.55,mz,false);roof.rotation.y=yaw;roof.userData.rabbitRoof=true;}for(const [x,z] of pts){const cap=box(5.8,.34,5.8,mat(0xded8ca),x,-.70,z,false);cap.userData.rabbitRoof=true;}}
  ...
  function shoot(){
    ...
    fireCooldown=weaponMode==='sniper'?.92:(weaponMode==='launcher'?1.15:(weaponMode==='shotgun'?.72:(weaponMode==='smg'?.075:.12)));
    ammo--;
  ...
  function update(dt){
    ...
    if(!rabbitTunnelMode&&rabbitTunnelCooldown<=0&&window.__inkRabbitEntrances?.length===2){for(const ent of window.__inkRabbitEntrances){if(Math.hypot(player.pos.x-ent.x,player.pos.z-ent.z)<1.9){rabbitTunnelMode=true;rabbitTunnelCooldown=1.0;grounded=true;verticalVelocity=0;player.pos.set(ent.targetX,ent.targetY,ent.targetZ);hitNoticeEl.textContent='TOCA DO COELHO • TELEPORTE SUBTERRÂNEO';hitNoticeEl.classList.add('show');hitNoticeTimer=.9;break;}}}
