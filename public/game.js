...
(() => {
  ...
  // V56.33: mãos de primeira pessoa trazidas para a lateral/parte inferior da arma,
  // visíveis de verdade e encaixadas no punho + guarda-mão em vez de apenas próximas ao receiver.
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const rearSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.115,.34,4,7),sleeveMat),2.2);
    rearSleeve.rotation.z=-.42; rearSleeve.position.set(.73,-.73,rearZ+.14); v.add(rearSleeve);
    const rearHand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.205,8,6),handMat),2.4);
    rearHand.scale.set(.92,.72,1.28); rearHand.position.set(.60,-.57,rearZ); rearHand.rotation.z=-.20; v.add(rearHand);
    const frontSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.11,.36,4,7),sleeveMat),2.2);
    frontSleeve.rotation.z=.55; frontSleeve.position.set(.73,-.61,frontZ+.18); v.add(frontSleeve);
    const frontHand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.195,8,6),handMat),2.4);
    frontHand.scale.set(.92,.70,1.30); frontHand.position.set(.59,-.42,frontZ); frontHand.rotation.z=.18; v.add(frontHand);
  }
  ...
  function makeRemotePlayer(state){
    ...
    body.scale.set(1.05,1.0,.70); body.position.set(0,1.18,0); group.add(body);
    ...
    const vest=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.43,.34,.105),mat(0x252a31)),2.4);
    vest.position.set(0,1.23,-.245); group.add(vest);
    const vestPlate=new THREE.Mesh(new THREE.BoxGeometry(.35,.22,.018),mat(0x3e4652)); vestPlate.position.set(0,1.23,-.305); group.add(vestPlate);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.115,.15,.13,8),paperMat);
    neck.position.set(0,1.62,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.123,.158,.132,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    neckOutline.position.copy(neck.position); group.add(neckOutline);
    const trapezius=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.60,.16,.34),accentMat),2.5);
    trapezius.position.set(0,1.50,0); trapezius.scale.set(1,.72,.72); group.add(trapezius);
    ...
    const shoulderL=new THREE.Vector3(-.34,1.45,-.015), shoulderR=new THREE.Vector3(.34,1.45,-.015);
    const elbowL=new THREE.Vector3(-.26,1.29,-.34), elbowR=new THREE.Vector3(.31,1.24,-.27);
    ...
    const mag=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.12,.28,.18),darkMat),2.2);
    mag.position.set(.16,1.06,-.68); mag.rotation.x=.10; gunGroup.add(mag);
