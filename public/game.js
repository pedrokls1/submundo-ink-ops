...
(() => {
  ...
  function buildGround() {
    ...
    const diagGroundMat=new THREE.MeshBasicMaterial({color:0xff00aa,side:THREE.DoubleSide,transparent:true,opacity:.72,depthWrite:true});const ground=new THREE.Mesh(g,diagGroundMat);ground.userData.diagName='GROUND_PRINCIPAL_MAGENTA';ground.rotation.x=-Math.PI/2;ground.position.y=-.06;ground.receiveShadow=true;scene.add(ground);
  ...
  function addTunnelDiagnosticLegend(){const el=document.createElement('div');el.id='tunnelDiagLegend';el.style.cssText='position:fixed;left:10px;top:92px;z-index:99;background:rgba(0,0,0,.78);color:white;padding:8px 10px;border:2px solid white;font:700 12px Arial;line-height:1.45;pointer-events:none';el.innerHTML='<b>DIAGNÓSTICO TOCA V22</b><br><span style="color:#ff00aa">■</span> MAGENTA = chão principal<br><span style="color:#00e5ff">■</span> CIANO = piso da toca<br><span style="color:#39ff14">■</span> VERDE = paredes<br><span style="color:#ffea00">■</span> AMARELO = teto<br><span style="color:#ff6d00">■</span> LARANJA = canto';document.body.appendChild(el);}addTunnelDiagnosticLegend();
  function buildRabbitTunnel(){
    ...
    let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],pieces=Math.max(1,Math.ceil(len/2.6));for(let j=0;j<pieces;j++){const u0=j/pieces,u1=(j+1)/pieces,um=(u0+u1)/2;const x0=ax+(bx-ax)*u0,z0=az+(bz-az)*u0,x1=ax+(bx-ax)*u1,z1=az+(bz-az)*u1,mx=(x0+x1)/2,mz=(z0+z1)/2,segLen=Math.hypot(x1-x0,z1-z0),t0=(acc+len*u0)/total,t1=(acc+len*u1)/total,tm=(t0+t1)/2,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(x1-x0,z1-z0),pitch=Math.atan2(y1-y0,segLen);
      const floorMat=new THREE.MeshBasicMaterial({color:0x00e5ff,side:THREE.DoubleSide,transparent:true,opacity:.88});const floor=box(5.55,.10,segLen+.12,floorMat,mx,ym,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;floor.userData.diagName='PISO_TOCA_CIANO';
      for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.76,wz=mz-Math.sin(yaw)*side*2.76;const wallMat=new THREE.MeshBasicMaterial({color:0x39ff14,side:THREE.DoubleSide,transparent:true,opacity:.82});const extra=Math.abs(y1-y0);const wallSeg=box(.30,3.35+extra,segLen+.28,wallMat,wx,ym+1.58,wz,false);wallSeg.rotation.y=yaw;wallSeg.userData.rabbitWall=true;wallSeg.userData.diagName='PAREDE_TOCA_VERDE';const ex=Math.abs(Math.cos(yaw))*.15+Math.abs(Math.sin(yaw))*(segLen+.28)/2,ez=Math.abs(Math.sin(yaw))*.15+Math.abs(Math.cos(yaw))*(segLen+.28)/2;colliders.push({minX:wx-ex,maxX:wx+ex,minZ:wz-ez,maxZ:wz+ez,minY:ym-.12,height:ym+3.35+extra});}
      if(window.__inkRabbitTunnelY(tm)<-1.75&&tm>.16&&tm<.84){const roofMat=new THREE.MeshBasicMaterial({color:0xffea00,side:THREE.DoubleSide,transparent:true,opacity:.80});const roof=box(5.90,.24,segLen+.25,roofMat,mx,ym+3.18,mz,false);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;roof.userData.diagName='TETO_TOCA_AMARELO';}
    }acc+=len;}
    ...
    for(let i=1;i<pts.length-1;i++){
      ...
      const cy=(ri.y-1.7);
      const cornerMat=new THREE.MeshBasicMaterial({color:0xff6d00,side:THREE.DoubleSide,transparent:true,opacity:.88});
      const corner=box(.72,3.55,.72,cornerMat,cx,cy+1.62,cz,false);
      corner.userData.rabbitWall=true;corner.userData.diagName='CANTO_TOCA_LARANJA';
