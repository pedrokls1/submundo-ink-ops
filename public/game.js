...
(() => {
  ...
  function getPlayerHeight(x,z){let best=1.7;for(const m of destructibleMushrooms.values()){if(!m.active)continue;const d=Math.hypot(x-m.x,z-m.z);const top=m.top||3.72*(m.s||1);if(d<(m.standRadius||1.72*(m.s||1))&&player.pos.y>=1.7+top-.72)best=Math.max(best,1.7+top);}if(window.__inkTeaRamp){const r=window.__inkTeaRamp;if(x>=r.minX&&x<=r.maxX&&z>=r.minZ&&z<=r.maxZ){const u=Math.max(0,Math.min(1,(x-r.startX)/(r.endX-r.startX)));best=Math.max(best,1.7+r.startTop+(r.endTop-r.startTop)*u);}}if(window.__inkClimbPlatforms){for(const p of window.__inkClimbPlatforms){const inside=p.radius!=null?Math.hypot(x-p.cx,z-p.cz)<p.radius:(x>p.minX&&x<p.maxX&&z>p.minZ&&z<p.maxZ);if(inside&&(p.minCurrentY==null||player.pos.y>=p.minCurrentY))best=Math.max(best,1.7+p.top);}}if(best>1.7)return best;if(window.__inkRabbitTunnelInfo&&window.__inkRabbitTunnelY){const ri=window.__inkRabbitTunnelInfo(x,z);const edge=Math.min(ri.t,1-ri.t);if(ri.d<2.22&&(edge<.105||player.pos.y<.75))return ri.y??window.__inkRabbitTunnelY(ri.t);}if(Math.abs(x)<2.6||Math.abs(x-30)<2.6||Math.abs(x+30)<2.6){const az=Math.abs(z);if(az<=3.5)return 2.20;if(az<5.9)return 1.70+(5.9-az)/2.4*.50;}return 1.7;}
  ...
  function setControlsEditing(on){
    ...
    for(const el of movableControls){el.style.filter=controlsEditing?'drop-shadow(0 0 5px #d73535)':'';if(el===hudVitalsEl||el===hudStatsEl||el.id==='onlineScore')el.style.pointerEvents=controlsEditing?'auto':'none';}
    applyControlMode();
  ...
  function createMultiplayerLobby(){
    ...
    wrap.innerHTML=`<div style="width:min(94vw,620px);max-height:92vh;overflow:auto;padding:22px;border:4px solid #171510;border-radius:14px;background:#fcfbf7;box-shadow:9px 9px 0 #294aa3;position:relative">
      ...
      <div data-room style="display:none;margin-top:14px">
        ...
        <button data-bots style="display:none;width:100%;margin-top:11px;padding:11px;border:3px solid #171510;border-radius:10px;background:#d73535;color:#fff;font:900 15px Georgia">BOTS: DESLIGADOS</button>
    ...
    q('[data-ready]').addEventListener('click',()=>{const me=multiplayer.room?.players?.find(p=>p.token===multiplayer.token);act('ready',{ready:!me?.ready});});
  ...
  // V45: netcode visual para jogadores remotos.
  // Guardamos snapshots e renderizamos um pouco no passado. Isso permite interpolar
  // entre dois estados reais em vez de perseguir o ultimo pacote que chegou.
  let remoteNetPing=130, remoteNetJitter=12, lastRealtimeSnapshotAt=0;
  window.__inkRealtimeNetStats=stats=>{if(!stats)return;const p=Number(stats.ping),j=Number(stats.jitter);if(Number.isFinite(p))remoteNetPing=p;if(Number.isFinite(j))remoteNetJitter=j;};
  function remoteInterpolationDelay(){
    // 2 updates (100 ms) + margem baseada no jitter. Limites evitam sensacao excessivamente atrasada.
    return Math.max(100,Math.min(165,100+remoteNetJitter*1.6));
  }
  function lerpAngle(a,b,t){let d=((b-a+Math.PI)%(Math.PI*2)+Math.PI)%(Math.PI*2)-Math.PI;return a+d*t;}

  function makeRemotePlayer(state){const group=new THREE.Group(),teamColor=state.team==='vermelho'?0xff2638:0x00a8ff,darkMat=mat(0x080b10),paperMat=mat(0xfff4c7),accentMat=mat(teamColor);const legs=[-.22,.22].map(x=>{const leg=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.23,.82,.24),darkMat),2.4);leg.position.set(x,.42,0);group.add(leg);return leg;});const body=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.88,1.06,.50),darkMat),3.4);body.position.y=1.18;group.add(body);const chest=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.68,.66,.54),accentMat),3.2);chest.position.set(0,1.24,-.03);group.add(chest);const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.32,8,6),paperMat),2.6);head.position.y=1.96;group.add(head);const visor=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.46,.13,.09),mat(0x070a0f)),2.4);visor.position.set(0,1.99,-.3);group.add(visor);for(const side of [-1,1]){const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.14,6,5),accentMat),2.3);shoulder.position.set(side*.48,1.48,-.02);group.add(shoulder);}const arms=[];for(const side of [-1,1]){const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.1,.5,3,5),darkMat),2.4);arm.position.set(side*.47,1.27,-.08);arm.rotation.z=side*.16;arm.rotation.x=-.72;group.add(arm);arms.push(arm);}const gunGroup=new THREE.Group();const gun=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.2,.18,1.05),darkMat),2.3);gun.position.set(.18,1.3,-.64);gunGroup.add(gun);const mag=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.14,.34,.22),accentMat),2.2);mag.position.set(.18,1.08,-.72);gunGroup.add(mag);group.add(gunGroup);const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;if(myTeam&&myTeam===state.team)group.add(makePlayerLabel(state.nickname,state.team==='vermelho'?'#d73535':'#356bc1'));group.traverse(o=>{if(o.isMesh){o.userData.remoteToken=state.token;o.userData.hitZone=o===head?'head':'body';}});group.position.set(state.x,state.y-1.7,state.z);group.rotation.y=state.yaw+Math.PI;scene.add(group);const initial=new THREE.Vector3(state.x,state.y-1.7,state.z),now=performance.now();return {group,legs,arms,gun:gunGroup,target:initial.clone(),predicted:initial.clone(),lastNetPos:initial.clone(),velocity:new THREE.Vector3(),lastPacketAt:now,targetYaw:state.yaw+Math.PI,step:0,footTimer:.12+Math.random()*.18,weapon:state.weapon,team:state.team,puddle:null,alive:state.alive!==false,snapshots:[{t:now,pos:initial.clone(),yaw:state.yaw+Math.PI,alive:state.alive!==false,weapon:state.weapon}]};}
  function updateRemotePlayers(dt){
    const renderTime=performance.now()-remoteInterpolationDelay();
    for(const remote of multiplayer.remotePlayers.values()){
      const snaps=remote.snapshots||[];let desired=null,desiredYaw=remote.group.rotation.y;
      while(snaps.length>2&&snaps[1].t<renderTime)snaps.shift();
      if(snaps.length>=2&&snaps[0].t<=renderTime&&renderTime<=snaps[1].t){
        const a=snaps[0],b=snaps[1],span=Math.max(1,b.t-a.t),t=Math.max(0,Math.min(1,(renderTime-a.t)/span));
        desired=a.pos.clone().lerp(b.pos,t);desiredYaw=lerpAngle(a.yaw,b.yaw,t);
      }else if(snaps.length){
        const last=snaps[snaps.length-1];desired=last.pos.clone();desiredYaw=last.yaw;
        // Se faltar pacote, extrapola somente por um curto periodo e com velocidade limitada.
        if(renderTime>last.t&&snaps.length>=2){const prev=snaps[snaps.length-2],span=Math.max(.02,(last.t-prev.t)/1000),extra=Math.min(.075,(renderTime-last.t)/1000);const vx=(last.pos.x-prev.pos.x)/span,vz=(last.pos.z-prev.pos.z)/span,speed=Math.hypot(vx,vz),scale=speed>8?8/speed:1;desired.x+=vx*scale*extra;desired.z+=vz*scale*extra;}
      }
      if(desired){
        const error=remote.group.position.distanceTo(desired);
        if(error>3.2)remote.group.position.copy(desired);else remote.group.position.lerp(desired,Math.min(1,dt*(error>.65?18:12)));
        let turn=((desiredYaw-remote.group.rotation.y+Math.PI)%(Math.PI*2)+Math.PI)%(Math.PI*2)-Math.PI;remote.group.rotation.y+=turn*Math.min(1,dt*16);
      }
      const moving=remote.alive&&desired&&Math.hypot(desired.x-remote.group.position.x,desired.z-remote.group.position.z)>.018;
  ...
  function applyRemoteStates(states){
    ...
    for(const state of states||[]){
      ...
      const nextPos=new THREE.Vector3(state.x,state.y-1.7,state.z),nextYaw=state.yaw+Math.PI;
      remote.lastNetPos.copy(nextPos);remote.lastPacketAt=now;remote.target.copy(nextPos);remote.targetYaw=nextYaw;remote.weapon=state.weapon;
      remote.snapshots=remote.snapshots||[];
      const prev=remote.snapshots[remote.snapshots.length-1];
      // Ignora duplicata quase simultanea (resposta do state + broadcast WS do mesmo tick).
      if(!prev||now-prev.t>8||prev.pos.distanceToSquared(nextPos)>.000001){remote.snapshots.push({t:now,pos:nextPos.clone(),yaw:nextYaw,alive:state.alive!==false,weapon:state.weapon});if(remote.snapshots.length>12)remote.snapshots.splice(0,remote.snapshots.length-12);}
      ...
      if(alive&&!remote.alive){removeInkPuddle(remote.puddle);remote.puddle=null;remote.snapshots=[{t:now,pos:nextPos.clone(),yaw:nextYaw,alive,weapon:state.weapon}];remote.group.position.copy(nextPos);}
  ...
  // WebSocket e a fonte principal dos snapshots remotos. O POST de state continua enviando
  // nosso movimento, mas nao deve criar uma segunda cadencia visual concorrente.
  window.__inkRealtimeSnapshot = payload => {
    if(!payload || !running) return;lastRealtimeSnapshotAt=performance.now();
  ...
  async function pushMultiplayerState(){
    ...
    try{
      ...
      const data=await response.json();if(response.ok&&performance.now()-lastRealtimeSnapshotAt>250)applyRemoteStates(data.players);
      ...
      if(response.ok&&data.self){
        ...
        else if(!wasAlive){spawnGrace=5;serverSpawnGrace=Math.max(serverSpawnGrace,5);hp=100;armor=100;reserveAmmo={...RESERVE_AMMO};ammo=weaponMode==='knife'?0:(MAGAZINES[weaponMode]||0);clearLocalDeathPuddle();hideDeathScreen();const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;player.pos.set(0,1.7,myTeam==='vermelho'?-36:36);player.yaw=myTeam==='vermelho'?Math.PI:0;hitNoticeEl.textContent='DE VOLTA AO RABISCO! • VIDA, COLETE E MUNIÇÃO CHEIOS';hitNoticeEl.classList.add('show');hitNoticeTimer=1.25;}
  ...
  function startGame(){
    ...
    updateHud(); matchEndOverlay.style.display='none';startEl.classList.add('hidden');overEl.classList.add('hidden');layoutBtn.style.display='block';returnLobbyBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';voiceBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';running=true;applyControlMode();if(isDesktopMode())setTimeout(()=>canvas.requestPointerLock?.(),0);last=performance.now();clock.start();requestAnimationFrame(loop);
  ...
  function update(dt){
    ...
    if(!scopeMode){
      const nearRay=new THREE.Raycaster();nearRay.setFromCamera(new THREE.Vector2(0,0),camera);nearRay.far=1.85;
      const nearTargets=[];scene.traverse(o=>{if(!o.isMesh||!o.visible||o.userData?.noBulletMark||o.userData?.mushroomSmokeParticle||o.userData?.rabbitFloor||o.userData?.rabbitRoof)return;let own=false;for(const v of Object.values(weaponVariants)){if(o===v||v.children.includes(o)){own=true;break;}}if(!own)nearTargets.push(o);});
      const nearHit=nearRay.intersectObjects(nearTargets,true)[0];if(nearHit)weaponWallPush=Math.max(0,Math.min(1,(1.85-nearHit.distance)/1.35));
    }
    ...
    activeWeapon.scale.multiplyScalar(weaponMode==='knife'?.96:.94);if(weaponWallPush>0){activeWeapon.position.z+=weaponWallPush*.24;activeWeapon.position.y-=weaponWallPush*.035;activeWeapon.rotation.x-=weaponWallPush*.18;activeWeapon.rotation.y+=weaponWallPush*.06;activeWeapon.scale.multiplyScalar(1-weaponWallPush*.025);}
