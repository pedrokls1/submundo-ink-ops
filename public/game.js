...
import { giantBedroomMapBody } from './maps/giant-bedroom.js?v=5626';
...
(() => {
  ...
  let cheshireCat=null,cheshireHitCooldown=0,goldenInkwell=null,rewardTakeBusy=false,rewardLocalRespawn=0,rewardHiddenUntil=0;
  ...
  function createRocketInkStains(position,normal=null){
    ...
    for(let i=0;i<count;i++){
      ...
      mesh.userData.noBulletMark=true; mesh.raycast=()=>{}; scene.add(mesh);rocketInkStains.push({mesh,life:16,maxLife:16});
    }
  ...
  async function damageMushroom(id,damage){const m=destructibleMushrooms.get(id);if(!m||!m.active)return;if(multiplayer.room?.status==='playing'){try{await fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'prop_hit',token:multiplayer.token,roomCode:multiplayer.roomCode,propId:id,damage:Math.round(damage)})});}catch(_){}}else{m.hp-=damage;if(m.hp<=0){applyMushroomState({id,hp:0,active:false});m.respawn=60;}}}function applyBedroomTeddyState(st){if(!bedroomTeddy||!st)return;bedroomTeddy.userData.hp=Number(st.hp??260);const alive=st.active===true&&bedroomTeddy.userData.hp>0;const wasVisible=bedroomTeddy.visible;bedroomTeddy.userData.dead=!alive;bedroomTeddy.visible=alive&&activeMap==='quarto';if(bedroomTeddyCollider)bedroomTeddyCollider.active=alive;bedroomTeddyRespawn=Math.max(0,Number(st.respawnMs||0)/1000);if(wasVisible&&!alive)burstTeddyCotton(bedroomTeddy.position);}
  window.__inkRealtimeProp=st=>{const id=String(st?.id||'');if(id==='teddy')applyBedroomTeddyState(st);else if(id.startsWith('r'))applyRabbitState(st);else applyMushroomState(st);};
  ...
  function makeDestructibleRabbit(id,x,z,yaw=0){
    ...
    const rec={id,x,z,baseX:x,baseZ:z,group:g,hp:80,active:true,respawn:0,deadUntil:0,patrolPhase:Math.random()*TAU,patrolSpeed:.52+Math.random()*.20,attackCooldown:0};destructibleRabbits.set(id,rec);return rec;
  }
  function applyRabbitState(st){const r=destructibleRabbits.get(st?.id);if(!r)return;const hp=Number(st.hp);let active=st.active!==false&&Number.isFinite(hp)&&hp>0;const respawnMs=Math.max(0,Number(st.respawnMs||0));if(!active&&respawnMs>0)r.deadUntil=Math.max(r.deadUntil||0,performance.now()+respawnMs);if(active&&performance.now()<(r.deadUntil||0))active=false;if(active)r.deadUntil=0;r.active=active;r.hp=active?hp:0;r.respawn=active?0:Math.max(r.respawn||0,Number(st.respawnMs||0)/1000);r.group.visible=active;r.group.traverse(o=>{if(o.isMesh){o.userData.rabbitId=r.id;o.userData.noBulletMark=!active;o.raycast=active?THREE.Mesh.prototype.raycast:()=>{};}});if(active){r.attackCooldown=.65;r.group.position.set(r.baseX,r.group.position.y,r.baseZ);}}
  ...
  function buildSpawnDirectionSigns(){
    ...
    makeWorldSign('← TOCA DO COELHO',-10,-31,0);
    makeWorldSign('MESA DO CHÁ →',10,-31,0);
    // Base azul: cada face recebe sua própria seta para apontar fisicamente ao destino.
    makeWorldSign('LABIRINTO →',-7,29.5,Math.PI,'← LABIRINTO');
    // A placa da toca foi puxada para perto da Rainha, longe da carta em (12,31).
    makeWorldSign('← TOCA DO COELHO',10,26.2,Math.PI,'TOCA DO COELHO →');
  }
  ...
  // V56.24 — interações leves do Quarto Gigante.
  function burstTeddyCotton(pos){
    if(!pos)return;
    for(let i=0;i<18;i++){
      const puff=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.22+Math.random()*.26,6,5),mat(0xf8f5ea)),1.2);
      puff.position.copy(pos);puff.position.y+=2.2+Math.random()*2.8;
      const a=Math.random()*TAU,s=.9+Math.random()*2.2;
      scene.add(puff);bedroomObjects.push(puff);
      bedroomCotton.push({mesh:puff,v:new THREE.Vector3(Math.cos(a)*s,1.8+Math.random()*2.7,Math.sin(a)*s),life:3.2+Math.random()*1.5});
    }
  }
  async function damageBedroomTeddy(amount){
    if(activeMap!=='quarto'||!bedroomTeddy?.visible||bedroomTeddy.userData.dead)return;
    if(multiplayer.room?.status==='playing'){
      try{const res=await fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'prop_hit',token:multiplayer.token,roomCode:multiplayer.roomCode,propId:'teddy',damage:Math.round(amount)})});const st=await res.json();if(st?.id==='teddy')applyBedroomTeddyState(st);}catch(_){}
      return;
    }
    bedroomTeddy.userData.hp=Math.max(0,(bedroomTeddy.userData.hp??260)-amount);
    const hp=bedroomTeddy.userData.hp;bedroomTeddy.rotation.z=(Math.random()-.5)*.08;
    if(hp<=0){bedroomTeddy.userData.dead=true;bedroomTeddy.visible=false;bedroomTeddyRespawn=50;if(bedroomTeddyCollider)bedroomTeddyCollider.active=false;burstTeddyCotton(bedroomTeddy.position);hitNoticeEl.textContent='PUFF! 🧸';hitNoticeEl.classList.add('show');hitNoticeTimer=1.0;}
  }
  function applyBedroomSharedState(st){
    if(!st)return;
    const off=Math.max(0,Number(st.lightOffMs||0))/1000, lock=Math.max(0,Number(st.lightLockMs||0))/1000;
    bedroomLightOff=off;bedroomLightLock=lock;
    if(activeMap==='quarto'){const dark=off>0;hemi.intensity=dark?.30:2.35;sun.intensity=dark?.22:2.85;if(bedroomLampLight)bedroomLampLight.intensity=dark?.03:2.15;}
    const b=st.ball;if(bedroomBall&&b&&Number.isFinite(Number(b.x))&&Number.isFinite(Number(b.z))){
      const ox=bedroomBall.position.x,oz=bedroomBall.position.z,nx=Number(b.x),nz=Number(b.z),dx=nx-ox,dz=nz-oz;
      bedroomBall.position.set(nx,3.15,nz);bedroomBall.rotation.z-=dx/3.15;bedroomBall.rotation.x+=dz/3.15;bedroomBallVel.set(Number(b.vx)||0,Number(b.vz)||0);
      if(bedroomBallCollider){bedroomBallCollider.minX=nx-3.15;bedroomBallCollider.maxX=nx+3.15;bedroomBallCollider.minZ=nz-3.15;bedroomBallCollider.maxZ=nz+3.15;}
      bedroomBallLastPos.set(nx,nz);
    }
    if(st.teddy)applyBedroomTeddyState(st.teddy);
  }
  async function hitBedroomBall(dir){
    if(!bedroomBall)return;
    const impulse=weaponMode==='sniper'?8.5:(weaponMode==='shotgun'?7.2:(weaponMode==='launcher'?10.5:(weaponMode==='rifle'?4.0:(weaponMode==='smg'?3.1:2.0))));
    if(multiplayer.room?.status==='playing'){try{await fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'ball_hit',token:multiplayer.token,roomCode:multiplayer.roomCode,dx:dir.x,dz:dir.z,impulse})});}catch(_){}}
    else{bedroomBallVel.x+=dir.x*impulse;bedroomBallVel.y+=dir.z*impulse;}
  }
  async function toggleBedroomLight(){
    if(multiplayer.room?.status==='playing'){try{const res=await fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'bedroom_light',token:multiplayer.token,roomCode:multiplayer.roomCode})});const st=await res.json();if(st?.bedroom)applyBedroomSharedState(st.bedroom);}catch(_){}return;}
    if(bedroomLightOff>0){hitNoticeEl.textContent='CLICK! • A LUZ JÁ ESTÁ APAGADA';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;}else if(bedroomLightLock>0){hitNoticeEl.textContent='INTERRUPTOR • AGUARDE '+Math.ceil(bedroomLightLock)+'s';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;}else{bedroomLightOff=10;hemi.intensity=.30;sun.intensity=.22;if(bedroomLampLight)bedroomLampLight.intensity=.03;hitNoticeEl.textContent='CLICK! • LUZES APAGADAS POR 10s';hitNoticeEl.classList.add('show');hitNoticeTimer=1.1;}
  }
  ...
  function buildSmg(){
    ...
    // V56.37: carregador convencional removido; o potinho de tinta é o carregador da SMG.
  ...
  function buildKnife(){
    ...
    // V56.28: faca mais sólida na mão, com guarda e lâmina estilo nanquim.
    const hand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.20,8,6),mat(wallLight)),2.4);
    hand.scale.set(1.0,.82,1.15); hand.position.set(.47,-.61,-.47); g.add(hand);
    const handle=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.20,.22,.58),mat(0x17191d)),2.5);
    handle.position.set(.46,-.54,-.69); handle.rotation.x=-.08; g.add(handle);
    addHatching(handle,.14,.14,.29,3,false);
    const guard=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.42,.075,.12),mat(red)),2.3);
    guard.position.set(.46,-.48,-.98); guard.rotation.z=-.04; g.add(guard);
    const bladeShape=new THREE.Shape();
    bladeShape.moveTo(-.03,0); bladeShape.lineTo(.17,.03); bladeShape.lineTo(.20,.68); bladeShape.lineTo(.06,1.15); bladeShape.lineTo(-.04,.72); bladeShape.closePath();
    const bladeGeo=new THREE.ExtrudeGeometry(bladeShape,{depth:.07,bevelEnabled:false});
    const blade=addOutline(new THREE.Mesh(bladeGeo,mat(0x25282d)),2.5);
    blade.rotation.y=Math.PI/2; blade.rotation.z=-.07; blade.position.set(.39,-.43,-1.02); g.add(blade);
    addHatching(blade,.13,.58,.045,5,false);
    const bladeAccent=new THREE.Mesh(new THREE.BoxGeometry(.035,.035,.48),mat(red));
    bladeAccent.position.set(.455,-.43,-1.38); bladeAccent.rotation.x=Math.PI/2; g.add(bladeAccent);
  ...
  // V56.36: viewmodel confirmado dentro de game.js; pote e mãos trazidos para a camada visível da câmera.
  // V56.34: potinho de tinta como carregador visível nas armas em primeira pessoa.
  function addInkPotMagazine(v,kind){
    if(kind==='launcher')return;
    const pot=new THREE.Group(); pot.userData.inkPotMagazine=true; pot.userData.loadedInkPot=true;
    // Pote/cartucho improvisado: corpo claro, gargalo encaixado na arma e tinta vermelha escorrendo.
    const body=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.17,.18,.38,10),mat(0xf2eee3)),2.4); body.userData.keepViewColor=true; pot.add(body);
    const rim=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.13,.13,.075,10),mat(0x252a31)),2.1); rim.userData.keepViewColor=true; rim.position.y=.225; pot.add(rim);
    const neck=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.085,.10,.11,9),mat(0xf2eee3)),2.0); neck.userData.keepViewColor=true; neck.position.y=.315; pot.add(neck);
    for(const [x,len] of [[-.09,.18],[-.025,.12],[.065,.16]]){
      const d=new THREE.Mesh(new THREE.CapsuleGeometry(.022,len,3,6),mat(0xb51f2e)); d.userData.keepViewColor=true; d.position.set(x,-.055,.174); pot.add(d);
    }
    const stain=new THREE.Mesh(new THREE.SphereGeometry(.062,7,5),mat(0xb51f2e)); stain.userData.keepViewColor=true; stain.scale.set(1.5,.7,.30); stain.position.set(.055,.09,.177); pot.add(stain);
    // Cada arma recebe o pote exatamente onde ficaria o carregador, não escondido dentro do receptor.
    const pos={rifle:[.40,-.76,-.92],sniper:[.40,-.76,-1.16],smg:[.42,-.77,-1.03],shotgun:[.42,-.75,-1.16]}[kind]||[.40,-.70,-.92];
    pot.position.set(...pos); pot.rotation.set(.08,0,-.05); v.add(pot);
  }
  for(const [kind,v] of Object.entries(weaponVariants))if(kind!=='knife')addInkPotMagazine(v,kind);

  // V56.40: pose FPS diagonal baseada nas referências do usuário; braços entram pelas bordas e encontram a arma à direita.
  // V56.37: braços + mãos fazem parte do viewmodel. Antes aparecia quase só a mão em algumas armas.
  // Antebraços vêm da borda inferior da tela até os pontos de pegada e permanecem presos à arma.
  for(const [kind,v] of Object.entries(weaponVariants)){
    if(kind==='knife')continue;
    const handMat=mat(0xfffbef), sleeveMat=mat(0x315f9d);
    const grips={rifle:{rear:[.62,-.53,-.50],front:[.31,-.39,-1.28]},sniper:{rear:[.63,-.54,-.72],front:[.31,-.39,-1.52]},smg:{rear:[.62,-.53,-.56],front:[.31,-.39,-1.12]},shotgun:{rear:[.63,-.53,-.58],front:[.31,-.39,-1.58]},launcher:{rear:[.62,-.53,-.82],front:[.31,-.39,-1.52]}}[kind];
    const makeArmHand=(pt,front=false)=>{
      const hg=new THREE.Group(); hg.position.set(...pt); hg.userData.viewArmHand=true; hg.userData.supportHand=front;
      const sleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.115,1.02,4,7),sleeveMat),2.3);
      // V56.41: antebraço realmente entra de fora/baixo da tela e termina na mão, como nas referências FPS.
      // A mão de apoio vem mais da esquerda; a do gatilho vem de baixo/direita.
      sleeve.rotation.z=front?.91:-.72; sleeve.rotation.x=front?.12:-.06; sleeve.position.set(front?-.58:.43,-.54,.23); hg.add(sleeve);
      const cuff=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.25,.14,.23),sleeveMat),2.1); cuff.position.set(front?-.075:.075,-.115,.045); hg.add(cuff);
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.25,.18,.25),handMat),2.3); palm.userData.keepViewColor=true; palm.rotation.z=front?.10:-.10; hg.add(palm);
      // Dedos em bloco curvo/simplificado envolvendo a arma, sem virar uma bolinha.
      const fingers=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.22,.10,.22),handMat),2.0); fingers.userData.keepViewColor=true; fingers.position.set(0,.015,-.12); hg.add(fingers);
      const thumb=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.045,.15,3,6),handMat),2.0); thumb.userData.keepViewColor=true; thumb.rotation.z=front?-1.05:1.00; thumb.position.set(front?-.12:.12,-.005,-.055); hg.add(thumb);
      v.add(hg);
    };
    makeArmHand(grips.rear,false); makeArmHand(grips.front,true);
  }
  // V56.28: armas grafite/preto hachurado, mantendo pequenos detalhes vermelhos.
  for(const v of Object.values(weaponVariants))v.traverse(o=>{if(o.isMesh&&!o.userData.sniperLens&&o.material?.color&&!o.userData.keepViewColor){const original=o.material.color.getHex();const preservePaper=(original===0xfffbef||original===0xf2eee3||original===0xf3eee2);const preserveInkRed=(original===0xb51f2e||original===0xc62838);if(original===red||original===0xff2038||original===0xd73535||preserveInkRed)o.material.color.setHex(0xb51f2e);else if(!preservePaper)o.material.color.setHex(0x17191d);if(o.material.emissive)o.material.emissive.setHex(0x000000);}});
  // V56.30: hachuras visíveis no viewmodel. Antes todas as linhas eram ocultadas,
  // portanto addHatching() existia mas nunca aparecia na arma em primeira pessoa.
  for(const v of Object.values(weaponVariants))v.traverse(o=>{if(o.isLine||o.isLineSegments){o.visible=true;o.frustumCulled=true;if(o.material){o.material.color?.setHex(0x747d8c);o.material.opacity=.48;o.material.transparent=true;}}});
  // V56.40: a espingarda tinha uma linha/hachura escapando do modelo; nela ocultamos somente linhas auxiliares.
  weaponVariants.shotgun.traverse(o=>{if(o.isLine||o.isLineSegments)o.visible=false;});
  // Detalhes vermelhos adicionais sem transformar a arma num bloco colorido.
  for(const [kind,v] of Object.entries(weaponVariants)){if(kind==='knife')continue;const accent=new THREE.Mesh(new THREE.BoxGeometry(.065,.06,kind==='sniper'?.78:.54),mat(0xc62838));accent.position.set(.405,-.115,kind==='sniper'?-1.36:-1.04);v.add(accent);}
  ...
  function blocked(x,z) {
    ...
    for(const c of colliders){
      ...
      if(c.map && c.map!==activeMap)continue; // V56.38: nunca deixa collider de outro mapa bloquear o Reino.
  ...
  function reloadWeapon(){
    ...
    reloadTimer=reloadMode==='sniper'?1.55:(reloadMode==='launcher'?1.70:(reloadMode==='shotgun'?1.28:(reloadMode==='smg'?1.02:1.16)));
    reloadBtn.textContent='↻...';
    if(!reloadCartridge){
      reloadCartridge=new THREE.Group();
      const bottle=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.34,.40,.30),mat(0xf3eee2)),2.4); reloadCartridge.add(bottle);
      const rim=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.37,.07,.33),mat(0x252a31)),2.1); rim.position.y=.22; reloadCartridge.add(rim);
      // Pote claro com manchas/escorridos vermelhos, não um pote inteiro vermelho.
      for(const x of [-.10,-.025,.075]){const drip=new THREE.Mesh(new THREE.CapsuleGeometry(.025,.13+(x<0?.06:0),3,6),mat(0xb51f2e));drip.position.set(x,-.04,.165);drip.userData.keepViewColor=true;reloadCartridge.add(drip);}
      const stain=new THREE.Mesh(new THREE.SphereGeometry(.055,7,5),mat(0xb51f2e)); stain.scale.set(1.35,.65,.35); stain.position.set(.075,.09,.17);stain.userData.keepViewColor=true; reloadCartridge.add(stain);
      const handGroup=new THREE.Group();
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.25,.19,.28),mat(0xfffbef)),2.4); palm.userData.keepViewColor=true; handGroup.add(palm); const fingers=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.19,.11,.24),mat(0xfffbef)),2.1); fingers.userData.keepViewColor=true; fingers.position.set(0,-.10,-.015); handGroup.add(fingers);
      const thumb=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.05,.15,3,6),mat(0xfffbef)),2.1); thumb.rotation.z=-.95; thumb.position.set(.13,-.01,-.03); handGroup.add(thumb);
      handGroup.position.set(-.25,-.08,.055); handGroup.rotation.z=-.18; reloadCartridge.add(handGroup);
      const reloadSleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.105,.70,4,7),mat(0x315f9d)),2.2); reloadSleeve.rotation.z=-.78; reloadSleeve.position.set(-.62,-.40,.10); reloadCartridge.add(reloadSleeve);
      reloadCartridge.scale.setScalar(.70); reloadCartridge.position.set(-1.45,-.86,-.54); reloadCartridge.rotation.set(-.15,.12,-.22); camera.add(reloadCartridge);
    }
    reloadCartridge.visible=true;reloadCartridge.userData.reloadLife=reloadTimer;reloadCartridge.userData.reloadTotal=reloadTimer;
    setTimeout(()=>{
    ...
    },reloadMode==='sniper'?1550:(reloadMode==='launcher'?1700:(reloadMode==='shotgun'?1280:(reloadMode==='smg'?1020:1160))));
  }
  ...
  function shoot(){
    ...
    if(activeMap==='quarto'&&hit?.object?.userData?.bedroomTeddy)damageBedroomTeddy(weaponMode==='sniper'?90:(weaponMode==='shotgun'?65:(weaponMode==='launcher'?120:32)));if(activeMap==='quarto'&&hit?.object?.userData?.bedroomBall)hitBedroomBall(dir);
    ...
    if(activeMap==='quarto'&&hit?.object?.userData?.lightSwitch)toggleBedroomLight();

    if(weaponMode==='knife'){
      fireCooldown=.38; knifeSwing=1; knifeSound();
    ...
    if(hit){
      ...
      if(remoteToken){
        ...
        const headshot=hit.object.userData?.hitZone==='head';
        // V56.27: sniper HEAD=100 (morte em 1 tiro); fuzil HEAD=34 (3 tiros em 100 HP).
  ...
  function makeRemotePlayer(state){
    // V56.27: personagem multiplayer redesenhado para combinar com a capa do DRAWN FRONT.
    // Cabeça redonda maior, corpo compacto e grandes blocos de cor deixam os times legíveis à distância.
    const group=new THREE.Group();
    const teamColor=state.team==='vermelho'?0xff2038:0x087cff;
    const darkMat=mat(0x08090b), paperMat=mat(0xfffbef), accentMat=mat(teamColor);
    const legs=[];
    for(const x of [-.25,.25]){
      const leg=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.145,.48,4,7),accentMat),3.0);
      leg.position.set(x,.42,0); leg.rotation.z=x<0?.055:-.055; group.add(leg); legs.push(leg);
      const boot=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.28,.18,.40),darkMat),2.7);
      boot.position.set(x,.13,-.09); boot.rotation.y=x<0?-.05:.05; group.add(boot);
    }
    const body=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.43,.48,5,8),accentMat),3.5);
    body.scale.set(1.15,1.03,.78); body.position.set(0,1.18,0); group.add(body); // V56.37: corpo voltou a ser realmente adicionado ao grupo; fecha o vazado do tronco.
    // V56.33: colete fino desenhado SOMENTE sobre o tórax. Não alcança axilas/ombros,
    // portanto os braços podem dobrar à frente sem atravessar uma caixa preta.
    const vest=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.68,.40,.070),mat(0x252a31)),2.4);
    vest.position.set(0,1.23,-.285); group.add(vest);
    const vestBack=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.68,.40,.070),mat(0x252a31)),2.4);
    vestBack.position.set(0,1.23,.285); group.add(vestBack);
    const vestPlate=new THREE.Mesh(new THREE.BoxGeometry(.54,.26,.018),mat(0x3e4652)); vestPlate.position.set(0,1.23,-.328); group.add(vestPlate);
    const vestBackPlate=new THREE.Mesh(new THREE.BoxGeometry(.54,.26,.018),mat(0x3e4652)); vestBackPlate.position.set(0,1.23,.328); group.add(vestBackPlate);
    const chestMark=new THREE.Mesh(new THREE.BoxGeometry(.31,.055,.018),accentMat);
    chestMark.position.set(0,1.31,-.318); group.add(chestMark);

    // A esfera visível É a referência da hitbox: tudo que pertence à cabeça recebe hitZone=head.
    // V56.29: cabeça limpa. Não usamos EdgesGeometry na esfera porque as arestas internas
    // pareciam riscos/cabelo. O contorno agora é apenas a silhueta externa.
    const head=new THREE.Mesh(new THREE.SphereGeometry(.44,18,12),paperMat);
    head.position.y=2.24; group.add(head);
    const headOutline=new THREE.Mesh(new THREE.SphereGeometry(.458,18,12),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    headOutline.position.y=2.24; group.add(headOutline);
    const browMat=darkMat;
    for(const side of [-1,1]){
      const eye=new THREE.Mesh(new THREE.BoxGeometry(.17,.075,.045),browMat);
      eye.position.set(side*.15,2.26,-.405); eye.rotation.z=side*.20; group.add(eye);
    }
    // V56.31: rosto fiel à referência do usuário: olhos fechados/inclinados e boca única,
    // simples, baixa e quase reta. Sem sorriso, sem traços extras perto dos olhos.
    const mouth=new THREE.Mesh(new THREE.BoxGeometry(.22,.025,.035),darkMat);
    mouth.position.set(0,2.095,-.425); mouth.rotation.z=.10; group.add(mouth);
    // V56.33: pescoço curto + trapézio dão largura aos ombros sem voltar às antigas 'bolas'.
    // São apenas visuais/body: a hitbox de HEADSHOT continua restrita à cabeça acima.
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.145,.19,.46,8),paperMat);
    neck.position.set(0,1.84,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.153,.198,.465,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    neckOutline.position.copy(neck.position); group.add(neckOutline);
    const trapezius=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.94,.18,.38),accentMat),2.5);
    trapezius.position.set(0,1.49,0); trapezius.scale.set(1,.78,.74); group.add(trapezius);

    const arms=[];
    // V56.32: braços em DUAS partes, com cotovelo real. Evita o aspecto de tábua/régua da V56.31.
    const armBetween=(a,b)=>{
      const mid=a.clone().add(b).multiplyScalar(.5), len=a.distanceTo(b);
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.082,Math.max(.10,len-.16),4,7),accentMat),2.4);
      arm.position.copy(mid); arm.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),b.clone().sub(a).normalize());
      group.add(arm); arms.push(arm); return arm;
    };
    // Ombros discretos e braços assimétricos: mão dianteira sustenta o cano; mão traseira fica no punho/gatilho.
    const shoulderL=new THREE.Vector3(-.50,1.47,-.005), shoulderR=new THREE.Vector3(.50,1.47,-.005);
    const elbowL=new THREE.Vector3(-.38,1.23,-.34), elbowR=new THREE.Vector3(.34,1.20,-.16);
    const handLPos=new THREE.Vector3(-.04,1.16,-.91), handRPos=new THREE.Vector3(.24,1.16,-.48); // V56.38: mãos separadas; apoio à frente e gatilho atrás, sem formar triângulo no peito.
    for(const pos of [shoulderL,shoulderR]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.088,8,6),accentMat),2.2);
      shoulder.scale.set(1.12,.80,.82); shoulder.position.copy(pos); group.add(shoulder);
    }
    armBetween(shoulderL,elbowL); armBetween(elbowL,handLPos);
    armBetween(shoulderR,elbowR); armBetween(elbowR,handRPos);
    for(const [pos,front] of [[handLPos,true],[handRPos,false]]){
      const hg=new THREE.Group(); hg.position.copy(pos);
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.16,.13,.18),paperMat),2.2); palm.rotation.z=front?.10:-.10; hg.add(palm);
      const thumb=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.035,.10,3,6),paperMat),2.0); thumb.rotation.z=front?-1.00:.95; thumb.position.set(front?-.075:.075,-.015,-.035); hg.add(thumb);
      group.add(hg);
    }

    // Arma do personagem: preta, fina e curta o bastante para não esconder a silhueta.
    const gunGroup=new THREE.Group();
    const gun=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.145,.14,.80),darkMat),2.5);
    gun.position.set(.16,1.25,-.60); gunGroup.add(gun);
    const barrel=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.035,.045,.48,7),darkMat),2.2);
    barrel.rotation.x=Math.PI/2; barrel.position.set(.16,1.25,-1.12); gunGroup.add(barrel);
    // V56.34: o 'carregador' é um potinho de tinta pendurado sob a arma.
    const mag=new THREE.Group();
    const potBody=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.18,.24,.18),mat(0xf2eee3)),2.1); mag.add(potBody);
    const potNeck=new THREE.Mesh(new THREE.BoxGeometry(.10,.07,.10),mat(0x252a31)); potNeck.position.y=.145; mag.add(potNeck);
    for(const x of [-.045,.035]){const drip=new THREE.Mesh(new THREE.CapsuleGeometry(.014,.08,3,5),mat(0xb51f2e));drip.position.set(x,-.055,-.098);mag.add(drip);}
    mag.position.set(.16,1.04,-.68); mag.rotation.x=.10; gunGroup.add(mag);
    // Hachura contida no corpo da arma. A versão anterior usava linhas largas demais e
    // duas delas apareciam 'vazando' para os lados do personagem.
    const remoteGunHatch=addHatching(gun,.12,.105,.48,7,true); remoteGunHatch.scale.x=.095;
    // V56.31: quebra o bloco preto com trilhos/placas grafite e detalhes vermelhos legíveis.
    const gunRail=new THREE.Mesh(new THREE.BoxGeometry(.155,.025,.52),mat(0x596273)); gunRail.position.set(.16,1.335,-.60); gunGroup.add(gunRail);
    const gunSide=new THREE.Mesh(new THREE.BoxGeometry(.151,.065,.34),mat(0x343941)); gunSide.position.set(.16,1.255,-.61); gunGroup.add(gunSide);
    const gunAccent=new THREE.Mesh(new THREE.BoxGeometry(.175,.035,.30),mat(0xc62838));
    gunAccent.position.set(.16,1.315,-.58); gunGroup.add(gunAccent);
    const gunAccent2=new THREE.Mesh(new THREE.BoxGeometry(.05,.15,.22),mat(0xc62838)); gunAccent2.position.set(.16,1.17,-.74); gunGroup.add(gunAccent2);
    const muzzleAccent=new THREE.Mesh(new THREE.BoxGeometry(.075,.055,.08),mat(0xc62838)); muzzleAccent.position.set(.16,1.25,-1.35); gunGroup.add(muzzleAccent);
    group.add(gunGroup);

    const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;
    if(myTeam&&myTeam===state.team)group.add(makePlayerLabel(state.nickname,state.team==='vermelho'?'#ff2038':'#087cff'));
    group.traverse(o=>{if(o.isMesh){o.userData.remoteToken=state.token;o.userData.hitZone='body';}});
    // Cabeça + olhos são uma única região lógica. Não existe mais faixa/objeto de rosto marcado como corpo.
    head.userData.hitZone='head';
    // V56.33: pescoço/trapézio permanecem BODY. Só a esfera da cabeça e os elementos do rosto
    // que estão sobre ela contam como headshot; adicionar pescoço não aumenta a área de HS.
    // V56.41: headshot EXCLUSIVAMENTE por hitZone da cabeça/pescoço. Removido fallback por altura que fazia o tórax virar HEADSHOT.
    // O pescoço foi alongado/reposicionado para ficar realmente visível entre trapézio e cabeça.
    head.userData.hitZone='head'; headOutline.userData.hitZone='head'; neck.userData.hitZone='head'; neckOutline.userData.hitZone='head';
    for(const o of group.children){if(o.isMesh && (o===head||o===headOutline||o===neck||o===neckOutline||o.position.y>2.00))o.userData.hitZone='head';}
    trapezius.userData.hitZone='body'; body.userData.hitZone='body'; vest.userData.hitZone='body'; vestBack.userData.hitZone='body';
    group.position.set(state.x,state.y-1.7,state.z); group.rotation.y=state.yaw+Math.PI; scene.add(group);
    const initial=new THREE.Vector3(state.x,state.y-1.7,state.z);
    return {group,legs,arms,gun:gunGroup,head,vest,target:initial.clone(),predicted:initial.clone(),lastNetPos:initial.clone(),velocity:new THREE.Vector3(),lastPacketAt:performance.now(),targetYaw:state.yaw+Math.PI,step:0,footTimer:.12+Math.random()*.18,weapon:state.weapon,team:state.team,puddle:null,alive:state.alive!==false};
  }
  function updateRemotePlayers(dt){
    ...
    for(const remote of multiplayer.remotePlayers.values()){
      ...
      remote.step+=dt*(moving?14.5:2);const walk=moving?Math.sin(remote.step):0;remote.legs[0].rotation.x=walk*.92;remote.legs[1].rotation.x=-walk*.92;remote.legs[0].rotation.z=.055+(moving?Math.abs(walk)*.075:0);remote.legs[1].rotation.z=-.055-(moving?Math.abs(walk)*.075:0);remote.legs[0].position.y=.42+(moving?Math.max(0,-walk)*.055:0);remote.legs[1].position.y=.42+(moving?Math.max(0,walk)*.055:0);remote.footTimer=(remote.footTimer||0)-dt;if(moving&&remote.footTimer<=0){remoteFootstepSound(remote);remote.footTimer=.32+Math.random()*.06;}
  ...
  window.__inkRealtimeSnapshot = payload => {
    ...
    applyRemoteStates(payload.players || []);if(Array.isArray(payload.props))for(const st of payload.props){const id=String(st?.id||'');if(id==='teddy')applyBedroomTeddyState(st);else if(id.startsWith('r'))applyRabbitState(st);else applyMushroomState(st);}if(payload.bedroom)applyBedroomSharedState(payload.bedroom);if(payload.cat)setCheshireState(payload.cat);if(payload.reward)setGoldenRewardState(payload.reward);if(payload.match){const sec=Math.max(0,Math.ceil(Number(payload.match.remainingMs||0)/1000)),mm=String(Math.floor(sec/60)),ss=String(sec%60).padStart(2,'0'),clockText=mm+':'+ss+'  •  40 KILLS';if(matchClockEl.style.display!=='block')matchClockEl.style.display='block';if(matchClockEl.textContent!==clockText)matchClockEl.textContent=clockText;if(payload.match.status==='finished')window.__inkRealtimeMatchEnd?.(payload.match);}
  ...
  function update(dt){
    ...
    outletCooldown=Math.max(0,outletCooldown-dt);if(multiplayer.room?.status!=='playing')bedroomLightLock=Math.max(0,bedroomLightLock-dt);if(multiplayer.room?.status!=='playing'&&bedroomLightOff>0){const prev=bedroomLightOff;bedroomLightOff=Math.max(0,bedroomLightOff-dt);if(prev>0&&bedroomLightOff<=0){hemi.intensity=2.35;sun.intensity=2.85;if(bedroomLampLight)bedroomLampLight.intensity=2.15;bedroomLightLock=35;hitNoticeEl.textContent='💡 LUZES ACESAS • 35s';hitNoticeEl.classList.add('show');hitNoticeTimer=1.0;}}duckBounceCooldown=Math.max(0,duckBounceCooldown-dt);
    if(activeMap==='quarto'){
      // Bola no solo: tiros empurram, ela rola, rebate nas paredes e pode esmagar o jogador.
      if(bedroomBall&&multiplayer.room?.status!=='playing'){
        const speed=bedroomBallVel.length();if(speed>.02){const ox=bedroomBall.position.x,oz=bedroomBall.position.z;bedroomBall.position.x+=bedroomBallVel.x*dt;bedroomBall.position.z+=bedroomBallVel.y*dt;if(bedroomBall.position.x<-45||bedroomBall.position.x>45){bedroomBall.position.x=Math.max(-45,Math.min(45,bedroomBall.position.x));bedroomBallVel.x*=-.72;}if(bedroomBall.position.z<-36||bedroomBall.position.z>36){bedroomBall.position.z=Math.max(-36,Math.min(36,bedroomBall.position.z));bedroomBallVel.y*=-.72;}const dx=bedroomBall.position.x-ox,dz=bedroomBall.position.z-oz;bedroomBall.rotation.z-=dx/3.15;bedroomBall.rotation.x+=dz/3.15;bedroomBallVel.multiplyScalar(Math.pow(.34,dt));if(bedroomBallCollider){const x=bedroomBall.position.x,z=bedroomBall.position.z;bedroomBallCollider.minX=x-3.15;bedroomBallCollider.maxX=x+3.15;bedroomBallCollider.minZ=z-3.15;bedroomBallCollider.maxZ=z+3.15;}if(speed>7&&Math.hypot(player.pos.x-bedroomBall.position.x,player.pos.z-bedroomBall.position.z)<3.8&&hp>0){hp=0;armor=0;createLocalDeathPuddle();showDeathScreen(4);hitNoticeEl.textContent='💥 ESMAGADO PELA BOLA!';hitNoticeEl.classList.add('show');hitNoticeTimer=1.4;}}
      }
      // Pelúcia: permanece rasgada por 50 s; algodão cai e desaparece sem criar colisores.
      if(bedroomTeddyRespawn>0){bedroomTeddyRespawn=Math.max(0,bedroomTeddyRespawn-dt);if(bedroomTeddyRespawn<=0&&bedroomTeddy){bedroomTeddy.userData.hp=260;bedroomTeddy.userData.dead=false;bedroomTeddy.rotation.z=0;bedroomTeddy.visible=true;if(bedroomTeddyCollider)bedroomTeddyCollider.active=true;}}
      for(let i=bedroomCotton.length-1;i>=0;i--){const c=bedroomCotton[i];c.life-=dt;c.v.y-=4.8*dt;c.mesh.position.addScaledVector(c.v,dt);if(c.mesh.position.y<.18){c.mesh.position.y=.18;c.v.multiplyScalar(.55);c.v.y=0;}if(c.life<=0){scene.remove(c.mesh);bedroomCotton.splice(i,1);}}
    ...
    if(reloadCartridge?.visible){reloadCartridge.userData.reloadLife=Math.max(0,(reloadCartridge.userData.reloadLife||0)-dt);const total=Math.max(.01,reloadCartridge.userData.reloadTotal||.95),p=1-Math.min(1,reloadCartridge.userData.reloadLife/total),approach=Math.max(0,Math.min(1,p/.68)),insert=Math.max(0,Math.min(1,(p-.55)/.30));const smooth=approach*approach*(3-2*approach);reloadCartridge.rotation.y=-.34+smooth*.30;reloadCartridge.rotation.z=-.28+smooth*.22;reloadCartridge.position.x=-1.45+smooth*1.63;reloadCartridge.position.y=-.86+Math.sin(approach*Math.PI)*.18+insert*.10;reloadCartridge.position.z=-.54-smooth*.58;}
    ...
    // V56.39: durante a recarga, a mão de apoio sai da arma e entra pela esquerda com o pote; evita a 'terceira mão'.
    if(weaponMode!=='knife')activeWeapon.traverse(o=>{if(o.userData?.supportHand)o.visible=reloadTimer<=0;if(o.userData?.loadedInkPot)o.visible=reloadTimer<=0;});
    ...
    if(!scopeMode && weaponMode!=='knife'){
      ...
      for(const d of samples){
        ...
        for(const c of colliders){
          if(c.active===false)continue;
          if(c.map&&c.map!==activeMap)continue;
          if(c.minY!=null && feetY<c.minY-.08)continue;
          if(c.height!=null && feetY>c.height-.08)continue;
      ...
      // V56.39: o centro caminhável da Rainha não deve recolher a arma por collider fantasma/auxiliar.
      if(activeMap==='reino'&&Math.abs(player.pos.x)<4.2&&player.pos.z>6.0&&player.pos.z<13.6)weaponWallTarget=0;
    }
    ...
    if(weaponMode==='rifle'){
      ...
      k.position.set(.02+.16*swing+runSwayX,-.02-.13*swing-runSwayY,-.02-.26*swing);
      k.rotation.set(-.10-.92*swing,.18+.46*swing,-.16-1.28*swing+runTilt);
    ...
    // V56.29: escala FINAL por arma. Antes o update anulava as escalas definidas no build e
    // fazia principalmente a sniper ocupar quase metade da tela. Agora o viewmodel fica baixo e
    // no canto direito, sem alterar o tamanho da arma vista pelos outros jogadores.
    const viewScale={rifle:.61,sniper:.53,smg:.61,shotgun:.59,launcher:.57,knife:.78}[weaponMode]||.61;
    activeWeapon.scale.multiplyScalar(viewScale);
    if(weaponMode!=='knife'){activeWeapon.position.x+=.17;activeWeapon.position.y-=.115;activeWeapon.position.z+=.10;}
    // V56.34: durante a recarga a arma sobe/inclina para receber o potinho por baixo.
    if(reloadTimer>0&&weaponMode!=='knife'){const rt=Math.max(.01,reloadCartridge?.userData.reloadTotal||reloadTimer),rp=1-Math.min(1,reloadTimer/rt),lift=Math.sin(Math.min(1,rp)*Math.PI);activeWeapon.position.y+=.20*lift;activeWeapon.position.z+=.09*lift;activeWeapon.rotation.x-=.27*lift;activeWeapon.rotation.z+=.07*lift;}
    if(weaponWallPush>.001){activeWeapon.position.z+=weaponWallPush*.72;activeWeapon.position.y-=weaponWallPush*.12;activeWeapon.rotation.x-=weaponWallPush*.42;activeWeapon.rotation.y+=weaponWallPush*.10;activeWeapon.scale.multiplyScalar(1-weaponWallPush*.06);}
    weaponKick=Math.max(0,weaponKick-dt*4.5);
    knifeSwing=Math.max(0,knifeSwing-dt*8.5);
