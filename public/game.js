...
(() => {
  ...
  window.__inkRealtimeSnapshot = payload => {
    ...
    if(payload.self){const oldHp=hp,serverHp=Math.max(0,Math.min(100,Number(payload.self.health??hp)));const wasAlive=multiplayer.selfAlive;multiplayer.selfAlive=payload.self.alive;if(serverHp<oldHp)triggerDamageFeedback(oldHp-serverHp);if(!payload.self.alive){hp=0;createLocalDeathPuddle();showDeathScreen(payload.self.respawnSeconds);fireHeld=false;scopeMode=false;}else if(!wasAlive){hp=100;armor=100;reserveAmmo={...RESERVE_AMMO};ammo=weaponMode==='knife'?0:(MAGAZINES[weaponMode]||0);clearLocalDeathPuddle();hideDeathScreen();const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;player.pos.set(0,1.7,myTeam==='vermelho'?-36:36);player.yaw=myTeam==='vermelho'?0:Math.PI;updateHud();}else{hp=serverHp;hideDeathScreen();updateHud();}}
    if(payload.scores){
  ...
  async function ensureVoicePeer(token,offer=false){if(!voiceEnabled||!token||token===multiplayer.token)return null;let entry=voicePeers.get(token);if(entry)return entry;const pc=new RTCPeerConnection({iceServers:[{urls:'stun:stun.l.google.com:19302'}]});const audio=document.createElement('audio');audio.autoplay=true;audio.playsInline=true;document.body.appendChild(audio);entry={pc,audio};voicePeers.set(token,entry);voiceStream?.getTracks().forEach(track=>pc.addTrack(track,voiceStream));pc.ontrack=e=>{audio.srcObject=e.streams[0]};pc.onicecandidate=e=>{if(e.candidate)window.__inkRealtimeSendVoice?.(token,{candidate:e.candidate.toJSON()})};pc.onconnectionstatechange=()=>{if(['failed','closed','disconnected'].includes(pc.connectionState))closeVoicePeer(token)};if(offer){const desc=await pc.createOffer({offerToReceiveAudio:true});await pc.setLocalDescription(desc);await window.__inkRealtimeSendVoice?.(token,{description:pc.localDescription});}return entry;}
