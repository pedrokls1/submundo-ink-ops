...
(() => {
  ...
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const makeArmHand=(pt,front=false)=>{
      ...
      const sleeve=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.105,.66,4,7),sleeveMat),2.3);
      sleeve.rotation.z=front?.82:-.66; sleeve.rotation.x=front?.10:-.08; sleeve.position.set(front?-.40:.31,-.38,.20); hg.add(sleeve);
      const cuff=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.24,.12,.22),sleeveMat),2.1); cuff.position.set(front?-.06:.06,-.10,.04); hg.add(cuff);
  ...
  function shoot(){
    ...
    if(hit){
      ...
      if(remoteToken){
        ...
        const headshot=hit.object.userData?.hitZone==='head'||hitLocalY>=1.62;
  ...
  function makeRemotePlayer(state){
    ...
    head.position.y=2.20; group.add(head);
    ...
    headOutline.position.y=2.20; group.add(headOutline);
    const browMat=darkMat;
    for(const side of [-1,1]){
      const eye=new THREE.Mesh(new THREE.BoxGeometry(.17,.075,.045),browMat);
      eye.position.set(side*.15,2.22,-.405); eye.rotation.z=side*.20; group.add(eye);
    }
    ...
    mouth.position.set(0,2.055,-.425); mouth.rotation.z=.10; group.add(mouth);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.14,.18,.40,8),paperMat);
    neck.position.set(0,1.76,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.148,.188,.405,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    ...
    // V56.40: headshot somente na cabeça/rosto e no pescoço VISÍVEL. Nada do tórax entra por altura.
