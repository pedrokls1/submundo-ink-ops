...
(() => {
  ...
  function createMultiplayerLobby(){
    ...
    const render=payload=>{
      ...
      const me=payload.players.find(p=>p.token===multiplayer.token);const allReady=payload.players.length>0&&payload.players.every(p=>p.ready===true);const startButton=q('[data-start]');startButton.style.display=me?.is_host?'block':'none';startButton.disabled=!allReady;startButton.textContent=allReady?'INICIAR PARTIDA':'AGUARDANDO TODOS FICAREM PRONTOS';startButton.style.background=allReady?'#d73535':'#777';const readyButton=q('[data-ready]');readyButton.textContent=me?.ready?'PRONTO ✓ — TOQUE PARA CANCELAR':'MARCAR PRONTO';readyButton.style.background=me?.ready?'#3d8b68':'#eee7d4';readyButton.style.color=me?.ready?'#fff':'#171510';
      ...
      setStatus(payload.room.status==='playing'?'A partida começou! Entrando no mapa…':payload.players.length+'/10 jogadores • '+payload.players.filter(p=>p.ready).length+' prontos. Todos confirmam; depois o líder inicia.');
