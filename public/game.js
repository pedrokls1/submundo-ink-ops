...
(() => {
  ...
  function spawnMushroomSmoke(m){for(let i=0;i<54;i++){const puff=new THREE.Mesh(new THREE.SphereGeometry(.55+Math.random()*.75,7,6),new THREE.MeshBasicMaterial({color:i%4===0?0x203229:(i%3===0?0x111814:0x26372f),transparent:true,opacity:.88,depthWrite:false}));const a=Math.random()*TAU,r=Math.random()*3.5;puff.position.set(m.x+Math.cos(a)*r,.7+Math.random()*3.9,m.z+Math.sin(a)*r);scene.add(puff);mushroomSmoke.push({mesh:puff,vx:(Math.random()-.5)*.35,vy:.08+Math.random()*.22,vz:(Math.random()-.5)*.35,life:5.8+Math.random()*1.5,max:7});}}
  ...
  function buildRabbitTunnel(){const pts=[[-40,-31],[-34,-25],[-27,-22],[-18,-25],[-9,-31],[0,-32],[10,-26],[17,-15],[23,-5],[30,7],[35,20],[40,31]];window.__inkRabbitPath=pts;window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0},total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz);if(d<best.d)best={d,t:(acc+lens[i]*u)/total};acc+=lens[i];}return best;};window.__inkRabbitTunnelY=t=>{const edge=Math.min(t,1-t);return edge<.055?1.7-(edge/.055)*4.45:-2.75;};function entrance(x,z,rot){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const arch=addOutline(new THREE.Mesh(new THREE.TorusGeometry(2,.4,8,24,Math.PI),mat(wall)));arch.rotation.z=Math.PI;arch.position.y=1.7;gg.add(arch);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.66,20),new THREE.MeshBasicMaterial({color:black,side:THREE.DoubleSide}));dark.position.set(0,1.5,.1);gg.add(dark);const earL=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.24,.92,4,6),mat(wallLight)));earL.position.set(-.52,3.05,0);earL.rotation.z=-.16;gg.add(earL);const earR=earL.clone();earR.position.x=.52;earR.rotation.z=.16;gg.add(earR);scene.add(gg);makeWorldSign('TOCA DO COELHO',x+(rot===0?3.3:-3.3),z+(rot===0?-2.6:2.6),rot);return {x,z};}window.__inkRabbitEntrances=[entrance(-40,-31,0),entrance(40,31,Math.PI)];let total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(y1-y0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floor=box(4.8,.2,len+.85,mat(0xe9e4d8),mx,ym+.02,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;const roof=box(5.05,.3,len+1.05,mat(0xcfc8b9),mx,ym+3.05,mz,false);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.42,wz=mz-Math.sin(yaw)*side*2.42;const wallSeg=box(.3,3.25,len+1,mat(wall),wx,ym+1.53,wz,false);wallSeg.rotation.order='YXZ';wallSeg.rotation.y=yaw;wallSeg.rotation.x=-pitch;}acc+=len;}}
  function buildCheshireCat(){const gr=new THREE.Group();const body=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.05,9,7),mat(black)));body.scale.set(1.5,.75,1);gr.add(body);const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.78,9,7),mat(0x2c3140)));head.position.set(0,1.05,0);gr.add(head);for(const sx of [-1,1]){const eye=new THREE.Mesh(new THREE.SphereGeometry(.13,6,5),new THREE.MeshBasicMaterial({color:0x66d7ff}));eye.position.set(sx*.27,1.18,-.69);gr.add(eye);}const smile=new THREE.Mesh(new THREE.TorusGeometry(.34,.055,6,18,Math.PI),new THREE.MeshBasicMaterial({color:red}));smile.position.set(0,.9,-.7);smile.rotation.z=Math.PI;gr.add(smile);gr.position.set(-27,1.4,24);gr.userData.hp=180;gr.userData.alive=true;gr.userData.respawn=0;gr.userData.dir=new THREE.Vector3(1,0,.35).normalize();gr.traverse(o=>{if(o.isMesh)o.userData.cheshire=true;});scene.add(gr);cheshireCat=gr;}
  ...
  function updateCheshireCat(dt){if(!cheshireCat)return;if(!cheshireCat.userData.alive){if(multiplayer.room?.status!=='playing'){cheshireCat.userData.respawn-=dt;if(cheshireCat.userData.respawn<=0){cheshireCat.userData.hp=180;cheshireCat.userData.alive=true;cheshireCat.visible=true;}}return;}const dir=cheshireCat.userData.dir;let nx=cheshireCat.position.x+dir.x*1.7*dt,nz=cheshireCat.position.z+dir.z*1.7*dt;if(catBlocked(nx,nz)||nx>-10||nx<-42||nz<12||nz>38){dir.x*=-1;dir.z=(Math.random()-.5)*1.4;dir.normalize();nx=cheshireCat.position.x+dir.x*.4;nz=cheshireCat.position.z+dir.z*.4;}cheshireCat.position.x=nx;cheshireCat.position.z=nz;cheshireCat.position.y=1.5+Math.sin(performance.now()/280)*.12;cheshireHitCooldown=Math.max(0,cheshireHitCooldown-dt);if(player.pos.y>0&&cheshireHitCooldown<=0&&player.pos.distanceTo(cheshireCat.position)<2.15){cheshireHitCooldown=1.35;triggerDamageFeedback(26);hitNoticeEl.textContent='GATO DE CHESHIRE • ARRANHÃO FORTE!';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;if(multiplayer.room?.status==='playing')fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'hazardhit',token:multiplayer.token,roomCode:multiplayer.roomCode})}).catch(()=>{});else{let d=26;if(armor>0){const a=Math.min(armor,10);armor-=a;d-=a;}hp=Math.max(0,hp-d);updateHud();}}}
  ...
  let sniperAimArmed=false,sniperAimPointer=null,sniperReleaseStamp=0;
  const cancelScopeBtn=document.createElement('button');cancelScopeBtn.id='cancelScope';cancelScopeBtn.textContent='CANCELAR';cancelScopeBtn.style.cssText='position:fixed;left:50%;bottom:11%;transform:translateX(-50%);z-index:62;display:none;padding:9px 16px;border:3px solid #171510;border-radius:10px;background:rgba(215,53,53,.92);color:#fff;font:900 13px Georgia;pointer-events:auto';document.body.appendChild(cancelScopeBtn);
  function closeScope(cancelShot=false){if(cancelShot)sniperAimArmed=false;scopeMode=false;scopeBtn._look=null;scopeBtn.style.background='rgba(238,231,212,.82)';scopeBtn.style.color='#171510';cancelScopeBtn.style.display='none';}
  function fireSniperRelease(){if(!sniperAimArmed||weaponMode!=='sniper'||!running)return;const now=performance.now();if(now-sniperReleaseStamp<120)return;sniperReleaseStamp=now;sniperAimArmed=false;if(ammo>0&&reloadTimer<=0){if(fireCooldown>0&&fireCooldown<.18)fireCooldown=0;shoot();}closeScope(false);}
  function toggleScope(){if(!running||weaponMode==='knife')return;scopeMode=!scopeMode;if(!scopeMode)sniperAimArmed=false;scopeBtn.style.background=scopeMode?'rgba(49,94,155,.9)':'rgba(238,231,212,.82)';scopeBtn.style.color=scopeMode?'#fff':'#171510';centerEl.style.opacity=scopeMode&&weaponMode==='sniper'?'0':'1';cancelScopeBtn.style.display=scopeMode&&weaponMode==='sniper'?'block':'none';}
  ...
  window.__inkRealtimeSnapshot = payload => {
    ...
    applyRemoteStates(payload.players || []);if(Array.isArray(payload.props))for(const st of payload.props)applyMushroomState(st);if(payload.cat)setCheshireState(payload.cat);if(payload.match){const sec=Math.max(0,Math.ceil(Number(payload.match.remainingMs||0)/1000)),mm=String(Math.floor(sec/60)),ss=String(sec%60).padStart(2,'0'),clockText=mm+':'+ss+'  •  40 KILLS';if(matchClockEl.style.display!=='block')matchClockEl.style.display='block';if(matchClockEl.textContent!==clockText)matchClockEl.textContent=clockText;if(payload.match.status==='finished')window.__inkRealtimeMatchEnd?.(payload.match);}
    ...
    if(payload.scores){
      if(onlineScoreEl.style.display!=='block')onlineScoreEl.style.display='block';const scoreHTML='<span style="color:#294aa3">AZUL '+Number(payload.scores.azul||0)+'/40</span> &nbsp;×&nbsp; <span style="color:#d73535">'+Number(payload.scores.vermelho||0)+'/40 VERMELHO</span>';if(onlineScoreEl.innerHTML!==scoreHTML)onlineScoreEl.innerHTML=scoreHTML;
    }
  };
