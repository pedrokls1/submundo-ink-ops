...
(() => {
  ...
  function addInkPotMagazine(v,kind){
    ...
    const body=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.30,.40,.28),mat(0xf2eee3)),2.4); body.userData.keepViewColor=true; pot.add(body);
    const rim=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.32,.065,.30),mat(0x252a31)),2.1); rim.userData.keepViewColor=true; rim.position.y=.215; pot.add(rim);
    ...
    const pos={rifle:[.56,-.69,-1.05],sniper:[.54,-.61,-1.22],smg:[.55,-.70,-1.08],shotgun:[.55,-.61,-1.05]}[kind]||[.55,-.68,-1.08];
  ...
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const rearZ=kind==='sniper'?-1.02:-.79, frontZ=kind==='launcher'?-1.58:(kind==='sniper'?-1.90:-1.45);
    const makeGripHand=(x,y,z,front=false)=>{
      ...
      const palm=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.24,.18,.28),handMat),2.3); palm.userData.keepViewColor=true; palm.rotation.z=front?.12:-.12; hg.add(palm); const fingers=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.18,.10,.22),handMat),2.0); fingers.userData.keepViewColor=true; fingers.position.set(0,-.09,-.025); hg.add(fingers);
    ...
    makeGripHand(.66,-.54,rearZ,false);
    makeGripHand(.64,-.40,frontZ,true);
  }
  ...
  function makeRemotePlayer(state){
    ...
    body.scale.set(1.27,1.03,.74); body.position.set(0,1.18,0); group.add(body);
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.125,.17,.27,8),paperMat);
    neck.position.set(0,1.69,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.133,.178,.272,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
    neckOutline.position.copy(neck.position); group.add(neckOutline);
    const trapezius=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.88,.18,.38),accentMat),2.5);
    ...
    const shoulderL=new THREE.Vector3(-.49,1.47,-.005), shoulderR=new THREE.Vector3(.49,1.47,-.005);
