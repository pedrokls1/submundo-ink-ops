// V56.18 RAINHA — polimento: placas spawn, coelhos agressivos, toca/maze colisão, guache
...
(() => {
  ...
  function buildRabbitTunnel(){
    ...
    for(let i=1;i<pts.length-1;i++){
      ...
      colliders.push({minX:ox-1.47,maxX:ox+1.47,minZ:oz-1.47,maxZ:oz+1.47,minY:cy-.12,height:cy+3.55});
    }
  ...
  function buildSpawnDirectionSigns(){
    // Guias perto dos dois respawns: leitura imediata sem poluir os pontos de interesse.
    // As setas são relativas à visão de quem nasce em cada lado.
    for(const [text,x,z,rot] of [
      ['← TOCA',-11,-31,0],['↖ LABIRINTO',0,-31,0],['CHÁ →',11,-31,0],
      ['TOCA →',11,31,Math.PI],['LABIRINTO ←',0,31,Math.PI],['↘ CHÁ',-11,31,Math.PI]
