...
(() => {
  ...
  let bedroomObjects=[], kingdomObjects=[], bedroomColliders=[], outletCooldown=0, phonePhase=0, bedroomLightOff=0;
  ...
  // V56.3 — QUARTO GIGANTE: tomadas corrigidas, cama por BOING nas pontas, teto/janelas/porta.
  function buildGiantBedroom(){
    ...
    const ceiling=box(98,.30,80,mat(0xeee7da),0,27,0,false);ceiling.userData.bedroomCeiling=true;
    ...
    box(14,20,.20,mat(0xb99069),-18,10,39.72,false);box(15,.55,.35,mat(0x6f5847),-18,20.15,39.50,false);box(.55,20.5,.35,mat(0x6f5847),-25.25,10,39.50,false);box(.55,20.5,.35,mat(0x6f5847),-10.75,10,39.50,false);const knob=new THREE.Mesh(new THREE.SphereGeometry(.34,8,6),mat(0xc9a94f));knob.position.set(-13,10,39.35);scene.add(knob);
    ...
    // Escrivaninha e criado-mudo na parede direita.
    box(22,1.0,10,mat(0xb99a72),31,7.2,-22,false);for(const x of [22,40])for(const z of [-26,-18])box(1,7,1,mat(0x8f6f55),x,3.5,z,true);
    box(11,1,10,mat(0xb99a72),39,5.2,21,false);for(const x of [35,43])for(const z of [17,25])box(.9,5,.9,mat(0x8f6f55),x,2.5,z,true);
    ...
    const toyChest=(z,color,label)=>{box(16,4.4,5.2,mat(color),0,2.2,z,true);box(17,.45,5.8,mat(0xb68b61),0,4.62,z,false);makeMapLabel(label,0,5.25,z,.72);};
    ...
    bedroomOutlets.forEach(([x,z],i)=>{const side=Math.abs(x)>47;if(side){const px=x<0?-48.80:48.80,face=x<0?-48.60:48.60;const plate=box(.18,2.5,3.1,mat(0xf8f3e7),px,1.45,z,false);plate.userData.outlet=i;const dark=box(.20,.55,.78,mat(black),face,1.45,z,false);dark.userData.outlet=i;makeBolt(face,3.35,z,'x');bedroomOutlets[i][0]=face;}else{const pz=z<0?-39.80:39.80,face=z<0?-39.60:39.60;const plate=box(3.1,2.5,.18,mat(0xf8f3e7),x,1.45,pz,false);plate.userData.outlet=i;const dark=box(.78,.55,.20,mat(black),x,1.45,face,false);dark.userData.outlet=i;makeBolt(x,3.35,face,'z');bedroomOutlets[i][1]=face;}});
    ...
    const phone=box(8,.65,14,mat(0x394552),32,7.85,-22,false);phone.userData.giantPhone=true;box(6,.12,10,mat(0x9ec9c4),32,8.2,-22,false);makeMapLabel('TELEFONE',32,9,-22,.8);
  ...
  function update(dt){
    ...
    outletCooldown=Math.max(0,outletCooldown-dt);bedroomLightOff=Math.max(0,bedroomLightOff-dt);
    if(activeMap==='quarto'){
      ...
      if(outletCooldown<=0){const oi=bedroomOutlets.findIndex(([x,z])=>Math.hypot(player.pos.x-x,player.pos.z-z)<1.55);if(oi>=0){const choices=bedroomOutlets.map((_,i)=>i).filter(i=>i!==oi),to=choices[Math.floor(Math.random()*choices.length)],p=bedroomOutlets[to];spawnOutletZap(player.pos.clone());electricZapSound();outletCooldown=5;let tx=p[0],tz=p[1];if(Math.abs(tx)>47)tx+=tx<0?2.0:-2.0;else tz+=tz<0?2.0:-2.0;player.pos.set(tx,1.7,tz);verticalVelocity=0;grounded=true;setTimeout(()=>{if(activeMap==='quarto')spawnOutletZap(player.pos.clone());},35);hitNoticeEl.textContent='⚡ TRZZZT!  PUFT!';hitNoticeEl.classList.add('show');hitNoticeTimer=1.0;}}
      // Telefone vibra sutilmente; evento do humano fica deliberadamente para depois.
      phonePhase+=dt*18;for(const o of bedroomObjects)if(o.userData?.giantPhone)o.rotation.z=Math.sin(phonePhase)*.012;
    }
