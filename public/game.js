...
(() => {
  ...
  let bedroomObjects=[], kingdomObjects=[], bedroomColliders=[], outletCooldown=0, phonePhase=0, bedroomLightOff=0, bedroomLightLock=0, bedroomMatchElapsed=0, bedroomPhoneRingMask=0, duckBounceCooldown=0, bedroomReward=null, bedroomRewardRespawn=0;
  ...
  function applyMushroomState(st){const m=destructibleMushrooms.get(st?.id);if(!m)return;const active=st.active!==false&&(Number(st.hp)||0)>0;if(m.active&&!active)spawnMushroomSmoke(m);m.active=active;m.hp=active?Math.max(1,Number(st.hp)||120):0;m.group.visible=active&&activeMap==='reino';m.collider.active=active&&activeMap==='reino';m.group.traverse(o=>{if(o.isMesh)o.userData.noBulletMark=!active;});if(active)m.respawn=0;}
  ...
  function buildGiantBedroom(){
    ...
    box(1.0,5.8,34,mat(0x8f6f55),-46,2.9,0,true);
    ...
    bedroomReward=new THREE.Group();const rwBody=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(1.05,1.18,2.0,10),mat(0xd8d1bd)));rwBody.position.y=1.0;bedroomReward.add(rwBody);const rwInk=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.72,.72,.35,10),mat(0x20242a)));rwInk.position.y=2.12;bedroomReward.add(rwInk);const rwCrossV=new THREE.Mesh(new THREE.BoxGeometry(.30,1.0,.10),mat(0xd75a55));rwCrossV.position.set(0,1.05,-1.08);bedroomReward.add(rwCrossV);const rwCrossH=new THREE.Mesh(new THREE.BoxGeometry(.95,.30,.10),mat(0xd75a55));rwCrossH.position.set(0,1.05,-1.08);bedroomReward.add(rwCrossH);bedroomReward.position.set(34.5,7.7,-33);bedroomReward.traverse(o=>{if(o.isMesh)o.userData.bedroomReward=true;});scene.add(bedroomReward);makeMapLabel('TINTA DE SOCORRO',34.5,10.6,-33,.55);
    ...
    const bulb=new THREE.Mesh(new THREE.SphereGeometry(.75,10,8),new THREE.MeshBasicMaterial({color:0xffefad}));bulb.position.set(0,28.15,0);scene.add(bulb);bulb.userData.bedroomLamp=true;
  ...
  function phoneRingSound(){initAudio();if(!audioCtx)return;const now=audioCtx.currentTime;for(let k=0;k<3;k++){const t=now+k*.68;for(const f of [720,920]){const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type='sine';o.frequency.value=f;g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(.16,t+.02);g.gain.exponentialRampToValueAtTime(.0001,t+.34);o.connect(g);g.connect(audioCtx.destination);o.start(t);o.stop(t+.36);}}}
  ...
  function blocked(x,z) {
    ...
    if(activeMap==='quarto'){if(Math.abs(x)>48.5-r||Math.abs(z)>39.5-r)return true;const feetY=player.pos.y-1.7;for(const c of colliders){if(c.active===false||c.map==='reino')continue;if(c.minY!=null&&feetY<c.minY-.08)continue;if(c.height!=null&&feetY>c.height-.08)continue;if(x+r>c.minX&&x-r<c.maxX&&z+r>c.minZ&&z-r<c.maxZ)return true;}return false;}
  ...
  function localTeamSpawn(){
    ...
    const base=activeMap==='quarto'?35:36;
  ...
  function createMultiplayerLobby(){
    ...
    wrap.innerHTML=`<div style="width:min(94vw,620px);max-height:92vh;overflow:auto;padding:22px;border:4px solid #171510;border-radius:14px;background:#fcfbf7;box-shadow:9px 9px 0 #294aa3;position:relative">
      ...
      <div data-room style="display:none;margin-top:14px">
        ...
        <div data-mapbox style="margin-top:11px;padding:9px;border:3px solid #171510;border-radius:10px;background:#f5f0e3"><strong style="display:block;margin-bottom:7px">MAPA</strong><div style="display:grid;grid-template-columns:1fr 1fr;gap:7px"><button data-map="reino" style="padding:10px;border:2px solid #171510;border-radius:8px;font:900 13px Georgia">♛ PAÍS DAS MARAVILHAS</button><button data-map="quarto" style="padding:10px;border:2px solid #171510;border-radius:8px;font:900 13px Georgia">🧸 QUARTO GIGANTE</button></div></div><button data-bots style="display:none;width:100%;margin-top:11px;padding:11px;border:3px solid #171510;border-radius:10px;background:#d73535;color:#fff;font:900 15px Georgia">BOTS: DESLIGADOS</button>
  ...
  function update(dt){
    ...
    if(activeMap==='quarto'){
      ...
      bedroomMatchElapsed+=dt;let ringing=false;if(bedroomMatchElapsed>=85&&!(bedroomPhoneRingMask&1)){bedroomPhoneRingMask|=1;phoneRingSound();ringing=true;}if(bedroomMatchElapsed>=245&&!(bedroomPhoneRingMask&2)){bedroomPhoneRingMask|=2;phoneRingSound();ringing=true;}
      phonePhase+=dt*(ringing?42:18);for(const o of bedroomObjects)if(o.userData?.giantPhone)o.rotation.z=(ringing||Math.sin(bedroomMatchElapsed*5)>.995)?Math.sin(phonePhase)*.035:0;
    }
