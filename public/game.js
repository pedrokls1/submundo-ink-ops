...
(() => {
  ...
  let mouseLook = false;
  let fireHeld = false;
  let damageFlash=0,damageShake=0,rabbitTunnelCooldown=0,rabbitTunnelMode=false;
  const damageOverlay=document.createElement('div');damageOverlay.id='damageOverlay';damageOverlay.style.cssText='position:fixed;inset:0;z-index:65;pointer-events:none;opacity:0;background:radial-gradient(circle at center,transparent 43%,rgba(215,28,38,.10) 63%,rgba(190,10,22,.62) 100%);transition:opacity .04s linear';document.body.appendChild(damageOverlay);
  function triggerDamageFeedback(amount=10){const force=Math.max(.18,Math.min(1,Number(amount||10)/45));damageFlash=Math.max(damageFlash,.32+.42*force);damageShake=Math.max(damageShake,.035+.055*force);}
  ...
  const mushroomSmoke=[];
  let cheshireCat=null,cheshireHitCooldown=0;
  ...
  function detonateRocket(position,impactNormal=null){
    ...
    for(const [token,remote] of multiplayer.remotePlayers){const d=remote.group.position.distanceTo(position);if(d<=4.8)reportRemoteHit(token,d<=3.2?100:48,false);}for(const m of destructibleMushrooms.values())if(m.active&&Math.hypot(m.x-position.x,m.z-position.z)<=5.2)damageMushroom(m.id,100);
  ...
  function getPlayerHeight(x,z){
    if(window.__inkRabbitTunnelActive&&window.__inkRabbitTunnelInfo){const ri=window.__inkRabbitTunnelInfo(x,z);if(ri.d<2.15){const edge=Math.min(ri.t,1-ri.t);return edge<.085?1.7-(edge/.085)*3.30:-1.60;}}if(Math.abs(x)<2.6||Math.abs(x-30)<2.6||Math.abs(x+30)<2.6){const az=Math.abs(z);if(az<=3.5)return 2.20;if(az<5.9)return 1.70+(5.9-az)/2.4*.50;}return 1.7;}
  ...
  function makeWorldSign(text,x,z,rot=0){const post=cyl(.10,2.0,mat(black),x,1,z,7,false);const board=box(3.8,1.15,.18,mat(wallLight),x,2.05,z,false);board.rotation.y=rot;makeMapLabel(text,x,2.08,z-.12,1.25);return {post,board};}
  function makeCardProp(rank,suit,x,z,rot=0,scale=1){const redSuit=suit==='♥'||suit==='♦',card=box(2.5*scale,4.0*scale,.22*scale,mat(wall),x,2.0*scale,z,true);card.rotation.y=rot;const c=document.createElement('canvas');c.width=256;c.height=384;const ctx=c.getContext('2d');ctx.fillStyle='#fffdf7';ctx.fillRect(0,0,256,384);ctx.strokeStyle='#171510';ctx.lineWidth=12;ctx.strokeRect(6,6,244,372);ctx.fillStyle=redSuit?'#d73535':'#171510';ctx.font='900 64px Georgia';ctx.textAlign='left';ctx.fillText(rank,22,72);ctx.font='900 78px Georgia';ctx.fillText(suit,22,145);ctx.textAlign='center';ctx.font='900 128px Georgia';ctx.fillText(suit,128,260);const tx=new THREE.CanvasTexture(c);tx.colorSpace=THREE.SRGBColorSpace;const face=new THREE.Mesh(new THREE.PlaneGeometry(2.18*scale,3.55*scale),new THREE.MeshBasicMaterial({map:tx,transparent:true}));face.position.set(x,2.0*scale,z-.12*scale);face.rotation.y=rot;scene.add(face);return card;}
  function makeTreeProp(x,z,s=1){const trunk=cyl(.40*s,2.5*s,mat(0x34302a),x,1.25*s,z,8,true);for(const [dx,dy,dz,k] of [[0,3.1,0,1.5],[-.8,2.8,.25,1],[.85,2.75,-.2,1.05]]){const crown=addOutline(new THREE.Mesh(new THREE.SphereGeometry(k*s,7,5),mat(0x4a7f68)));crown.position.set(x+dx*s,dy*s,z+dz*s);scene.add(crown);}return trunk;}
  function spawnMushroomSmoke(m){for(let i=0;i<18;i++){const puff=new THREE.Mesh(new THREE.SphereGeometry(.24+Math.random()*.28,6,5),new THREE.MeshBasicMaterial({color:i%3===0?0x86a88d:0xe9e6dc,transparent:true,opacity:.55,depthWrite:false}));const a=Math.random()*TAU,r=Math.random()*1.4;puff.position.set(m.x+Math.cos(a)*r,1+Math.random()*2,m.z+Math.sin(a)*r);scene.add(puff);mushroomSmoke.push({mesh:puff,vx:(Math.random()-.5)*.55,vy:.45+Math.random()*.65,vz:(Math.random()-.5)*.55,life:2.3+Math.random()*.8,max:3});}}
  function makeDestructibleMushroom(id,x,z,s=1){const group=new THREE.Group();group.position.set(x,0,z);const stem=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.65*s,.85*s,2.6*s,8),mat(wallLight)));stem.position.y=1.3*s;group.add(stem);const cap=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.75*s,10,6),mat(0x4c986f)));cap.scale.y=.48;cap.position.y=2.75*s;group.add(cap);for(let i=0;i<7;i++){const spot=new THREE.Mesh(new THREE.CircleGeometry(.16*s,8),new THREE.MeshBasicMaterial({color:i%2?red:blue,side:THREE.DoubleSide}));const a=i/7*TAU;spot.position.set(Math.cos(a)*1.2*s,3.15*s,Math.sin(a)*1.2*s);spot.rotation.x=-Math.PI/2;group.add(spot);}group.traverse(o=>{if(o.isMesh)o.userData.mushroomId=id;});scene.add(group);const collider={minX:x-1.15*s,maxX:x+1.15*s,minZ:z-1.15*s,maxZ:z+1.15*s,height:3.4*s,active:true};colliders.push(collider);const rec={id,x,z,group,collider,hp:120,active:true,respawn:0};destructibleMushrooms.set(id,rec);return rec;}
  function applyMushroomState(st){const m=destructibleMushrooms.get(st?.id);if(!m)return;const active=st.active!==false&&(Number(st.hp)||0)>0;if(m.active&&!active)spawnMushroomSmoke(m);m.active=active;m.hp=active?Math.max(1,Number(st.hp)||120):0;m.group.visible=active;m.collider.active=active;if(active)m.respawn=0;}
  ...
  window.__inkRealtimeProp=st=>applyMushroomState(st);
  function buildCombatMaze(){const cols=9,rows=8,cell=3.45,ox=-42.0,oz=10.8,H=3.75,T=.42;const visited=Array.from({length:rows},()=>Array(cols).fill(false)),walls=Array.from({length:rows},()=>Array.from({length:cols},()=>({n:true,e:true,s:true,w:true})));let seed=137;const rnd=()=>((seed=(seed*1664525+1013904223)>>>0)/4294967296),stack=[[0,0]];visited[0][0]=true;while(stack.length){const [cx,cz]=stack[stack.length-1],opts=[];if(cz>0&&!visited[cz-1][cx])opts.push([cx,cz-1,'n','s']);if(cx<cols-1&&!visited[cz][cx+1])opts.push([cx+1,cz,'e','w']);if(cz<rows-1&&!visited[cz+1][cx])opts.push([cx,cz+1,'s','n']);if(cx>0&&!visited[cz][cx-1])opts.push([cx-1,cz,'w','e']);if(!opts.length){stack.pop();continue;}const pick=opts[Math.floor(rnd()*opts.length)],[nx,nz,a,b]=pick;walls[cz][cx][a]=false;walls[nz][nx][b]=false;visited[nz][nx]=true;stack.push([nx,nz]);}for(let z=1;z<rows-1;z++)for(let x=1;x<cols-1;x++)if(rnd()<.16){const dirs=[['e','w',1,0],['s','n',0,1]],d=dirs[Math.floor(rnd()*dirs.length)],nx=x+d[2],nz=z+d[3];walls[z][x][d[0]]=false;walls[nz][nx][d[1]]=false;}walls[rows-1][4].s=false;walls[0][0].n=false;const hedgeMat=mat(0xf4f6f1),wallSeg=(x,z,w,d)=>{const m=box(w,H,d,hedgeMat,x,H/2,z,true);addHatching(m,Math.max(.5,w*.65),2.7,Math.max(.08,d*.32),7,true);return m;};for(let z=0;z<rows;z++)for(let x=0;x<cols;x++){const cx=ox+x*cell+cell/2,cz=oz+z*cell+cell/2;if(walls[z][x].n)wallSeg(cx,oz+z*cell,cell+T,T);if(walls[z][x].w)wallSeg(ox+x*cell,cz,T,cell+T);if(x===cols-1&&walls[z][x].e)wallSeg(ox+cols*cell,cz,T,cell+T);if(z===rows-1&&walls[z][x].s)wallSeg(cx,oz+rows*cell,cell+T,T);}makeWorldSign('LABIRINTO',-27,39.2,0);}
  ...
  function buildTeaParty(){const tx=27,tz=-25;const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(7.0,7.0,.70,16),mat(wallLight)));table.position.set(tx,2.2,tz);scene.add(table);cyl(1.0,4.4,mat(wood),tx,2.0,tz,10,true);for(let i=0;i<8;i++){const a=i/8*TAU;const chair=box(2.2,4.0,2.0,mat(i%2?red:blue),tx+Math.cos(a)*10,2.0,tz+Math.sin(a)*9.2,true);chair.rotation.y=-a;}const pot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(2.0,10,7),mat(wall)));pot.position.set(tx,3.35,tz);scene.add(pot);cyl(.75,.35,mat(red),tx,5.0,tz,10,false);for(const [dx,dz] of [[-3.2,2.1],[3.0,-2.0],[1.8,3.7],[-1.2,-3.6]]){cyl(.85,1.3,mat(wall),tx+dx,3.0,tz+dz,10,false);const tea=cyl(.65,.05,mat(red),tx+dx,3.68,tz+dz,10,false);tea.userData.noBulletMark=true;}makeWorldSign('CHÁ MALUCO',27,-37.5,0);}
  function buildRabbitTunnel(){const pts=[[-40,-31],[-35,-24],[-27,-20],[-18,-23],[-10,-30],[0,-31],[9,-25],[15,-14],[22,-5],[29,7],[35,19],[40,31]];window.__inkRabbitPath=pts;window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0},total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz);if(d<best.d)best={d,t:(acc+lens[i]*u)/total};acc+=lens[i];}return best;};window.__inkRabbitTunnelY=t=>{const depth=Math.max(0,Math.min(1,Math.min(t/.085,(1-t)/.085)));return 1.7-3.0*depth;};function entrance(x,z,rot,label){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const arch=addOutline(new THREE.Mesh(new THREE.TorusGeometry(2.15,.42,8,24,Math.PI),mat(wall)));arch.rotation.z=Math.PI;arch.position.y=1.75;gg.add(arch);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.78,20),new THREE.MeshBasicMaterial({color:black,side:THREE.DoubleSide}));dark.position.set(0,1.55,.10);gg.add(dark);const earL=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.26,1.0,4,6),mat(wallLight)));earL.position.set(-.55,3.2,0);earL.rotation.z=-.18;gg.add(earL);const earR=earL.clone();earR.position.x=.55;earR.rotation.z=.18;gg.add(earR);scene.add(gg);makeWorldSign(label,x+(rot===0?0:-2.8),z+(rot===0?-3:0),rot);return {x,z};}window.__inkRabbitEntrances=[entrance(-40,-31,0,'TOCA DO COELHO'),entrance(40,31,Math.PI,'TOCA DO COELHO')];let total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,tm=(t0+t1)/2,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(y1-y0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floor=box(4.6,.18,len+.45,mat(wallLight),mx,ym+.02,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;const depth=Math.max(0,Math.min(1,Math.min(tm/.085,(1-tm)/.085)));for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.3,wz=mz-Math.sin(yaw)*side*2.3;const wallSeg=box(.24,3.1,len+.45,mat(wall),wx,ym+1.55,wz,false);wallSeg.rotation.y=yaw;}if(depth>.55){const roof=box(4.85,.25,len+.55,mat(0xe8e4da),mx,ym+3.05,mz,false);roof.rotation.y=yaw;roof.userData.rabbitRoof=true;}acc+=len;}}
  function buildCheshireCat(){const g=new THREE.Group();const body=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.05,9,7),mat(black)));body.scale.set(1.5,.75,1);g.add(body);const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.78,9,7),mat(0x2c3140)));head.position.set(0,1.05,0);g.add(head);for(const sx of [-1,1]){const eye=new THREE.Mesh(new THREE.SphereGeometry(.12,6,5),new THREE.MeshBasicMaterial({color:blue}));eye.position.set(sx*.27,1.18,-.69);g.add(eye);}const smile=new THREE.Mesh(new THREE.TorusGeometry(.34,.055,6,18,Math.PI),new THREE.MeshBasicMaterial({color:red}));smile.position.set(0,.90,-.70);smile.rotation.z=Math.PI;g.add(smile);g.position.set(-27,1.4,24);scene.add(g);cheshireCat=g;}
  function updateCheshireCat(dt){if(!cheshireCat)return;const t=performance.now()/1000;cheshireCat.position.set(-27+Math.sin(t*.42)*8.5,1.5+Math.sin(t*2.1)*.25,24+Math.sin(t*.29+.8)*9.0);const fade=.38+.62*Math.abs(Math.sin(t*.75));cheshireCat.traverse(o=>{if(o.material&&'opacity' in o.material){o.material.transparent=true;o.material.opacity=fade;}});cheshireHitCooldown=Math.max(0,cheshireHitCooldown-dt);if(player.pos.y>0&&cheshireHitCooldown<=0&&player.pos.distanceTo(cheshireCat.position)<2.25){cheshireHitCooldown=1.55;triggerDamageFeedback(18);hitNoticeEl.textContent='GATO DE CHESHIRE • ARRANHÃO DE TINTA!';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;if(multiplayer.room?.status==='playing')fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'hazardhit',token:multiplayer.token,roomCode:multiplayer.roomCode})}).catch(()=>{});else{let d=18;if(armor>0){const a=Math.min(armor,9);armor-=a;d-=a;}hp=Math.max(0,hp-d);updateHud();}}}
  function buildSeasonalReinoDasCopas(){buildCombatMaze();buildQueenCourt();buildTeaParty();buildRabbitTunnel();buildCheshireCat();const cards=[['A','♥',-6,-34,.12,1.15],['7','♠',12,31,-.2,1.0],['Q','♦',31,14,.18,1.05],['3','♣',-7,27,-.12,.9],['K','♥',43,-10,.12,1.0],['9','♦',-44,5,-.08,.95]];for(const c of cards)makeCardProp(...c);for(const [x,z,s] of [[-45,34,1.05],[45,35,1.0],[-9,-36,.85],[9,35,.8],[44,-36,.9],[-46,-35,.85]])makeTreeProp(x,z,s);for(const [id,x,z,s] of [['m1',-34,-27,1.2],['m2',-19,-34,1.05],['m3',18,-34,1.15],['m4',37,-21,1.15],['m5',33,22,1.0],['m6',-8,34,.95],['m7',13,23,.9],['m8',-17,8,.9]])makeDestructibleMushroom(id,x,z,s);for(const [x,z] of [[-47,18],[46,12],[-7,-20],[12,-12]]){for(let j=0;j<3;j++){const book=box(5.2-j*.45,.55,2.8,mat(j%2?woodLight:(j===2?red:wall)),x,.3+j*.55,z+j*.25,true);book.rotation.y=(j-1)*.08;}}for(const x of [-49,49])box(.18,.05,82,mat(ink),x,.03,0,false);box(98,.05,.18,mat(ink),0,.03,41,false);box(98,.05,.18,mat(ink),0,.03,-41,false);for(const [x,z,color] of [[0,36,blue],[0,-36,red]])box(8,.10,.8,mat(color),x,.08,z,false);}

  function buildWorld() {buildGround();buildRiver();buildBridge(-32);buildBridge(0);buildBridge(32);buildSeasonalReinoDasCopas();buildTeamSpawnGuides();}
  ...
  const movableControls=[stick,fireBtn,fireLeftBtn,weaponSwitchBtn,reloadBtn,scopeBtn,jumpBtn,fullscreenBtn].filter(Boolean);
  ...
  function blocked(x,z) {
    ...
    if(rabbitTunnelMode&&window.__inkRabbitTunnelInfo){const ri=window.__inkRabbitTunnelInfo(x,z);return ri.d>1.92;}
  ...
  function reloadWeapon(){
    ...
    reloadBtn.textContent='↻...';
    if(!reloadCartridge){reloadCartridge=new THREE.Group();const pot=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.20,.24,.36,10),mat(0xf7f4eb)));pot.rotation.z=Math.PI/2;reloadCartridge.add(pot);const paint=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.205,.245,.15,10),mat(red)));paint.rotation.z=Math.PI/2;paint.position.x=-.09;reloadCartridge.add(paint);const lid=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.22,.22,.08,10),mat(black)));lid.rotation.z=Math.PI/2;lid.position.x=.22;reloadCartridge.add(lid);for(const yy of [-.07,.05]){const drip=new THREE.Mesh(new THREE.SphereGeometry(.045,6,5),mat(red));drip.position.set(-.08,yy,-.22);reloadCartridge.add(drip);}reloadCartridge.scale.setScalar(.72);reloadCartridge.position.set(.16,-.38,-.76);camera.add(reloadCartridge);}reloadCartridge.visible=true;reloadCartridge.userData.reloadLife=reloadTimer;reloadCartridge.userData.reloadTotal=reloadTimer;
    setTimeout(()=>{
  ...
  function shoot(){
    ...
    if(hit){
      ...
      if(remoteToken){
        const headshot=hit.object.userData?.hitZone==='head';
        const damage=isSniper?(headshot?100:60):(weaponMode==='smg'?(headshot?20:18):(headshot?34:26));
  ...
  function makeRemotePlayer(state){
    ...
    const legs=[-.22,.22].map(x=>{const leg=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.20,.80,.22),darkMat));leg.position.set(x,.42,0);group.add(leg);return leg;});
    const body=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.76,.96,.42),bodyMat));body.position.y=1.18;group.add(body);
    const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.31,8,6),paperMat));head.position.y=1.95;group.add(head);
    const visor=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.44,.12,.08),darkMat));visor.position.set(0,1.98,-.29);group.add(visor);
    const arms=[];
    for(const side of [-1,1]){const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.095,.50,3,5),paperMat));arm.position.set(side*.47,1.27,-.08);arm.rotation.z=side*.16;arm.rotation.x=-.72;group.add(arm);arms.push(arm);const hand=new THREE.Mesh(new THREE.SphereGeometry(.105,6,5),paperMat);hand.position.set(side*.39,1.12,-.46);group.add(hand);}
    const gunGroup=new THREE.Group();
    const gun=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.20,.18,1.05),darkMat));gun.position.set(.18,1.30,-.64);gun.rotation.x=-.04;gunGroup.add(gun);
    const barrel=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.035,.045,.62,7),darkMat));barrel.rotation.x=Math.PI/2;barrel.position.set(.18,1.30,-1.45);gunGroup.add(barrel);
    const stock=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.22,.24,.38),paperMat));stock.position.set(.18,1.29,-.02);gunGroup.add(stock);
    const mag=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.14,.34,.22),mat(teamColor)));mag.position.set(.18,1.08,-.72);mag.rotation.x=.15;gunGroup.add(mag);
    group.add(gunGroup);
    ...
    return {group,legs,arms,gun:gunGroup,target:initial.clone(),predicted:initial.clone(),lastNetPos:initial.clone(),velocity:new THREE.Vector3(),lastPacketAt:performance.now(),targetYaw:state.yaw+Math.PI,step:0,weapon:state.weapon,team:state.team,puddle:null,alive:state.alive!==false};
  }
  ...
  window.__inkRealtimeSnapshot = payload => {
    ...
    applyRemoteStates(payload.players || []);if(Array.isArray(payload.props))for(const st of payload.props)applyMushroomState(st);if(payload.match){const sec=Math.max(0,Math.ceil(Number(payload.match.remainingMs||0)/1000)),mm=String(Math.floor(sec/60)),ss=String(sec%60).padStart(2,'0');matchClockEl.style.display='block';matchClockEl.textContent=mm+':'+ss+'  •  40 KILLS';if(payload.match.status==='finished')window.__inkRealtimeMatchEnd?.(payload.match);}
    if(payload.self){const oldHp=hp,oldArmor=armor,serverHp=Math.max(0,Math.min(100,Number(payload.self.health??hp))),serverArmor=Math.max(0,Math.min(100,Number(payload.self.armor??armor)));const wasAlive=multiplayer.selfAlive;multiplayer.selfAlive=payload.self.alive;if(serverHp<oldHp||serverArmor<oldArmor)triggerDamageFeedback((oldHp-serverHp)+(oldArmor-serverArmor)*.45);if(!payload.self.alive){hp=0;armor=serverArmor;createLocalDeathPuddle();showDeathScreen(payload.self.respawnSeconds);fireHeld=false;scopeMode=false;}else if(!wasAlive){hp=100;armor=100;reserveAmmo={...RESERVE_AMMO};ammo=weaponMode==='knife'?0:(MAGAZINES[weaponMode]||0);clearLocalDeathPuddle();hideDeathScreen();const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;player.pos.set(0,1.7,myTeam==='vermelho'?-36:36);player.yaw=myTeam==='vermelho'?0:Math.PI;updateHud();}else{hp=serverHp;armor=serverArmor;hideDeathScreen();updateHud();}}
    if(payload.scores){
      onlineScoreEl.style.display='block';
      onlineScoreEl.innerHTML='<span style="color:#294aa3">AZUL '+Number(payload.scores.azul||0)+'/40</span> &nbsp;×&nbsp; <span style="color:#d73535">'+Number(payload.scores.vermelho||0)+'/40 VERMELHO</span>';
    }
  };
  ...
  const killFeed=document.createElement('div');killFeed.style.cssText='position:fixed;right:12px;top:54px;z-index:46;display:flex;flex-direction:column;gap:5px;align-items:flex-end;pointer-events:none;font:800 12px Georgia';document.body.appendChild(killFeed);window.__inkRealtimeKill=ev=>{if(!ev)return;const row=document.createElement('div');row.style.cssText='max-width:48vw;padding:5px 8px;border:2px solid #171510;border-radius:7px;background:rgba(252,251,247,.88)';row.innerHTML=ev.self?'<b>'+ev.victim+'</b> virou a própria obra de arte 💥':'<b>'+ev.killer+'</b> ✎ <span style="color:#d73535">'+ev.victim+'</span>';killFeed.prepend(row);while(killFeed.children.length>4)killFeed.lastChild.remove();setTimeout(()=>row.remove(),4200);};let selfRocketDeathPending=false;
  ...
  function startGame(){
    initAudio(); score=0;kills=0;headshots=0;ammo=MAGAZINES.rifle;reserveAmmo={rifle:120,smg:144,shotgun:30,launcher:6,sniper:20};hp=100;armor=100;clearLocalDeathPuddle();hideDeathScreen();wave=1;waveDelay=0;reloadTimer=0;fireCooldown=0;fireHeld=false;weaponKick=0;fireZoom=0;sprintBlend=0;knifeSwing=0;sniperCycle=0;playerStepTimer=0;
    setWeaponMode('rifle');
    ...
    player.pos.set(0,1.7,myTeam==='vermelho'?-36:36);player.yaw=myTeam==='vermelho'?0:Math.PI;player.pitch=0;player.recoil=0;verticalVelocity=0;grounded=true;jumpPadCooldown=0;
    ...
    updateHud(); startEl.classList.add('hidden');overEl.classList.add('hidden');layoutBtn.style.display='block';voiceBtn.style.display=multiplayer.room?.status==='playing'?'block':'none';running=true;last=performance.now();clock.start();requestAnimationFrame(loop);
  ...
  function update(dt){
    ...
    if(!rabbitTunnelMode&&rabbitTunnelCooldown<=0&&window.__inkRabbitEntrances?.length===2){for(const ent of window.__inkRabbitEntrances){if(Math.hypot(player.pos.x-ent.x,player.pos.z-ent.z)<1.9){rabbitTunnelMode=true;rabbitTunnelCooldown=1.0;grounded=true;verticalVelocity=0;hitNoticeEl.textContent='TOCA DO COELHO • DESCENDO PELO TÚNEL';hitNoticeEl.classList.add('show');hitNoticeTimer=.9;break;}}}
    ...
    if(rabbitTunnelMode&&window.__inkRabbitTunnelInfo){const info=window.__inkRabbitTunnelInfo(player.pos.x,player.pos.z);player.pos.y=window.__inkRabbitTunnelY?window.__inkRabbitTunnelY(info.t):-1.3;grounded=true;verticalVelocity=0;if(rabbitTunnelCooldown<=0&&(info.t<.012||info.t>.988)){rabbitTunnelMode=false;rabbitTunnelCooldown=1.2;player.pos.y=1.7;hitNoticeEl.textContent='VOCÊ SAIU DA TOCA DO COELHO';hitNoticeEl.classList.add('show');hitNoticeTimer=.75;}}else{const terrainHeight=getPlayerHeight(player.pos.x,player.pos.z);if(grounded && terrainHeight<player.pos.y-.30) grounded=false;if(grounded){player.pos.y += (terrainHeight-player.pos.y)*Math.min(1,dt*14);}else{verticalVelocity-=13.5*dt;player.pos.y+=verticalVelocity*dt;if(player.pos.y<=terrainHeight){player.pos.y=terrainHeight;verticalVelocity=0;grounded=true;}}}
    ...
    if(fireHeld) shoot();
    updateCheshireCat(dt);
    for(const m of destructibleMushrooms.values())if(!multiplayer.room?.status&&m.respawn>0){m.respawn-=dt;if(m.respawn<=0){m.hp=120;applyMushroomState({id:m.id,hp:120,active:true});}}
    for(let i=mushroomSmoke.length-1;i>=0;i--){const s=mushroomSmoke[i];s.life-=dt;s.mesh.position.x+=s.vx*dt;s.mesh.position.y+=s.vy*dt;s.mesh.position.z+=s.vz*dt;s.mesh.scale.multiplyScalar(1+dt*.25);s.mesh.material.opacity=Math.max(0,.55*s.life/s.max);if(s.life<=0){scene.remove(s.mesh);s.mesh.geometry.dispose();s.mesh.material.dispose();mushroomSmoke.splice(i,1);}}
    ...
    damageFlash=Math.max(0,damageFlash-dt*1.75);damageShake=Math.max(0,damageShake-dt*.42);damageOverlay.style.opacity=String(Math.min(.72,damageFlash));
