...
(() => {
  ...
  function detonateRocket(position,impactNormal=null){
    ...
    for(const [token,remote] of multiplayer.remotePlayers){const d=remote.group.position.distanceTo(position);if(d<=4.8)reportRemoteHit(token,d<=3.2?100:48,false);}
  ...
  function buildSeasonalReinoDasCopas(){
    const heartPts=[];for(let i=0;i<36;i++){const t=i/36*TAU;const x=8.6*Math.sin(t)**3;const z=-(7.0*Math.cos(t)-2.8*Math.cos(2*t)-1.25*Math.cos(3*t)-.55*Math.cos(4*t));heartPts.push(new THREE.Vector3(x*.82,.10,z*.82));}heartPts.push(heartPts[0].clone());scene.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(heartPts),new THREE.LineBasicMaterial({color:red,transparent:true,opacity:.94})));
    const throne=box(3.6,4.6,1.0,mat(wall),0,2.3,7.8,true);addHatching(throne,2.8,3.5,.52,8,true);const crown=new THREE.Mesh(new THREE.ConeGeometry(1.15,1.25,5),mat(red));crown.position.set(0,5.2,7.8);crown.rotation.y=.3;addOutline(crown);scene.add(crown);
    for(const [x,z] of [[-9,7],[9,7],[-9,-7],[9,-7],[-5,0],[5,0]]){const q=box(3.2,1.25,.62,mat(woodLight),x,.63,z,true);addHatching(q,2.45,.78,.32,4,false);box(.45,.50,.10,mat(red),x,1.05,z-.33,false);}for(const [x,z] of [[-11,3],[11,3],[-11,-3],[11,-3]]){box(1.15,3.0,.24,mat(wall),x,1.5,z,true);box(.38,.38,.25,mat(red),x,1.65,z-.12,false);}
    const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(5.6,5.6,.55,14),mat(wallLight)));table.position.set(25,1.25,20);scene.add(table);cyl(.85,2.5,mat(wood),25,1.25,20,10,true);for(let i=0;i<7;i++){const ang=i/7*TAU;const chair=box(1.7,2.2,1.7,mat(i%2?woodLight:wall),25+Math.cos(ang)*8.2,1.1,20+Math.sin(ang)*8.2,true);chair.rotation.y=-ang;}const teaPot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.45,9,7),mat(wall)));teaPot.position.set(25,2.15,20);scene.add(teaPot);cyl(.55,.3,mat(red),25,3.35,20,10,false);for(const [dx,dz] of [[-2.8,1.7],[2.6,-1.8],[1.4,3.0]]){const cup=cyl(.62,1.0,mat(wall),25+dx,1.75,20+dz,10,true);addHatching(cup,.7,.7,.3,3,false);}
    const hedgeMat=mat(0xf7f7f2),hedge=(x,z,w,d)=>{const hh=3.35,m=box(w,hh,d,hedgeMat,x,hh/2,z,true);addHatching(m,Math.max(.5,w*.72),2.45,Math.max(.08,d*.35),8,true);return m;};hedge(-30,-11,22,.9);hedge(-18,-18,.9,15);hedge(-35,-20,.9,17);hedge(-27,-28,17,.9);hedge(-12,-27,11,.9);hedge(-40,-30,10,.9);hedge(-27,-19,9,.85);hedge(-23,-24,.85,6);hedge(-31,-24,7,.85);hedge(-13,-15,9,.85);hedge(-40,-14,8,.85);hedge(-39,-24,7,.85);
    window.__inkRabbitPath=[[-40,-1],[-34,7],[-26,11],[-16,10],[-7,5],[2,-3],[11,-9],[21,-12],[31,-8],[40,-1]];window.__inkRabbitTunnelActive=false;window.__inkRabbitTunnelInfo=function(x,z){const pts=window.__inkRabbitPath;let best={d:1e9,t:0},total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz);if(d<best.d)best={d,t:(acc+lens[i]*u)/total};acc+=lens[i];}return best;};
    function rabbitHole(x,z,rot,team){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const arch=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.8,.38,8,22,Math.PI),mat(wall)));arch.rotation.z=Math.PI;arch.position.y=1.55;gg.add(arch);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.48,18),new THREE.MeshBasicMaterial({color:0x171510,side:THREE.DoubleSide}));dark.position.set(0,1.38,.08);gg.add(dark);scene.add(gg);return {x,z,team};}window.__inkRabbitEntrances=[rabbitHole(-40,-1,-Math.PI/2,'azul'),rabbitHole(40,-1,Math.PI/2,'vermelho')];
    const pts=window.__inkRabbitPath;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],mx=(ax+bx)/2,mz=(az+bz)/2,len=Math.hypot(bx-ax,bz-az),yaw=Math.atan2(bx-ax,bz-az);const floor=box(4.6,.18,len+.35,mat(wallLight),mx,-3.38,mz,false);floor.rotation.y=yaw;const roof=box(4.8,.22,len+.45,mat(0xf0ede5),mx,-.15,mz,false);roof.rotation.y=yaw;roof.userData.rabbitRoof=true;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.3,wz=mz-Math.sin(yaw)*side*2.3;const wallSeg=box(.24,3.2,len+.4,mat(wall),wx,-1.75,wz,false);wallSeg.rotation.y=yaw;}}for(let i=1;i<pts.length-1;i++){const [x,z]=pts[i];cyl(.28,3.2,mat(ink),x,-1.75,z,7,false);}
    for(const [x,z,scale,color] of [[16,30,1.8,red],[-12,27,1.55,blue],[34,4,1.5,red],[-8,-35,1.7,blue]]){cyl(.45*scale,1.8*scale,mat(wall),x,.9*scale,z,8,true);const cap=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.25*scale,8,5),mat(color)));cap.scale.y=.45;cap.position.set(x,2.0*scale,z);scene.add(cap);}for(const [x,z,rot,color] of [[-25,28,.18,red],[10,29,-.15,blue],[35,28,.22,red],[31,-28,-.18,blue],[-5,-34,.10,red]]){const card=box(2.6,4.2,.22,mat(wall),x,2.1,z,true);card.rotation.y=rot;addHatching(card,1.9,3.2,.12,6,false);box(.55,.55,.25,mat(color),x,2.2,z-.13,false);}for(const [x,z] of [[-8,20],[10,-21],[35,-18]]){const watch=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(1.7,1.7,.25,18),mat(wallLight)));watch.rotation.x=Math.PI/2;watch.position.set(x,1.9,z);scene.add(watch);}for(const [x,z] of [[-43,35],[42,35],[-43,-36],[42,-36]])for(let j=0;j<3;j++){const book=box(5.0-j*.45,.55,2.8,mat(j%2?woodLight:wall),x,.3+j*.55,z+j*.25,true);book.rotation.y=(j-1)*.08;}
    for(const x of [-44,44])box(.18,.05,82,mat(ink),x,.03,0,false);box(88,.05,.18,mat(ink),0,.03,41,false);box(88,.05,.18,mat(ink),0,.03,-41,false);for(const [x,z,color] of [[0,33,blue],[0,-33,red]])box(7,.10,.8,mat(color),x,.08,z,false);
  }

  function buildWorld() {buildGround();buildRiver();buildBridge(-30);buildBridge(0);buildBridge(30);buildSeasonalReinoDasCopas();buildTeamSpawnGuides();}
  ...
  function blocked(x,z) {
    ...
    if(window.__inkRabbitTunnelActive&&window.__inkRabbitTunnelInfo){const ri=window.__inkRabbitTunnelInfo(x,z);return ri.d>2.02;}
    if(rabbitTunnelMode)return Math.abs(x)>1.82||Math.abs(z)>28.4;
  ...
  function reloadWeapon(){
    ...
    reloadBtn.textContent='↻...';
    if(!reloadCartridge){reloadCartridge=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.22,.48,.16),mat(0xf7f4eb)));reloadCartridge.position.set(-.20,-.18,-.68);const band=new THREE.Mesh(new THREE.BoxGeometry(.24,.10,.18),mat(blue));band.position.y=.06;reloadCartridge.add(band);camera.add(reloadCartridge);}reloadCartridge.visible=true;reloadCartridge.userData.reloadLife=reloadTimer;
    setTimeout(()=>{
  ...
  function shoot(){
    ...
    if(weaponMode==='knife'){
      ...
      if(hit && hit.distance<=2.55){
        ...
        if(remoteToken){const knifeHead=hit.object.userData?.hitZone==='head';reportRemoteHit(remoteToken,knifeHead?55:45,knifeHead);hitSound(false);hitNoticeEl.textContent='CORTE DE TINTA!';hitNoticeEl.classList.add('show');hitNoticeTimer=.62;updateHud();return;}
    ...
    if(weaponMode==='shotgun'){
      if(worldHits[0]){const shotgunMark=addImpactMark(worldHits[0]);shotgunMark.scale.setScalar(1.8);}
  ...
  function applyRemoteStates(states){
    ...
    for(const state of states||[]){
      ...
      remote.alive=alive;remote.group.visible=alive;
    }
  ...
  function update(dt){
    ...
    if(window.__inkRabbitTunnelInfo&&window.__inkRabbitEntrances?.length===2){const li=Math.hypot(player.pos.x+40,player.pos.z+1),ri=Math.hypot(player.pos.x-40,player.pos.z+1);if(!window.__inkRabbitTunnelActive&&(li<2.1||ri<2.1))window.__inkRabbitTunnelActive=true;if(window.__inkRabbitTunnelActive&&player.pos.y>1.45&&(li<1.7||ri<1.7)){const info=window.__inkRabbitTunnelInfo(player.pos.x,player.pos.z);if(info.t<.025||info.t>.975)window.__inkRabbitTunnelActive=false;}}rabbitTunnelCooldown=Math.max(0,rabbitTunnelCooldown-dt);if(!rabbitTunnelMode&&rabbitTunnelCooldown<=0&&window.__inkRabbitEntrances?.length===2){const [top,bottom]=window.__inkRabbitEntrances;if(Math.hypot(player.pos.x-top.x,player.pos.z-top.z)<1.7){rabbitTunnelMode=true;player.pos.set(0,-1.30,26.2);grounded=true;verticalVelocity=0;hitNoticeEl.textContent='TOCA DO COELHO • PASSAGEM SUBTERRÂNEA';hitNoticeEl.classList.add('show');hitNoticeTimer=.9;}else if(Math.hypot(player.pos.x-bottom.x,player.pos.z-bottom.z)<1.7){rabbitTunnelMode=true;player.pos.set(0,-1.30,-26.2);grounded=true;verticalVelocity=0;hitNoticeEl.textContent='TOCA DO COELHO • PASSAGEM SUBTERRÂNEA';hitNoticeEl.classList.add('show');hitNoticeTimer=.9;}}
    ...
    if(reloadCartridge?.visible){reloadCartridge.userData.reloadLife=Math.max(0,(reloadCartridge.userData.reloadLife||0)-dt);const p=1-Math.min(1,reloadCartridge.userData.reloadLife/Math.max(.01,reloadTimer||.95));reloadCartridge.rotation.z=p*TAU*1.4;reloadCartridge.position.y=-.18+Math.sin(p*Math.PI)*.24;}
