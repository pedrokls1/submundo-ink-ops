...
(() => {
  ...
  function updateRemotePlayers(dt){
    for(const remote of multiplayer.remotePlayers.values()){
      const packetAge=Math.min(.04,(performance.now()-remote.lastPacketAt)/1000);
  ...
  function applyRemoteStates(states){
    ...
    for(const state of states||[]){
      ...
      if(!alive&&remote.alive){remote.puddle=createInkPuddle(nextPos,state.team||remote.team);}
  ...
  function createLocalDeathPuddle(){if(localDeathPuddle)return;localDeathPuddle=createInkPuddle(player.pos.clone(),multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team||'azul');}
  ...
  async function pushMultiplayerState(){
    ...
    try{
      ...
      if(response.ok&&data.self){
        ...
        else if(!wasAlive){hp=100;armor=100;reserveAmmo={...RESERVE_AMMO};ammo=weaponMode==='knife'?0:(MAGAZINES[weaponMode]||0);clearLocalDeathPuddle();hideDeathScreen();const myTeam=multiplayer.room?.players?.find(p=>p.token===multiplayer.token)?.team;player.pos.set(0,1.7,myTeam==='vermelho'?-30:30);player.yaw=myTeam==='vermelho'?0:Math.PI;hitNoticeEl.textContent='DE VOLTA AO RABISCO! • VIDA, COLETE E MUNIÇÃO CHEIOS';hitNoticeEl.classList.add('show');hitNoticeTimer=1.25;}
