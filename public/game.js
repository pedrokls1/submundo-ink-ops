...
(() => {
  ...
  function buildGround() {
    ...
    const holes=[[-45,-38,-39,-11.5],[19.5,30.5,45,37.5]];
  ...
  function buildRabbitTunnel(){
    ...
    // V17: fecha fisicamente e visualmente as emendas das curvas. Os segmentos
    // retos encostam aqui em angulo e podiam deixar uma fresta/face ausente.
    // O pequeno pilar de canto e DoubleSide, bloqueia jogador e tambem entra
    // no raycast normal dos tiros por ser uma Mesh real.
    for(let i=1;i<pts.length-1;i++){
      const [cx,cz]=pts[i],ri=window.__inkRabbitTunnelInfo(cx,cz);
      // V18: nao fecha curvas que ainda pertencem as rampas de entrada/saida.
      // Na V17 esses pilares viravam uma coluna no meio da descida.
      if(ri.t<.18||ri.t>.82)continue;
      const cy=(ri.y-1.7);
      const cornerMat=mat(wall);cornerMat.side=THREE.DoubleSide;
      const corner=box(.72,3.55,.72,cornerMat,cx,cy+1.62,cz,false);
      corner.userData.rabbitWall=true;
      colliders.push({minX:cx-.38,maxX:cx+.38,minZ:cz-.38,maxZ:cz+.38,minY:cy-.15,height:cy+3.55});
    }
  }
  ...
  // DIAGNOSTICO V23: colore TODA mesh de cenário que cruza as duas entradas da toca.
  // Executa imediatamente apos buildWorld(), antes de jogadores/armas, para isolar o mapa.
  (function diagnosticRabbitEntranceMeshes(){
    const zones=[
      new THREE.Box3(new THREE.Vector3(-48,-8,-42),new THREE.Vector3(-35,8,-10)),
      new THREE.Box3(new THREE.Vector3(17,-8,27),new THREE.Vector3(48,8,42))
    ];
    const palette=[0xff1744,0x00e5ff,0xffea00,0x76ff03,0xd500f9,0xff6d00,0x2979ff,0xf50057,0x00e676,0x651fff,0xff9100,0x1de9b6];
    const box3=new THREE.Box3(); let n=0;
    scene.updateMatrixWorld(true);
    scene.traverse(o=>{
      if(!o.isMesh || !o.geometry) return;
      try{box3.setFromObject(o);}catch(_){return;}
      if(!zones.some(z=>z.intersectsBox(box3))) return;
      const c=palette[n++%palette.length];
      const old=o.material;
      o.material=new THREE.MeshBasicMaterial({color:c,side:THREE.DoubleSide,transparent:false,opacity:1,depthTest:true,depthWrite:true,wireframe:false});
      o.userData.__diagV23=true;o.userData.__diagOldMaterial=old;
    });
    const legend=document.createElement('div');
    legend.id='diagV23Legend';
    legend.style.cssText='position:fixed;left:8px;top:128px;z-index:99;max-width:330px;padding:9px 11px;border:3px solid #111;background:#fff;color:#111;font:900 12px Georgia;pointer-events:none';
    legend.textContent='DIAGNÓSTICO V23 • TODAS AS MESHES QUE CRUZAM AS ENTRADAS ESTÃO COLORIDAS ('+n+') • filme a superfície que esconde o boneco';
    document.body.appendChild(legend);
    console.log('[V23 DIAG] meshes coloridas nas entradas:',n);
  })();
  resetEnemies();
