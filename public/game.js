...
(() => {
  ...
  canvas.addEventListener('mousedown',e=>{
    if(!running||!isDesktopMode())return;
    e.preventDefault();
    if(e.button===2){
      pcRightHeld=true;
      if(weaponMode!=='knife'){
  ...
  document.addEventListener('pointerlockchange',()=>{mouseLook=document.pointerLockElement===canvas;canvas.style.cursor=(isDesktopMode()?'none':'crosshair');});
