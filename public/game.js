// V56.19 RAINHA — coelhos respawn, toca física, Cheshire, placas por spawn
...
(() => {
  ...
  function buildRabbitTunnel(){
    ...
    for(let i=1;i<pts.length-1;i++){
      ...
      colliders.push({minX:ox-1.58,maxX:ox+1.58,minZ:oz-1.58,maxZ:oz+1.58,minY:cy-.22,height:cy+3.70,rabbitWall:true});
    }
  ...
  function applyRabbitState(st){const r=destructibleRabbits.get(st?.id);if(!r)return;const hp=Number(st.hp);const active=st.active!==false&&Number.isFinite(hp)&&hp>0;r.active=active;r.hp=active?hp:0;r.group.visible=active;r.group.traverse(o=>{if(o.isMesh){o.userData.rabbitId=r.id;o.userData.noBulletMark=!active;o.raycast=THREE.Mesh.prototype.raycast;}});if(active){r.respawn=0;r.attackCooldown=.65;}}
  ...
  function buildCheshireCat(){
    const gr=new THREE.Group(),fur=mat(0x59606f),dark=mat(0x343947),pink=mat(0xc986a0);
    const body=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.92,10,8),fur));body.scale.set(1.35,.78,.82);body.position.y=.82;gr.add(body);
    const chest=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.52,9,7),dark));chest.scale.set(.85,1.05,.65);chest.position.set(0,1.05,-.36);gr.add(chest);
    const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.72,10,8),fur));head.position.set(0,1.62,-.34);gr.add(head);
    for(const sx of [-1,1]){const ear=addOutline(new THREE.Mesh(new THREE.ConeGeometry(.30,.70,5),fur));ear.position.set(sx*.43,2.18,-.28);ear.rotation.z=-sx*.18;gr.add(ear);const inner=new THREE.Mesh(new THREE.ConeGeometry(.14,.40,5),pink);inner.position.set(sx*.43,2.18,-.52);inner.rotation.z=-sx*.18;gr.add(inner);const eye=new THREE.Mesh(new THREE.SphereGeometry(.11,7,5),new THREE.MeshBasicMaterial({color:0xb8f1ff}));eye.position.set(sx*.25,1.76,-.98);gr.add(eye);}
    const nose=new THREE.Mesh(new THREE.SphereGeometry(.09,6,5),new THREE.MeshBasicMaterial({color:0xd59aaf}));nose.position.set(0,1.55,-1.04);gr.add(nose);
    const smile=new THREE.Mesh(new THREE.TorusGeometry(.34,.045,6,20,Math.PI),new THREE.MeshBasicMaterial({color:0xf4e5cf}));smile.position.set(0,1.42,-1.00);smile.rotation.z=Math.PI;gr.add(smile);
    for(const sx of [-1,1])for(const y of [1.48,1.38]){const w=new THREE.Mesh(new THREE.CylinderGeometry(.012,.012,.58,4),new THREE.MeshBasicMaterial({color:0x171510}));w.rotation.z=Math.PI/2+sx*.08;w.position.set(sx*.43,y,-.99);gr.add(w);}
    for(const sx of [-1,1]){const paw=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.28,7,6),dark));paw.scale.set(.75,.65,1.15);paw.position.set(sx*.48,.28,-.40);gr.add(paw);}
    const tail=addOutline(new THREE.Mesh(new THREE.TorusGeometry(.78,.16,7,20,Math.PI*1.35),fur));tail.position.set(.95,.88,.25);tail.rotation.x=Math.PI/2;tail.rotation.z=-.55;gr.add(tail);
    gr.position.set(-28.125,0,25.075);gr.userData.hp=180;gr.userData.alive=true;gr.userData.respawn=0;gr.userData.dir=new THREE.Vector3(1,0,.38).normalize();gr.userData.turnTimer=0;gr.traverse(o=>{if(o.isMesh)o.userData.cheshire=true;});scene.add(gr);cheshireCat=gr;
  }
