// SUBMUNDO INK OPS V13 BETA POLIMENTO
...
(() => {
  ...
  let rabbitActors=[],rabbitHitCooldown=0,tableAccessLatch=false,matchIntroActive=false,matchIntroTimer=null;
  ...
  function buildTeaParty(){const tx=27,tz=-25;const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(7.0,7.0,.70,16),mat(wallLight)));table.position.set(tx,2.2,tz);scene.add(table);cyl(1.0,4.4,mat(wood),tx,2.0,tz,10,true);for(let i=0;i<8;i++){if(i===4||i===3)continue;const a=i/8*TAU;const chair=box(2.2,4.0,2.0,mat(i%2?red:blue),tx+Math.cos(a)*10,2.0,tz+Math.sin(a)*9.2,true);chair.rotation.y=-a;}const pot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(2.0,10,7),mat(wall)));pot.position.set(tx,3.35,tz);scene.add(pot);cyl(.75,.35,mat(red),tx,5.0,tz,10,false);colliders.push({minX:tx-2.0,maxX:tx+2.0,minZ:tz-2.0,maxZ:tz+2.0,minY:2.50,height:5.35});for(const [dx,dz] of [[-3.2,2.1],[3.0,-2.0],[1.8,3.7],[-1.2,-3.6]]){cyl(.85,1.3,mat(wall),tx+dx,3.0,tz+dz,10,false);const tea=cyl(.65,.05,mat(red),tx+dx,3.68,tz+dz,10,false);tea.userData.noBulletMark=true;}const stepTops=[.30,.67,1.04,1.41,1.78,2.15,2.46];for(let i=0;i<stepTops.length;i++){const top=stepTops[i],w=4.9,d=2.55,xx=15.8+i*1.18,zz=-25;box(w,.20,d,mat(i===stepTops.length-1?red:woodLight),xx,top-.10,zz,false);window.__inkClimbPlatforms.push({minX:xx-w/2,maxX:xx+w/2,minZ:zz-d/2,maxZ:zz+d/2,top,minCurrentY:i===0?1.60:1.7+stepTops[i-1]-.18});}window.__inkClimbPlatforms.push({cx:tx,cz:tz,radius:6.65,top:2.55,minCurrentY:4.04});makeWorldSign('CHÁ MALUCO',27,-37.5,0);}
  function buildRabbitTunnel(){const pts=[[-42,-34],[-42,-20],[-42,34],[28,34],[42,34]];window.__inkRabbitPath=pts;let total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}window.__inkRabbitTunnelY=t=>{const ramp=.095;if(t<ramp)return 1.7-(t/ramp)*4.15;if(t>1-ramp)return 1.7-((1-t)/ramp)*4.15;return -2.45;};window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0,y:1.7},acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz),t=(acc+lens[i]*u)/total;if(d<best.d)best={d,t,y:window.__inkRabbitTunnelY(t)};acc+=lens[i];}return best;};makeWorldSign('TOCA DO COELHO',-37,-36,0);makeWorldSign('TOCA DO COELHO',37,38,Math.PI);let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(y1-y0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floor=box(5.8,.30,len+.8,mat(wallLight),mx,ym,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.76,wz=mz-Math.sin(yaw)*side*2.76;const wallSeg=box(.28,3.15,len+.7,mat(wall),wx,ym+1.56,wz,false);wallSeg.rotation.order='YXZ';wallSeg.rotation.y=yaw;wallSeg.rotation.x=-pitch;}if(y0<-1.1&&y1<-1.1){const roof=box(5.95,.28,len+.65,mat(0xe8e2d5),mx,ym+3.16,mz,false);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;}acc+=len;}for(let i=1;i<pts.length-1;i++){let a=0;for(let j=0;j<i;j++)a+=lens[j];const t=a/total,y=window.__inkRabbitTunnelY(t)-1.7;if(y<-1.1){const [x,z]=pts[i];const floorCap=box(6.0,.30,6.0,mat(wallLight),x,y,z,false);const roofCap=box(6.05,.28,6.05,mat(0xe8e2d5),x,y+3.16,z,false);roofCap.userData.rabbitRoof=true;for(const sx of [-1,1])for(const sz of [-1,1])box(.34,3.15,.34,mat(wall),x+sx*2.78,y+1.56,z+sz*2.78,false);}}}
  function buildRabbitHazards(){
    rabbitActors=[];
    const spots=[[-42,2],[-18,34],[18,34]];
    for(let i=0;i<spots.length;i++){const [x,z]=spots[i],gr=new THREE.Group();const body=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.38,7,5),mat(wallLight)),1.2);body.scale.set(1.25,.85,1);body.position.y=.34;gr.add(body);const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.27,7,5),mat(wall)),1.1);head.position.set(0,.66,-.28);gr.add(head);for(const sx of [-1,1]){const ear=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.055,.30,3,5),mat(wallLight)),1);ear.position.set(sx*.11,.98,-.25);ear.rotation.z=sx*.12;gr.add(ear);}const eye=new THREE.Mesh(new THREE.SphereGeometry(.035,5,4),mat(red));eye.position.set(.12,.70,-.52);gr.add(eye);const ri=window.__inkRabbitTunnelInfo?.(x,z);gr.position.set(x,(ri?.y??-2.45)-1.35,z);gr.userData.baseY=gr.position.y;gr.userData.phase=i*2.1;gr.traverse(o=>{if(o.isMesh){o.userData.noBulletMark=true;o.userData.rabbitHazard=true;}});scene.add(gr);rabbitActors.push(gr);}
  }
  function updateRabbitHazards(dt){rabbitHitCooldown=Math.max(0,rabbitHitCooldown-dt);const now=performance.now()*.001;for(const r of rabbitActors){r.position.y=r.userData.baseY+Math.abs(Math.sin(now*2.8+r.userData.phase))*.16;r.rotation.y=Math.sin(now*.7+r.userData.phase)*.8;const dx=player.pos.x-r.position.x,dz=player.pos.z-r.position.z;if(player.pos.y<0&&Math.hypot(dx,dz)<1.15&&rabbitHitCooldown<=0){rabbitHitCooldown=1.25;triggerDamageFeedback(6);hitNoticeEl.textContent='MORDIDA DE COELHO  -6';hitNoticeEl.classList.add('show');hitNoticeTimer=.55;if(multiplayer.room?.status==='playing')fetch('/api/multiplayer',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'rabbit_hit',token:multiplayer.token,roomCode:multiplayer.roomCode})}).catch(()=>{});else{let d=6;if(armor>0){const a=Math.min(armor,3);armor-=a;d-=a;}hp=Math.max(0,hp-d);updateHud();}}}}

  function buildCheshireCat(){const gr=new THREE.Group();const body=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.05,9,7),mat(black)));body.scale.set(1.5,.75,1);gr.add(body);const head=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.78,9,7),mat(0x2c3140)));head.position.set(0,1.05,0);gr.add(head);for(const sx of [-1,1]){const eye=new THREE.Mesh(new THREE.SphereGeometry(.13,6,5),new THREE.MeshBasicMaterial({color:0x66d7ff}));eye.position.set(sx*.27,1.18,-.69);gr.add(eye);}const smile=new THREE.Mesh(new THREE.TorusGeometry(.34,.055,6,18,Math.PI),new THREE.MeshBasicMaterial({color:red}));smile.position.set(0,.9,-.7);smile.rotation.z=Math.PI;gr.add(smile);gr.position.set(-28.125,1.4,25.075);gr.userData.hp=180;gr.userData.phase=0;gr.userData.alive=false;gr.userData.respawn=60;gr.userData.dir=new THREE.Vector3(1,0,.35).normalize();gr.visible=false;gr.traverse(o=>{if(o.isMesh)o.userData.cheshire=true;});scene.add(gr);cheshireCat=gr;}
  ...
  function setCheshireState(st){if(!cheshireCat||!st)return;cheshireCat.userData.hp=Number(st.hp??cheshireCat.userData.hp);cheshireCat.userData.alive=st.active===true&&cheshireCat.userData.hp>0;cheshireCat.visible=cheshireCat.userData.alive;cheshireCat.userData.respawn=Number(st.respawnMs||0)/1000;if(st.message){hitNoticeEl.textContent=st.message;hitNoticeEl.classList.add('show');hitNoticeTimer=2.1;}}window.__inkRealtimeCat=st=>setCheshireState(st);
  ...
  for(const v of Object.values(weaponVariants))v.traverse(o=>{if(o.isLineSegments){o.visible=true;o.material.color.setHex(0x151a22);o.material.opacity=.88;o.material.depthTest=false;o.material.depthWrite=false;o.renderOrder=102;}else if(o.isMesh){o.material.depthTest=false;o.material.depthWrite=false;o.renderOrder=101;}});weaponVariants.sniper.visible=false; weaponVariants.smg.visible=false; weaponVariants.shotgun.visible=false; weaponVariants.launcher.visible=false; weaponVariants.knife.visible=false;
  ...
  function updateRemotePlayers(dt){
    for(const remote of multiplayer.remotePlayers.values()){
      const packetAge=Math.min(.055,(performance.now()-remote.lastPacketAt)/1000);
      ...
      remote.group.position.lerp(remote.predicted,Math.min(1,dt*18));
      let turn=((remote.targetYaw-remote.group.rotation.y+Math.PI)%(Math.PI*2)+Math.PI)%(Math.PI*2)-Math.PI;
      remote.group.rotation.y+=turn*Math.min(1,dt*20);
  ...
  // O transporte WebSocket entrega snapshots a cada 40 ms; aplicar aqui evita esperar
  ...
  const matchIntro=document.createElement('div');matchIntro.style.cssText='position:fixed;inset:0;z-index:78;display:none;place-items:center;background:#171510;font-family:Georgia,serif;pointer-events:none;overflow:hidden';matchIntro.innerHTML=`<style>@keyframes inkBookOpen{0%{transform:perspective(900px) rotateY(-84deg) scale(.82)}100%{transform:perspective(900px) rotateY(0) scale(1)}}@keyframes inkBookOpenR{0%{transform:perspective(900px) rotateY(84deg) scale(.82)}100%{transform:perspective(900px) rotateY(0) scale(1)}}</style><div style="position:relative;width:min(88vw,760px);height:min(68vh,410px);display:flex;filter:drop-shadow(0 12px 22px #000)"><div data-page-l style="width:50%;padding:24px 18px;border:5px solid #171510;border-right:2px solid #171510;border-radius:18px 4px 4px 18px;background:#f6f0df;transform-origin:right center;animation:inkBookOpen .85s ease-out both"><div style="font:900 25px Georgia;text-align:center;color:#294aa3">REINO DAS COPAS</div><div style="margin:18px auto;width:88%;height:230px;border:3px solid #294aa3;position:relative;background:repeating-linear-gradient(0deg,transparent 0 24px,rgba(41,74,163,.08) 25px),#fcfbf7"><div style="position:absolute;left:48%;top:0;width:7%;height:100%;background:#9abbd0"></div><b style="position:absolute;left:8%;top:10%">LABIRINTO</b><b style="position:absolute;left:56%;top:18%;color:#d73535">RAINHA</b><b style="position:absolute;left:57%;bottom:12%">CHÁ</b><b style="position:absolute;left:5%;bottom:12%">TOCA</b></div></div><div data-page-r style="width:50%;padding:28px 18px;border:5px solid #171510;border-left:2px solid #171510;border-radius:4px 18px 18px 4px;background:#fbf7e9;transform-origin:left center;animation:inkBookOpenR .85s ease-out both;display:grid;place-items:center;text-align:center"><div><div style="font:900 18px Georgia;color:#171510">A TINTA VAI CORRER...</div><div data-count style="font:900 clamp(72px,18vw,132px) Georgia;color:#d73535;line-height:1;margin-top:14px">5</div><div style="font:900 14px Georgia;color:#294aa3">PREPARE-SE NA SUA BASE</div></div></div></div>`;document.body.appendChild(matchIntro);const matchIntroCount=matchIntro.querySelector('[data-count]');
  function startMatchIntro(){clearInterval(matchIntroTimer);matchIntroActive=true;matchIntro.style.display='grid';let n=5;matchIntroCount.textContent=n;matchIntroTimer=setInterval(()=>{n--;if(n>0){matchIntroCount.textContent=n;}else{clearInterval(matchIntroTimer);matchIntroTimer=null;matchIntroCount.textContent='JÁ!';setTimeout(()=>{matchIntro.style.display='none';matchIntroActive=false;hitNoticeEl.textContent='5s DE PROTEÇÃO DE NASCIMENTO';hitNoticeEl.classList.add('show');hitNoticeTimer=1.4;},650);}},1000);}

  function startMultiplayerSync(){clearInterval(multiplayer.networkTimer);clearInterval(multiplayer.polling);multiplayer.selfAlive=true;if(multiplayer.roomCode&&multiplayer.room?.status==='playing'){onlineScoreEl.style.display='block';multiplayer.networkTimer=setInterval(pushMultiplayerState,40);pushMultiplayerState();hitNoticeEl.textContent='CONECTADO À SALA '+multiplayer.roomCode;hitNoticeEl.classList.add('show');hitNoticeTimer=1.4;}}
  ...
  function startGame(){
    ...
    setWeaponMode('rifle');
    scopeMode=false;adsProgress=0;spawnGrace=10;rabbitTunnelMode=false;rabbitTunnelCooldown=0;tableAccessLatch=false;if(cheshireCat){cheshireCat.userData.alive=false;cheshireCat.visible=false;cheshireCat.userData.respawn=60;}
    ...
    startMultiplayerSync();if(multiplayer.room?.status==='playing')startMatchIntro();
  }
  ...
  function update(dt){
    if(controlsEditing||matchIntroActive)return;
    ...
    {const terrainHeight=getPlayerHeight(player.pos.x,player.pos.z);if(grounded && terrainHeight<player.pos.y-.30) grounded=false;if(grounded){player.pos.y += (terrainHeight-player.pos.y)*Math.min(1,dt*14);}else{verticalVelocity-=(verticalVelocity<0?20.5:13.5)*dt;player.pos.y+=verticalVelocity*dt;if(player.pos.y<=terrainHeight){player.pos.y=terrainHeight;verticalVelocity=0;grounded=true;}}const td=Math.hypot(player.pos.x-27,player.pos.z+25),stairGate=player.pos.x>20&&player.pos.x<24&&Math.abs(player.pos.z+25)<2.2&&player.pos.y>3.25;if(stairGate)tableAccessLatch=true;if(td>7.25||player.pos.y<3.0)tableAccessLatch=false;if(td<6.75&&!tableAccessLatch&&player.pos.y>3.18){player.pos.y=3.18;verticalVelocity=Math.min(0,verticalVelocity);grounded=false;}}
    ...
    if(fireHeld) shoot();
    updateCheshireCat(dt);updateRabbitHazards(dt);if(goldenInkwell?.visible){goldenInkwell.rotation.y+=dt*.9;tryTakeGoldenReward();}if(multiplayer.room?.status!=='playing'&&rewardLocalRespawn>0){rewardLocalRespawn-=dt;if(rewardLocalRespawn<=0&&goldenInkwell){goldenInkwell.visible=true;hitNoticeEl.textContent='TINTEIRO DOURADO RENASCEU NO LABIRINTO!';hitNoticeEl.classList.add('show');hitNoticeTimer=2.2;}}
    ...
    const bobY=moving?Math.sin(player.bob)*(.026+.010*sprintBlend):0;
    ...
    const runSwayX=Math.sin(player.bob*.78)*.042*sprintBlend;
    const runSwayY=Math.abs(Math.cos(player.bob*.78))*.028*sprintBlend;
    const runTilt=Math.sin(player.bob*.78)*.024*sprintBlend;
    ...
    if(weaponMode==='rifle'){
      ...
      weapon.scale.setScalar(.86*(1+.025*ease));
      ...
      s.scale.setScalar(.76*(1+.07*ease));
      ...
      k.scale.setScalar(.90*(1+.04*swing));
    }else{
      // Submetralhadora, espingarda e lança-foguetes também acompanham a corrida.
      const baseX=.10+runSwayX,baseY=(weaponMode==='shotgun'?-.13:-.08)-player.recoil*.24-runSwayY+weaponKick*.025,baseZ=(weaponMode==='shotgun'?.34:(weaponMode==='smg'?.24:.28))+weaponKick*.035;activeWeapon.position.set(baseX,baseY,baseZ);
      activeWeapon.rotation.set(player.recoil*.13+weaponKick*.055,0,runTilt);
      activeWeapon.scale.setScalar(weaponMode==='shotgun'?.68:(weaponMode==='smg'?.78:.82));
    }
