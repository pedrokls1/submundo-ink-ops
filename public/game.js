...
(() => {
  ...
  function buildTeaParty(){const tx=27,tz=-25;const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(7.0,7.0,.70,16),mat(wallLight)));table.position.set(tx,2.2,tz);scene.add(table);cyl(1.0,4.4,mat(wood),tx,2.0,tz,10,true);for(let i=0;i<8;i++){const a=i/8*TAU;const chair=box(2.2,4.0,2.0,mat(i%2?red:blue),tx+Math.cos(a)*10,2.0,tz+Math.sin(a)*9.2,true);chair.rotation.y=-a;}const pot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(2.0,10,7),mat(wall)));pot.position.set(tx,3.35,tz);scene.add(pot);cyl(.75,.35,mat(red),tx,5.0,tz,10,false);for(const [dx,dz] of [[-3.2,2.1],[3.0,-2.0],[1.8,3.7],[-1.2,-3.6]]){cyl(.85,1.3,mat(wall),tx+dx,3.0,tz+dz,10,false);const tea=cyl(.65,.05,mat(red),tx+dx,3.68,tz+dz,10,false);tea.userData.noBulletMark=true;}for(let i=0;i<5;i++){const hh=.28,y=.14+i*.34,w=4.2,d=3.0,xx=18.0+i*.95,zz=-25;box(w,hh,d,mat(i===4?red:woodLight),xx,y,zz,true);window.__inkClimbPlatforms.push({minX:xx-w/2,maxX:xx+w/2,minZ:zz-d/2,maxZ:zz+d/2,top:y+hh/2});}window.__inkClimbPlatforms.push({minX:20,maxX:34,minZ:-32,maxZ:-18,top:2.55});makeWorldSign('CHÁ MALUCO',27,-37.5,0);}
  function buildRabbitTunnel(){
    const pts=[[-40,-31],[-40,-15],[-40,18],[-34,27],[-20,32],[20,32],[34,27],[40,18],[40,31]];
    window.__inkRabbitPath=pts;let total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}
    window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0},acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz);if(d<best.d)best={d,t:(acc+lens[i]*u)/total};acc+=lens[i];}return best;};
    window.__inkRabbitTunnelY=t=>{const ramp=.145;if(t<ramp)return 1.7-(t/ramp)*4.45;if(t>1-ramp)return 1.7-((1-t)/ramp)*4.45;return -2.75;};
    function entrance(x,z,rot){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const ring=addOutline(new THREE.Mesh(new THREE.TorusGeometry(2.0,.40,8,24,Math.PI),mat(wall)));ring.rotation.z=Math.PI;ring.position.y=1.70;gg.add(ring);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.68,20),new THREE.MeshBasicMaterial({color:black,side:THREE.DoubleSide}));dark.position.set(0,1.50,.10);gg.add(dark);scene.add(gg);makeWorldSign('TOCA DO COELHO',x+(x<0?4:-4),z+(z<0?-2.8:2.8),rot);return {x,z};}
    window.__inkRabbitEntrances=[entrance(-40,-31,0),entrance(40,31,Math.PI)];let acc=0;
    for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,py0=window.__inkRabbitTunnelY(t0)-1.7,py1=window.__inkRabbitTunnelY(t1)-1.7,py=(py0+py1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(py1-py0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floor=box(5.45,.30,len+1.35,mat(wallLight),mx,py,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.62,wz=mz-Math.sin(yaw)*side*2.62;const wallSeg=box(.36,3.65,len+1.45,mat(wall),wx,py+1.82,mz,false);wallSeg.rotation.y=yaw;}const roof=box(5.85,.36,len+1.55,mat(0xded8ca),mx,py+3.62,mz,false);roof.rotation.order='YXZ';roof.rotation.y=yaw;roof.rotation.x=-pitch;roof.userData.rabbitRoof=true;acc+=len;}
    let acc2=0;for(let i=1;i<pts.length-1;i++){acc2+=lens[i-1];const [x,z]=pts[i],t=acc2/total,y=window.__inkRabbitTunnelY(t)-1.7;const cap=box(6.0,.38,6.0,mat(0xded8ca),x,y+3.62,z,false);cap.userData.rabbitRoof=true;}
  }
  ...
  function buildSeasonalReinoDasCopas(){window.__inkClimbPlatforms=[];buildCombatMaze();buildQueenCourt();buildTeaParty();buildRabbitTunnel();buildCheshireCat();buildGoldenInkwell();const cards=[['A','♥',-6,-34,.12,1.15],['7','♠',12,31,-.2,1.0],['Q','♦',31,14,.18,1.05],['3','♣',-7,27,-.12,.9],['K','♥',43,-10,.12,1.0],['9','♦',-44,5,-.08,.95],['5','♠',-30,-27,.16,1.0],['J','♦',-17,-31,-.14,1.0],['8','♣',16,-17,.10,.95],['2','♥',34,-33,-.12,1.0]];for(const c of cards)makeCardProp(...c);for(const [x,z,s] of [[-45,34,1.05],[45,35,1.0],[-9,-36,.85],[9,35,.8],[44,-36,.9],[-46,-35,.85]])makeTreeProp(x,z,s);for(const [id,x,z,s] of [['m1',-34,-27,1.2],['m2',-19,-34,1.05],['m3',18,-34,1.15],['m4',37,-21,1.15],['m5',33,22,1.0],['m6',-8,34,.95],['m7',13,23,.9],['m8',-17,8,.9]])makeDestructibleMushroom(id,x,z,s);for(const [x,z] of [[-47,18],[46,12],[-7,-20],[12,-12]]){const levels=[[5.8,.30,3.6,.15,0],[5.4,.30,3.35,.45,.34],[5.0,.30,3.1,.75,.68]];for(let j=0;j<levels.length;j++){const [w,hh,d,y,dz]=levels[j],zz=z+dz;box(w,hh,d,mat(j===2?red:(j%2?woodLight:wall)),x,y,zz,true);window.__inkClimbPlatforms.push({minX:x-w/2,maxX:x+w/2,minZ:zz-d/2,maxZ:zz+d/2,top:y+hh/2});}}for(const x of [-49,49])box(.18,.05,82,mat(ink),x,.03,0,false);box(98,.05,.18,mat(ink),0,.03,41,false);box(98,.05,.18,mat(ink),0,.03,-41,false);for(const [x,z,color] of [[0,36,blue],[0,-36,red]])box(8,.10,.8,mat(color),x,.08,z,false);}
  ...
  const updateLayoutTools=()=>{resetLayoutBtn.style.display=controlsEditing?'block':'none';if(typeof cancelScopeBtn!=='undefined')cancelScopeBtn.style.display=controlsEditing?'block':(scopeMode&&weaponMode==='sniper'?'block':'none');};
  ...
  function reloadWeapon(){
    ...
    reloadBtn.textContent='↻...';
    if(!reloadCartridge){reloadCartridge=new THREE.Group();const jar=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.16,.19,.30,12),mat(red)));jar.rotation.z=Math.PI/2;reloadCartridge.add(jar);const rim=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(.18,.18,.055,12),mat(black)));rim.rotation.z=Math.PI/2;rim.position.x=.17;reloadCartridge.add(rim);const label=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.035,.16,.20),mat(0xf7f4eb)));label.position.set(-.02,0,-.17);reloadCartridge.add(label);for(const [yy,zz] of [[-.06,-.19],[.04,-.19]]){const drip=new THREE.Mesh(new THREE.SphereGeometry(.035,6,5),mat(0xb51f2b));drip.position.set(-.10,yy,zz);reloadCartridge.add(drip);}const handle=new THREE.Mesh(new THREE.TorusGeometry(.15,.025,6,14,Math.PI),mat(black));handle.rotation.y=Math.PI/2;handle.position.set(-.05,.18,0);reloadCartridge.add(handle);reloadCartridge.scale.setScalar(.62);reloadCartridge.position.set(.26,-.48,-.62);camera.add(reloadCartridge);}reloadCartridge.visible=true;reloadCartridge.userData.reloadLife=reloadTimer;reloadCartridge.userData.reloadTotal=reloadTimer;
    setTimeout(()=>{
  ...
  function update(dt){
    ...
    if(!rabbitTunnelMode&&rabbitTunnelCooldown<=0&&window.__inkRabbitEntrances?.length===2){for(let ei=0;ei<window.__inkRabbitEntrances.length;ei++){const ent=window.__inkRabbitEntrances[ei];if(Math.hypot(player.pos.x-ent.x,player.pos.z-ent.z)<2.3){rabbitTunnelMode=true;rabbitTunnelEntrySide=ei;rabbitTunnelCanBacktrack=false;rabbitTunnelCooldown=1.0;grounded=true;verticalVelocity=0;hitNoticeEl.textContent='TOCA DO COELHO • DESCENDO A RAMPA';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;break;}}}
    ...
    if(rabbitTunnelMode&&window.__inkRabbitTunnelInfo){const info=window.__inkRabbitTunnelInfo(player.pos.x,player.pos.z);player.pos.y=window.__inkRabbitTunnelY?window.__inkRabbitTunnelY(info.t):-2.75;grounded=true;verticalVelocity=0;const progress=rabbitTunnelEntrySide===0?info.t:(1-info.t);if(progress>.09)rabbitTunnelCanBacktrack=true;const atEntry=rabbitTunnelEntrySide===0?info.t<.018:info.t>.982;const atOther=rabbitTunnelEntrySide===0?info.t>.982:info.t<.018;if(rabbitTunnelCooldown<=0&&(atOther||(atEntry&&rabbitTunnelCanBacktrack))){rabbitTunnelMode=false;rabbitTunnelEntrySide=-1;rabbitTunnelCanBacktrack=false;rabbitTunnelCooldown=1.4;player.pos.y=1.7;hitNoticeEl.textContent='VOCÊ SAIU DA TOCA DO COELHO';hitNoticeEl.classList.add('show');hitNoticeTimer=.7;}}else{const terrainHeight=getPlayerHeight(player.pos.x,player.pos.z);if(grounded && terrainHeight<player.pos.y-.30) grounded=false;if(grounded){player.pos.y += (terrainHeight-player.pos.y)*Math.min(1,dt*14);}else{verticalVelocity-=(verticalVelocity<0?20.5:13.5)*dt;player.pos.y+=verticalVelocity*dt;if(player.pos.y<=terrainHeight){player.pos.y=terrainHeight;verticalVelocity=0;grounded=true;}}}
