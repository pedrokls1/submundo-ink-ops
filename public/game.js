...
(() => {
  ...
  function beginMatchIntro(myTeam){
    ...
    // V51: não alteramos camera.up. Isso evita inversão/rolagem residual no Safari/iPhone.
    camera.up.set(0,1,0);camera.position.set(0,82,0.01);camera.rotation.set(-Math.PI/2,0,0);camera.fov=58;camera.updateProjectionMatrix();
  ...
  function updateMatchIntro(now){
    ...
    if(t>=5){I.root.remove();I.hide.forEach((el,i)=>el.style.visibility=I.oldVis[i]);scene.fog=I.oldFog;camera.up.set(0,1,0);camera.rotation.set(0,I.spawnYaw,0);camera.fov=NORMAL_FOV;camera.updateProjectionMatrix();player.pos.set(I.spawnX,1.7,I.spawnZ);player.yaw=I.spawnYaw;player.pitch=0;matchIntro=null;last=performance.now();applyControlMode();if(isDesktopMode())setTimeout(()=>canvas.requestPointerLock?.(),0);return false;}
    return true;
  }
