...
(() => {
  ...
  // V56.30: mão do jogador segurando cada arma de fogo em primeira pessoa.
  for(const [kind,v] of Object.entries(weaponVariants)){
    if(kind==='knife')continue;
    const handMat=mat(0xfffbef);
    const rearHand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.19,8,6),handMat),2.3);
    rearHand.scale.set(1.0,.78,1.25); rearHand.position.set(.38,-.69,kind==='sniper'?-1.02:-.78); v.add(rearHand);
    const frontHand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.18,8,6),handMat),2.3);
    frontHand.scale.set(1.0,.76,1.28); frontHand.position.set(.40,-.48,kind==='launcher'?-1.55:(kind==='sniper'?-2.00:-1.55)); v.add(frontHand);
  }
  ...
  function makeRemotePlayer(state){
    ...
    const vest=addOutline(new THREE.Mesh(new THREE.BoxGeometry(.72,.48,.48),darkMat),3.0);
    vest.position.set(0,1.22,-.08); group.add(vest);
    ...
    const arms=[];
    // V56.31: ombros menores e braços construídos ENTRE o ombro e a mão.
    // Assim a articulação realmente converge para a arma, sem bolas soltas ou mãos atravessando o fuzil.
    const armBetween=(a,b)=>{
      const mid=a.clone().add(b).multiplyScalar(.5), len=a.distanceTo(b);
      const arm=addOutline(new THREE.Mesh(new THREE.CapsuleGeometry(.085,Math.max(.12,len-.17),4,7),accentMat),2.5);
    ...
    const shoulderL=new THREE.Vector3(-.365,1.43,-.05), shoulderR=new THREE.Vector3(.365,1.43,-.05);
    const handLPos=new THREE.Vector3(.145,1.235,-.94), handRPos=new THREE.Vector3(.155,1.155,-.61);
    for(const pos of [shoulderL,shoulderR]){
      const shoulder=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.115,8,6),accentMat),2.5);
      shoulder.position.copy(pos); group.add(shoulder);
    }
    armBetween(shoulderL,handLPos); armBetween(shoulderR,handRPos);
    for(const [pos,scale] of [[handLPos,[.92,.72,1.08]],[handRPos,[.92,.72,1.04]]]){
      const hand=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.105,8,6),paperMat),2.2);
    ...
    return {group,legs,arms,gun:gunGroup,head,target:initial.clone(),predicted:initial.clone(),lastNetPos:initial.clone(),velocity:new THREE.Vector3(),lastPacketAt:performance.now(),targetYaw:state.yaw+Math.PI,step:0,footTimer:.12+Math.random()*.18,weapon:state.weapon,team:state.team,puddle:null,alive:state.alive!==false};
  }
  function updateRemotePlayers(dt){
    ...
    for(const remote of multiplayer.remotePlayers.values()){
      ...
      remote.step+=dt*(moving?13.5:2);remote.legs[0].rotation.x=moving?Math.sin(remote.step)*.68:0;remote.legs[1].rotation.x=moving?-Math.sin(remote.step)*.68:0;remote.legs[0].rotation.z=.055+(moving?Math.abs(Math.sin(remote.step))*.055:0);remote.legs[1].rotation.z=-.055-(moving?Math.abs(Math.sin(remote.step))*.055:0);remote.footTimer=(remote.footTimer||0)-dt;if(moving&&remote.footTimer<=0){remoteFootstepSound(remote);remote.footTimer=.32+Math.random()*.06;}
