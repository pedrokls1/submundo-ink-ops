...
(() => {
  ...
  function buildTeaParty(){const tx=27,tz=-25;const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(7.0,7.0,.70,16),mat(wallLight)));table.position.set(tx,2.2,tz);scene.add(table);cyl(1.0,4.4,mat(wood),tx,2.0,tz,10,true);for(let i=0;i<8;i++){if(i===4||i===3)continue;const a=i/8*TAU;const chair=box(2.2,4.0,2.0,mat(i%2?red:blue),tx+Math.cos(a)*10,2.0,tz+Math.sin(a)*9.2,true);chair.rotation.y=-a;}const pot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(2.0,10,7),mat(wall)));pot.position.set(tx,3.35,tz);scene.add(pot);cyl(.75,.35,mat(red),tx,5.0,tz,10,false);colliders.push({minX:tx-6.65,maxX:tx+6.65,minZ:tz-6.65,maxZ:tz+6.65,height:2.58});
  // V13: a mesa é um volume sólido; não é mais possível atravessá-la por baixo durante o pulo.for(const [dx,dz] of [[-3.2,2.1],[3.0,-2.0],[1.8,3.7],[-1.2,-3.6]]){cyl(.85,1.3,mat(wall),tx+dx,3.0,tz+dz,10,false);const tea=cyl(.65,.05,mat(red),tx+dx,3.68,tz+dz,10,false);tea.userData.noBulletMark=true;}const stepTops=[.30,.67,1.04,1.41,1.78,2.15,2.46];for(let i=0;i<stepTops.length;i++){const top=stepTops[i],w=4.9,d=2.55,xx=15.8+i*1.18,zz=-25;box(w,.20,d,mat(i===stepTops.length-1?red:woodLight),xx,top-.10,zz,false);window.__inkClimbPlatforms.push({minX:xx-w/2,maxX:xx+w/2,minZ:zz-d/2,maxZ:zz+d/2,top,minCurrentY:i===0?1.60:1.7+stepTops[i-1]-.18});}window.__inkClimbPlatforms.push({cx:tx,cz:tz,radius:6.65,top:2.55,minCurrentY:4.04});makeWorldSign('CHÁ MALUCO',27,-37.5,0);}
  function buildRabbitTunnel(){const pts=[[-42,-34],[-42,-20],[-42,34],[28,34],[42,34]];window.__inkRabbitPath=pts;let total=0,lens=[];for(let i=0;i<pts.length-1;i++){const l=Math.hypot(pts[i+1][0]-pts[i][0],pts[i+1][1]-pts[i][1]);lens.push(l);total+=l;}window.__inkRabbitTunnelY=t=>{const ramp=.095;if(t<ramp)return 1.7-(t/ramp)*4.15;if(t>1-ramp)return 1.7-((1-t)/ramp)*4.15;return -2.45;};window.__inkRabbitTunnelInfo=function(x,z){let best={d:1e9,t:0,y:1.7},acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],vx=bx-ax,vz=bz-az,ll=vx*vx+vz*vz;let u=((x-ax)*vx+(z-az)*vz)/ll;u=Math.max(0,Math.min(1,u));const px=ax+vx*u,pz=az+vz*u,d=Math.hypot(x-px,z-pz),t=(acc+lens[i]*u)/total;if(d<best.d)best={d,t,y:window.__inkRabbitTunnelY(t)};acc+=lens[i];}return best;};makeWorldSign('TOCA DO COELHO',-37,-36,0);makeWorldSign('TOCA DO COELHO',37,38,Math.PI);let acc=0;for(let i=0;i<pts.length-1;i++){const [ax,az]=pts[i],[bx,bz]=pts[i+1],len=lens[i],t0=acc/total,t1=(acc+len)/total,y0=window.__inkRabbitTunnelY(t0)-1.7,y1=window.__inkRabbitTunnelY(t1)-1.7,ym=(y0+y1)/2,yaw=Math.atan2(bx-ax,bz-az),pitch=Math.atan2(y1-y0,len),mx=(ax+bx)/2,mz=(az+bz)/2;const floorMat=mat(wallLight);floorMat.side=THREE.DoubleSide;const floor=box(5.8,.30,len+1.15,floorMat,mx,ym,mz,false);floor.rotation.order='YXZ';floor.rotation.y=yaw;floor.rotation.x=-pitch;for(const side of [-1,1]){const wx=mx+Math.cos(yaw)*side*2.76,wz=mz-Math.sin(yaw)*side*2.76;const wallMat=mat(wall);wallMat.side=THREE.DoubleSide;const wallSeg=box(.34,3.22,len+1.05,wallMat,wx,ym+1.56,wz,false);wallSeg.rotation.order='YXZ';wallSeg.rotation.y=yaw;wallSeg.rotation.x=-pitch;}if(y0<-1.1&&y1<-1.1){const roof=box(5.95,.28,len+.65,mat(0xe8e2d5),mx,ym+3.16,mz,false);roof.rotation.y=yaw;roof.userData.rabbitRoof=true;}acc+=len;}
  // V13: soleiras extras escondem a emenda da rampa com o piso externo nas duas entradas.
  for(const [x,z,yaw] of [[-42,-34,0],[42,34,Math.PI/2]]){const apronMat=mat(wallLight);apronMat.side=THREE.DoubleSide;const apron=box(5.9,.24,3.0,apronMat,x,.02,z,false);apron.rotation.y=yaw;}
  }
  ...
  // V13: remove traços auxiliares/hachuras da arma em primeira pessoa que podiam vazar pela tela.weaponVariants.sniper.visible=false; weaponVariants.smg.visible=false; weaponVariants.shotgun.visible=false; weaponVariants.launcher.visible=false; weaponVariants.knife.visible=false;
