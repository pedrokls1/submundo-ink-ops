...
(() => {
  ...
  function getPlayerHeight(x,z){if(Math.abs(x)<2.6||Math.abs(x-30)<2.6||Math.abs(x+30)<2.6){const az=Math.abs(z);if(az<=3.5)return 2.20;if(az<5.9)return 1.70+(5.9-az)/2.4*.50;}return 1.7;}
  ...
  function buildSeasonalReinoDasCopas(){
    const heartPts=[];for(let i=0;i<30;i++){const t=i/30*TAU;const x=5.7*Math.sin(t)**3;const z=-(4.7*Math.cos(t)-1.9*Math.cos(2*t)-.85*Math.cos(3*t)-.38*Math.cos(4*t));heartPts.push(new THREE.Vector3(x*.74,.10,z*.74));}heartPts.push(heartPts[0].clone());scene.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(heartPts),new THREE.LineBasicMaterial({color:red,transparent:true,opacity:.92})));
    const throne=box(2.35,3.0,.72,mat(wall),0,1.5,5.7,true);addHatching(throne,1.8,2.25,.37,6,true);const crown=new THREE.Mesh(new THREE.ConeGeometry(.78,.82,5),mat(red));crown.position.set(0,3.42,5.7);crown.rotation.y=.3;addOutline(crown);scene.add(crown);
    for(const [x,z] of [[-6.4,5],[6.4,5],[-6.4,-5],[6.4,-5]]){const b=box(2.4,.95,.50,mat(woodLight),x,.48,z,true);addHatching(b,1.8,.58,.26,3,false);box(.32,.38,.08,mat(red),x,.8,z-.27,false);}for(const [x,z] of [[-8.2,1.8],[8.2,1.8],[-8.2,-1.8],[8.2,-1.8]]){box(.9,2.35,.20,mat(wall),x,1.18,z,true);box(.28,.28,.22,mat(red),x,1.3,z-.1,false);}
    const table=addOutline(new THREE.Mesh(new THREE.CylinderGeometry(3.0,3.0,.34,12),mat(wallLight)));table.position.set(20,.72,19);scene.add(table);cyl(.50,1.6,mat(wood),20,.8,19,10,true);for(let i=0;i<5;i++){const a=i/5*TAU;const chair=box(.9,1.0,.9,mat(i%2?woodLight:wall),20+Math.cos(a)*4,.5,19+Math.sin(a)*4,true);chair.rotation.y=-a;}const teaPot=addOutline(new THREE.Mesh(new THREE.SphereGeometry(.72,8,6),mat(wall)));teaPot.position.set(20,1.25,19);scene.add(teaPot);
    const hedgeMat=mat(0xf7f7f2);const hedge=(x,z,w,d)=>{const hh=2.45;const m=box(w,hh,d,hedgeMat,x,hh/2,z,true);addHatching(m,Math.max(.4,w*.72),1.75,Math.max(.08,d*.35),7,true);return m;};hedge(-25,-16,14,.85);hedge(-31,-21,.85,11);hedge(-20,-23,10,.85);hedge(-18,-17,.85,9);hedge(-27,-27,12,.85);hedge(-25,-21,6,.78);hedge(-22,-18,.78,5);hedge(-29,-18,4.2,.78);hedge(-24,-25,.78,4.5);hedge(-17,-26,5,.78);
    const rabbitEntrances=[];function rabbitHole(x,z,team,rot=0){const gg=new THREE.Group();gg.position.set(x,0,z);gg.rotation.y=rot;const arch=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.55,.32,7,20,Math.PI),mat(wall)));arch.rotation.z=Math.PI;arch.position.y=1.35;gg.add(arch);const dark=new THREE.Mesh(new THREE.CircleGeometry(1.26,18),new THREE.MeshBasicMaterial({color:0x171510,side:THREE.DoubleSide}));dark.position.set(0,1.20,.08);gg.add(dark);scene.add(gg);rabbitEntrances.push({x,z,team});}rabbitHole(0,29,'azul',0);rabbitHole(0,-29,'vermelho',Math.PI);window.__inkRabbitEntrances=rabbitEntrances;
    const tunnelFloor=new THREE.Mesh(new THREE.BoxGeometry(4.4,.16,56),mat(wallLight));tunnelFloor.position.set(0,-3.05,0);tunnelFloor.receiveShadow=true;scene.add(tunnelFloor);for(const sx of [-2.2,2.2]){const side=box(.22,2.8,56,mat(wall),sx,-1.62,0,false);addHatching(side,1.8,2.1,.10,8,true);}for(let z=-24;z<=24;z+=6){const stripe=box(3.9,.04,.18,mat(z<0?red:blue),0,-2.94,z,false);stripe.userData.noBulletMark=true;}for(const z of [-26,26])for(const sx of [-2.2,2.2])box(.28,3.0,.28,mat(ink),sx,-1.55,z,false);
    for(const x of [-44,44])box(.18,.05,82,mat(ink),x,.03,0,false);box(88,.05,.18,mat(ink),0,.03,41,false);box(88,.05,.18,mat(ink),0,.03,-41,false);for(const [x,z,color] of [[0,33,blue],[0,-33,red]])box(6,.10,.8,mat(color),x,.08,z,false);
  }
  ...
  async function setVoiceEnabled(on){if(on){try{voiceStream=voiceStream||await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true},video:false});voiceEnabled=true;voiceBtn.textContent='🎙✓';voiceBtn.style.background='#dff3e8';voiceBtn.style.opacity='1';try{await audioCtx?.resume?.();}catch(_){}for(const token of multiplayer.remotePlayers.keys())window.__inkRealtimeSendVoice?.(token,{ready:true});}catch(_){voiceEnabled=false;voiceBtn.textContent='🎙!';voiceBtn.title='Microfone indisponível';}}else{voiceEnabled=false;voiceBtn.textContent='🎙';voiceBtn.style.opacity='.72';for(const token of [...voicePeers.keys()])closeVoicePeer(token);voiceStream?.getTracks().forEach(t=>t.stop());voiceStream=null;}}
  ...
  function showDeathScreen(seconds){deathOverlay.style.display='grid';const s=Math.max(1,Math.ceil(Number(seconds)||1));if(lastDeathSeconds===null)deathJoke.textContent=deathJokes[Math.floor(Math.random()*deathJokes.length)];lastDeathSeconds=s;deathCount.textContent=String(s);}
