...
import { giantBedroomMapBody } from './maps/giant-bedroom.js?v=5624';
...
(() => {
  ...
  function damageBedroomTeddy(amount){
    ...
    const hp=bedroomTeddy.userData.hp;
    bedroomTeddy.rotation.z=(Math.random()-.5)*.08;
    if(hp<=0){
      bedroomTeddy.userData.dead=true;bedroomTeddy.visible=false;bedroomTeddyRespawn=50;
      if(bedroomTeddyCollider)bedroomTeddyCollider.active=false;
      burstTeddyCotton(bedroomTeddy.position);
      hitNoticeEl.textContent='PUFF! 🧸';hitNoticeEl.classList.add('show');hitNoticeTimer=1.0;
    }
  }
  ...
  function shoot(){
    ...
    if(activeMap==='quarto'&&hit?.object?.userData?.bedroomTeddy)damageBedroomTeddy(weaponMode==='sniper'?90:(weaponMode==='shotgun'?65:(weaponMode==='launcher'?120:32)));
