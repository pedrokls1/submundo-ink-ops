...
(() => {
  ...
  for(const [kind,v] of Object.entries(weaponVariants)){
    ...
    const makeArmHand=(pt,front=false)=>{
      const hg=new THREE.Group(); hg.position.set(...pt);
  ...
  function makeRemotePlayer(state){
    ...
    const neck=new THREE.Mesh(new THREE.CylinderGeometry(.125,.17,.38,8),paperMat);
    neck.position.set(0,1.735,0); group.add(neck);
    const neckOutline=new THREE.Mesh(new THREE.CylinderGeometry(.133,.178,.382,8),new THREE.MeshBasicMaterial({color:0x08090b,side:THREE.BackSide}));
