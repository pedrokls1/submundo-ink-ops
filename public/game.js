...
(() => {
  ...
  function applyRemoteStates(states){
    ...
    for(const state of states||[]){
      ...
      remote.lastNetPos.copy(nextPos);remote.lastPacketAt=now;remote.target.copy(nextPos);remote.targetYaw=state.yaw+Math.PI;remote.weapon=state.weapon;remote.snapshots=remote.snapshots||[];remote.snapshots.push({t:now,pos:nextPos.clone(),yaw:state.yaw+Math.PI});if(remote.snapshots.length>12)remote.snapshots.splice(0,remote.snapshots.length-12);
