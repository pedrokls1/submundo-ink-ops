...
(() => {
  ...
  // V56.2 — QUARTO GIGANTE: cama justa, brinquedos-cover, 2 BOINGs e tomadas coladas.
  function buildGiantBedroom(){
    ...
    box(98,18,.35,mat(0xf2eadb),0,9,-40,true);box(98,18,.35,mat(0xf2eadb),0,9,40,true);box(.35,18,80,mat(0xf2eadb),-49,9,0,true);box(.35,18,80,mat(0xf2eadb),49,9,0,true);
    ...
    // Centro: poucas coberturas grandes e fáceis de ler.
    ...
    // Proteção dos spawns agora parece parte real do quarto: dois baús de brinquedos equivalentes.
    ...
    // Covers centrais reconhecíveis: livros, robô, boneca e urso — poucos e grandes.
    ...
    const doll=new THREE.Group();doll.position.set(16,0,1);const skirt=addOutline(new THREE.Mesh(new THREE.ConeGeometry(2.7,4.5,9),mat(0xc996ad)));skirt.position.y=2.25;doll.add(skirt);const dhd=addOutline(new THREE.Mesh(new THREE.SphereGeometry(1.45,9,7),mat(0xe4c8a6)));dhd.position.y=5.2;doll.add(dhd);scene.add(doll);colliders.push({minX:13.2,maxX:18.8,minZ:-1.8,maxZ:3.8});
    ...
    // Oito tomadas coladas às quatro paredes.
    bedroomOutlets.forEach(([x,z],i)=>{const side=Math.abs(x)>47;const plate=box(side?.22:3.1,2.5,side?3.1:.22,mat(0xf8f3e7),x,1.45,z,false);plate.userData.outlet=i;const ix=side?(x<0?.14:-.14):0,iz=!side?(z<0?.14:-.14):0;const dark=box(side?.24:.75,.48,side?.75:.24,mat(black),x+ix,1.55,z+iz,false);dark.userData.outlet=i;makeMapLabel('⚡',x+(side?(x<0?.35:-.35):0),3.0,z+(!side?(z<0?.35:-.35):0),.7);});
    // Dois Slinkys simétricos: são o único acesso normal ao topo da cama.
    const makeSlinky=(z,color)=>{const spring=new THREE.Group();for(let i=0;i<14;i++){const ring=addOutline(new THREE.Mesh(new THREE.TorusGeometry(1.45,.10,6,20),mat(i%2?color:toyYellow)));ring.rotation.x=Math.PI/2;ring.position.y=.22+i*.16;spring.add(ring);}spring.position.set(-20,0,z);scene.add(spring);makeMapLabel('BOING!',-20,2.8,z,.85);};makeSlinky(-12,red);makeSlinky(12,blue);
  ...
  function applyMapVisibility(){
    kingdomObjects.forEach(o=>o.visible=activeMap==='reino');bedroomObjects.forEach(o=>o.visible=activeMap==='quarto');if(cheshireCat&&activeMap!=='reino')cheshireCat.visible=false;if(goldenInkwell&&activeMap!=='reino')goldenInkwell.visible=false;
  ...
  function update(dt){
    ...
    if(activeMap==='quarto'){
      ...
      const onBedroomSlinky=Math.hypot(player.pos.x+20,player.pos.z+12)<1.8||Math.hypot(player.pos.x+20,player.pos.z-12)<1.8;
      if(grounded&&jumpPadCooldown<=0&&onBedroomSlinky){verticalVelocity=14.8;grounded=false;jumpPadCooldown=1.4;player.pos.x-=2.2;hitNoticeEl.textContent='BOOOING!';hitNoticeEl.classList.add('show');hitNoticeTimer=.8;}
