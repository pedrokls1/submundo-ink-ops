...
(() => {
  ...
  // V34: modo PC/Mobile. No PC escondemos controles touch e mantemos apenas a engrenagem.
  const settingsPanel=document.createElement('div');
  settingsPanel.style.cssText='position:fixed;left:50%;top:52px;transform:translateX(-50%);z-index:70;display:none;width:min(92vw,330px);padding:14px;border:3px solid #171510;border-radius:12px;background:rgba(252,251,247,.97);box-shadow:6px 6px 0 rgba(23,21,16,.18);font:700 12px Georgia;color:#171510;pointer-events:auto';
  settingsPanel.innerHTML='<div style="font:900 15px Georgia;margin-bottom:10px">CONFIGURAÇÕES</div><div style="margin-bottom:7px">CONTROLES</div><div style="display:flex;gap:6px"><button data-mode="auto">AUTO</button><button data-mode="pc">PC</button><button data-mode="mobile">MOBILE</button></div><div style="margin-top:10px;font-size:10px;line-height:1.35">PC: WASD • mouse olha • esquerdo atira • direito mira • R recarrega • J pula • 1–6 armas</div>';
  document.body.appendChild(settingsPanel);
  for(const b of settingsPanel.querySelectorAll('button')) b.style.cssText='flex:1;padding:8px 5px;border:2px solid #171510;border-radius:8px;background:#eee7d4;font:800 11px Georgia;color:#171510;pointer-events:auto';
  ...
  function applyControlMode(){
    ...
    for(const el of touchControls) el.style.display=pc?'none':'';
    const lobby=document.getElementById('returnLobbyBtn'),voice=document.getElementById('voiceControl'); if(lobby)lobby.style.display=pc?'none':(multiplayer.room?.status==='playing'?'block':'none');if(voice)voice.style.display=pc?'none':(multiplayer.room?.status==='playing'?'block':'none');
    layoutBtn.textContent='⚙'; layoutBtn.style.left='auto';layoutBtn.style.right='12px';layoutBtn.style.transform='none';layoutBtn.style.top='12px';
    layoutTip.style.display='none'; resetLayoutBtn.style.display='none'; hudOpacityBtn.style.display='none'; controlsEditing=false;
    for(const b of settingsPanel.querySelectorAll('button')) b.style.background=b.dataset.mode===controlMode?'#cfd9ef':'#eee7d4';
  }
  for(const b of settingsPanel.querySelectorAll('button')) b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();controlMode=b.dataset.mode;localStorage.setItem('inkopsControlMode',controlMode);applyControlMode();});
  layoutBtn.addEventListener('pointerdown',e=>{if(isDesktopMode()){e.preventDefault();e.stopImmediatePropagation();settingsPanel.style.display=settingsPanel.style.display==='block'?'none':'block';}},true);
  applyControlMode();
  ...
  function updateRemotePlayers(dt){
    for(const remote of multiplayer.remotePlayers.values()){
      ...
      remote.gun.scale.z=remote.weapon==='launcher'?1.28:(remote.weapon==='knife'?.52:1);remote.group.visible=remote.alive; // V24: nunca ocultar jogador remoto por estar na toca/superficie
    }
  }
  ...
  canvas.addEventListener('contextmenu',e=>e.preventDefault());canvas.addEventListener('pointerdown',e=>{if(!running)return;if(e.pointerType==='mouse'){if(document.pointerLockElement!==canvas){canvas.requestPointerLock?.();return;}if(e.button===0){fireHeld=weaponMode!=='sniper'&&weaponMode!=='launcher';shoot();}if(e.button===2&&weaponMode!=='knife'){scopeMode=true;if(weaponMode==='sniper')sniperAimArmed=true;}}else if(!isDesktopMode()){touchLookId=e.pointerId;lastLookX=e.clientX;lastLookY=e.clientY;canvas.setPointerCapture(e.pointerId);}});document.addEventListener('mousemove',e=>{if(running&&document.pointerLockElement===canvas)look(e.movementX,e.movementY,.00235);});canvas.addEventListener('pointermove',e=>{if(!running||e.pointerType==='mouse'||isDesktopMode())return;if(e.pointerId===touchLookId){const dx=e.clientX-lastLookX,dy=e.clientY-lastLookY;look(dx,dy,.0042);lastLookX=e.clientX;lastLookY=e.clientY;}});window.addEventListener('pointerup',e=>{if(e.pointerType==='mouse'){if(e.button===0)fireHeld=false;if(e.button===2){scopeMode=false;sniperAimArmed=false;}}if(e.pointerId===touchLookId)touchLookId=null;});document.addEventListener('pointerlockchange',()=>{mouseLook=document.pointerLockElement===canvas;canvas.style.cursor=mouseLook?'none':'crosshair';});
