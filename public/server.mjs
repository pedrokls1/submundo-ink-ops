...
export function createGameServer(){
  const rooms=new Map();
  const weapons={rifle:{body:26,head:34,delay:100,range:100},smg:{body:18,head:20,delay:65,range:75},shotgun:{body:55,head:65,delay:650,range:15},launcher:{body:100,head:100,delay:900,range:100},sniper:{body:60,head:100,delay:900,range:150},knife:{body:45,head:55,delay:350,range:3}};
  ...
  const snapshotFor=(r,p,now=Date.now())=>{const players=[...r.players.values()].map(pose);return {players:players.filter(v=>v.token!==p.id),scores:r.scores,props:[...r.props.entries()].map(([id,v])=>({id,hp:v.hp,active:!v.respawnAt,respawnMs:Math.max(0,(v.respawnAt||0)-now)})),match:{status:r.status,remainingMs:r.status==='playing'?Math.max(0,r.matchEnds-now):0,killLimit:r.killLimit,winner:r.winner||null},self:{...pose(p),alive:p.health>0,respawnSeconds:Math.max(0,Math.ceil((p.respawnAt-now)/1000))}};};
  ...
  const tick=setInterval(()=>{
    ...
    for(const r of rooms.values()){
      ...
      for(const [id,prop] of r.props||[]){if(prop.respawnAt&&now>=prop.respawnAt){prop.hp=120;prop.respawnAt=0;for(const pl of r.players.values())send(pl.ws,{type:'prop',data:{id,hp:120,active:true,respawnMs:0}});}}
      ...
      try{
        ...
        if(m.action==='create'||m.action==='join'){
          ...
          if(m.action==='create'){
            ...
            r={code,status:'lobby',botsEnabled:false,host:null,players:new Map(),scores:{azul:0,vermelho:0},props:freshProps(),matchStart:0,matchEnds:0,killLimit:40,winner:null};rooms.set(code,r);
        ...
        if(m.action==='start'){if(p.id!==r.host)return fail('Somente o líder pode iniciar.');r.status='playing';r.scores={azul:0,vermelho:0};r.props=freshProps();r.matchStart=Date.now();r.matchEnds=r.matchStart+6*60*1000;r.winner=null;for(const pl of r.players.values()){pl.kills=0;pl.deaths=0;spawn(pl);}reply(lobby(r));publishLobby(r);return;}
        ...
        if(m.action==='hit'){
          ...
          if(eliminated){target.deaths++;target.respawnAt=now+4000;p.kills++;r.scores[p.team]++;for(const other of r.players.values())send(other.ws,{type:'kill',data:{killer:p.nickname,victim:target.nickname,self:false,team:p.team}});if(r.scores[p.team]>=r.killLimit)finishMatch(r,'kills');}
...
if(process.argv[1]===fileURLToPath(import.meta.url)){const game=createGameServer();game.server.listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Submundo V75 Reino das Copas pronto'));process.on('SIGTERM',async()=>{await game.close();process.exit(0);});}
