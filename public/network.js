...
(() => {
  ...
  function connect(){
    ...
    socket.onopen=()=>{const roomCode=localStorage.getItem('inkopsRoomCode')||'';if(playerId&&secret&&roomCode&&!resumeBusy){resumeBusy=true;setTimeout(()=>{if(socket?.readyState!==1){resumeBusy=false;return;}const id=nextId++;pending.set(id,{resolve:data=>{resumeBusy=false;data=normalizeLobby(data);lobby=data;window.__inkRealtimeLobby?.(data);window.__inkRealtimeReconnected?.(data);},reject:()=>{resumeBusy=false;}});socket.send(JSON.stringify({id,action:'resume',roomCode,playerId,secret}));},350);}};
  ...
  window.fetch = async (input,init={}) => {
    ...
    try{
      ...
      else if(action==='settings'||action==='ready'||action==='team'||action==='start'||action==='leave'||action==='hit'||action==='selfhit'||action==='prop_hit'||action==='hazardhit'||action==='cat_hit'||action==='reward_take') data=await call(action,wireBody);
