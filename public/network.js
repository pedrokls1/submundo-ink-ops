...
(() => {
  ...
  function connect(){
    ...
    socket.onopen=()=>{const roomCode=localStorage.getItem('inkopsRoomCode')||'';if(playerId&&secret&&roomCode&&!resumeBusy){resumeBusy=true;setTimeout(()=>{if(socket?.readyState!==1){resumeBusy=false;return;}const id=nextId++;pending.set(id,{resolve:data=>{resumeBusy=false;lobby=data;window.__inkRealtimeLobby?.(data);window.__inkRealtimeReconnected?.(data);},reject:()=>{resumeBusy=false;}});socket.send(JSON.stringify({id,action:'resume',roomCode,playerId,secret}));},350);}};
    ...
    socket.onclose = () => { socket=null;resumeBusy=false;for(const p of pending.values())p.reject(new Error('Conexão encerrada'));pending.clear();clearTimeout(reconnectTimer);if(localStorage.getItem('inkopsRoomCode')&&playerId&&secret)reconnectTimer=setTimeout(connect,1100); };
  }
  ...
  window.fetch = async (input,init={}) => {
    ...
    try{
      ...
      else if(action==='settings'||action==='start'||action==='leave'||action==='hit'||action==='selfhit'||action==='prop_hit'||action==='hazardhit'||action==='cat_hit'||action==='reward_take'||action==='rabbit_hit') data=await call(action,wireBody);
