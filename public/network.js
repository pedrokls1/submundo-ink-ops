...
(() => {
  ...
  window.fetch = async (input,init={}) => {
    ...
    try{
      ...
      else if(action==='settings'||action==='start'||action==='leave'||action==='hit'||action==='selfhit'||action==='prop_hit'||action==='hazardhit'||action==='cat_hit') data=await call(action,wireBody);
