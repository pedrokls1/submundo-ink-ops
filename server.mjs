...
export function createGameServer(){
  const rooms=new Map();
  const weapons={rifle:{body:26,head:34,delay:100,range:100},smg:{body:28,head:32,delay:65,range:75},shotgun:{body:55,head:65,delay:650,range:15},launcher:{body:100,head:100,delay:900,range:100},sniper:{body:60,head:100,delay:850,range:150},knife:{body:85,head:100,delay:350,range:3}};
...
if(process.argv[1]===fileURLToPath(import.meta.url)){const game=createGameServer();game.server.listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Submundo V77 Reino das Copas pronto'));process.on('SIGTERM',async()=>{await game.close();process.exit(0);});}
