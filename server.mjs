      ...
      try{
        ...
        if(m.action==='create'||m.action==='join'){
          ...
          if(m.action==='create'){
            ...
            r={code,status:'lobby',botsEnabled:false,map:'reino',host:null,players:new Map(),scores:{azul:0,vermelho:0},props:freshProps(),cat:{hp:180,respawnAt:0},reward:{active:false,respawnAt:0,x:-27,z:24},matchStart:0,matchEnds:0,killLimit:40,winner:null};rooms.set(code,r);
        ...
        if(m.action==='settings'){if(p.id!==r.host||r.status!=='lobby')return fail('Somente o líder pode alterar no lobby.');r.botsEnabled=m.botsEnabled===true;const wanted=String(m.map||r.map||'reino');if(wanted==='reino'||wanted==='quarto')r.map=wanted;reply(lobby(r));publishLobby(r);return;}
