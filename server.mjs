    ...
    ws.on('message',raw=>{
      ...
      try{
        ...
        if(m.action==='state'){
          ...
          for(const [key,min,max] of [['x',-48,48],['y',-4,18],['z',-48,48],['yaw',-100000,100000],['pitch',-1.5,1.5]])if(typeof m[key]==='number'&&Number.isFinite(m[key]))p[key]=Math.max(min,Math.min(max,m[key]));
        ...
        if(m.action==='hit'){
          ...
          if(eliminated){target.deaths++;target.respawnAt=now+4000;p.kills++;r.scores[p.team]++;for(const other of r.players.values())send(other.ws,{type:'kill',data:{killer:p.nickname,victim:target.nickname,self:false,team:p.team}});}
...
if(process.argv[1]===fileURLToPath(import.meta.url)){const game=createGameServer();game.server.listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Submundo V74 pronto'));process.on('SIGTERM',async()=>{await game.close();process.exit(0);});}
