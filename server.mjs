...
export function createGameServer(){
  ...
  const snapshotFor=(r,p,now=Date.now())=>{const players=[...r.players.values()].map(pose);return {players:players.filter(v=>v.token!==p.id),scores:r.scores,props:[...r.props.entries()].map(([id,v])=>({id,hp:v.hp,active:!v.respawnAt,respawnMs:Math.max(0,(v.respawnAt||0)-now)})),cat:{hp:r.cat?.hp??180,active:!(r.cat?.respawnAt),respawnMs:Math.max(0,(r.cat?.respawnAt||0)-now)},match:{status:r.status,remainingMs:r.status==='playing'?Math.max(0,r.matchEnds-now):0,killLimit:r.killLimit,winner:r.winner||null},self:{...pose(p),alive:p.health>0,respawnSeconds:Math.max(0,Math.ceil((p.respawnAt-now)/1000))}};};
      ...
      try{
        ...
        if(m.action==='create'||m.action==='join'){
          ...
          if(m.action==='create'){
            ...
            r={code,status:'lobby',botsEnabled:false,host:null,players:new Map(),scores:{azul:0,vermelho:0},props:freshProps(),cat:{hp:180,respawnAt:0},matchStart:0,matchEnds:0,killLimit:40,winner:null};rooms.set(code,r);
        ...
        if(m.action==='start'){if(p.id!==r.host)return fail('Somente o líder pode iniciar.');r.status='playing';r.scores={azul:0,vermelho:0};r.props=freshProps();r.cat={hp:180,respawnAt:0};r.matchStart=Date.now();r.matchEnds=r.matchStart+6*60*1000;r.winner=null;for(const pl of r.players.values()){pl.kills=0;pl.deaths=0;spawn(pl);}reply(lobby(r));publishLobby(r);return;}
...
if(process.argv[1]===fileURLToPath(import.meta.url)){const game=createGameServer();game.server.listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Submundo V76 Reino das Copas pronto'));process.on('SIGTERM',async()=>{await game.close();process.exit(0);});}
