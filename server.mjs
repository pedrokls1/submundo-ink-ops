...
export function createGameServer(){
  ...
  const files={'/':'public/index.html','/index.html':'public/index.html','/game.js':'public/game.js','/network.js':'public/network.js','/load-test.html':'public/load-test.html','/vendor/three.module.js':'node_modules/three/build/three.module.js','/vendor/three.core.js':'node_modules/three/build/three.core.js'};
  ...
  const lobby=r=>({room:{code:r.code,status:r.status,botsEnabled:r.botsEnabled,killLimit:r.killLimit||40},players:[...r.players.values()].map(p=>({token:p.id,nickname:p.nickname,team:p.team,is_host:p.id===r.host,ready:p.ready===true}))});
  ...
  const tick=setInterval(()=>{
    ...
    for(const r of rooms.values()){
      for(const p of r.players.values()){
        ...
        if(p.respawnAt&&now>=p.respawnAt)spawn(p,5000,teamSlot(r,p));
      }
  ...
  wss.on('connection',ws=>{
    ...
    ws.on('message',raw=>{
      ...
      try{
        ...
        if(m.action==='start'){if(p.id!==r.host)return fail('Somente o líder pode iniciar.');if(r.status!=='lobby')return fail('A sala ainda não está pronta para iniciar.');const waiting=[...r.players.values()].filter(pl=>pl.id!==r.host&&!pl.ready);if(waiting.length)return fail('Os outros jogadores precisam marcar PRONTO antes de iniciar.');r.status='playing';r.scores={azul:0,vermelho:0};r.props=freshProps();r.cat={hp:180,respawnAt:0};r.reward={active:false,respawnAt:0,x:-27,z:24};r.matchStart=Date.now();r.matchEnds=r.matchStart+6*60*1000;r.winner=null;for(const pl of r.players.values()){pl.kills=0;pl.deaths=0;pl.ready=false;spawn(pl,15000,teamSlot(r,pl));}reply(lobby(r));publishLobby(r);return;}
