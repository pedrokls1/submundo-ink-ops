...
export function createGameServer(){
  ...
  const spawn=p=>{const now=Date.now();p.x=0;p.y=1.7;p.z=p.team==='azul'?36:-36;p.yaw=p.team==='azul'?0:Math.PI;p.pitch=0;p.health=100;p.armor=100;p.respawnAt=0;p.invulnerableUntil=now+5000;p.seq++;};
    ...
    ws.on('message',raw=>{
      ...
      try{
        ...
        if(m.action==='create'||m.action==='join'){
          ...
          const p={id:randomUUID(),secret:randomBytes(24).toString('hex'),ws,nickname:name,team:blue<=r.players.size-blue?'azul':'vermelho',weapon:'rifle',kills:0,deaths:0,seq:0,lastHit:0,ready:false,invulnerableUntil:0};spawn(p);r.players.set(p.id,p);r.host??=p.id;ws.roomCode=r.code;ws.playerId=p.id;
