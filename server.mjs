...
export function createGameServer(){
  ...
  const snapshotFor=(r,p,now=Date.now())=>{const players=[...r.players.values()].map(pose);const catActive=!!r.cat&&r.cat.active===true;return {players:players.filter(v=>v.token!==p.id),scores:r.scores,props:[...r.props.entries()].map(([id,v])=>({id,hp:v.hp,active:!v.respawnAt,respawnMs:Math.max(0,(v.respawnAt||0)-now)})),cat:{hp:r.cat?.hp??180,active:catActive,respawnMs:catActive?0:Math.max(0,(r.cat?.nextAt||0)-now)},match:{status:r.status,remainingMs:r.status==='playing'?Math.max(0,r.matchEnds-Math.max(now,r.matchStart||now)):0,countdownMs:r.status==='playing'?Math.max(0,(r.matchStart||0)-now):0,killLimit:r.killLimit,winner:r.winner||null},reward:{active:!!r.reward?.active,respawnMs:Math.max(0,(r.reward?.respawnAt||0)-now),x:r.reward?.x??-27,z:r.reward?.z??24},self:{...pose(p),alive:p.health>0,respawnSeconds:Math.max(0,Math.ceil((p.respawnAt-now)/1000)),spawnGraceMs:Math.max(0,(p.invulnerableUntil||0)-now),damageFrom:p.lastDamageFrom||null,damageAt:p.lastDamageAt||0}};};
  const spawn=p=>{p.x=0;p.y=1.7;p.z=p.team==='azul'?36:-36;p.yaw=p.team==='azul'?0:Math.PI;p.pitch=0;p.health=100;p.armor=100;p.respawnAt=0;p.invulnerableUntil=Date.now()+5000;p.seq++;};
  const freshProps=()=>new Map(['m1','m2','m3','m4','m5','m6','m7','m8'].map(id=>[id,{hp:120,respawnAt:0}]));
  const finishMatch=(r,reason='tempo')=>{if(r.status!=='playing')return;const winner=r.scores.azul===r.scores.vermelho?'empate':(r.scores.azul>r.scores.vermelho?'azul':'vermelho');for(const p of r.players.values())send(p.ws,{type:'matchend',data:{winner,reason,scores:r.scores}});r.status='lobby';r.winner=null;r.matchStart=0;r.matchEnds=0;setTimeout(()=>{if(rooms.get(r.code)===r)publishLobby(r);},120);};
  ...
  const tick=setInterval(()=>{
    ...
    for(const r of rooms.values()){
      ...
      for(const [id,prop] of r.props||[]){if(prop.respawnAt&&now>=prop.respawnAt){prop.hp=120;prop.respawnAt=0;for(const pl of r.players.values())send(pl.ws,{type:'prop',data:{id,hp:120,active:true,respawnMs:0}});}}if(r.cat&&!r.cat.active&&r.cat.nextAt&&now>=r.cat.nextAt&&(r.cat.appearances||0)<(r.cat.maxAppearances||3)){r.cat.hp=180;r.cat.active=true;r.cat.nextAt=0;r.cat.appearances=(r.cat.appearances||0)+1;for(const pl of r.players.values())send(pl.ws,{type:'cat',data:{hp:180,active:true,respawnMs:0,message:'O GATO DE CHESHIRE APARECEU NO LABIRINTO!'}});}if(r.reward&&!r.reward.active&&r.reward.respawnAt&&now>=r.reward.respawnAt){r.reward.active=true;r.reward.respawnAt=0;for(const pl of r.players.values())send(pl.ws,{type:'reward',data:{active:true,respawnMs:0,x:r.reward.x,z:r.reward.z,message:'TINTEIRO DOURADO RENASCEU NO LABIRINTO!'}});}
      ...
      try{
        ...
        if(m.action==='create'||m.action==='join'){
          ...
          if(m.action==='create'){
            ...
            r={code,status:'lobby',botsEnabled:false,host:null,players:new Map(),scores:{azul:0,vermelho:0},props:freshProps(),cat:{hp:180,active:false,nextAt:0,appearances:0,maxAppearances:3},reward:{active:false,respawnAt:0,x:-27,z:24},matchStart:0,matchEnds:0,killLimit:40,winner:null};rooms.set(code,r);
        ...
        if(m.action==='start'){if(p.id!==r.host)return fail('Somente o líder pode iniciar.');r.status='playing';r.scores={azul:0,vermelho:0};r.props=freshProps();const now=Date.now();r.matchStart=now+5000;r.matchEnds=r.matchStart+6*60*1000;r.cat={hp:180,active:false,nextAt:r.matchStart+60000,appearances:0,maxAppearances:3};r.reward={active:false,respawnAt:0,x:-27,z:24};r.winner=null;for(const pl of r.players.values()){pl.kills=0;pl.deaths=0;spawn(pl);pl.invulnerableUntil=Math.max(pl.invulnerableUntil||0,r.matchStart+5000);}reply(lobby(r));publishLobby(r);return;}
        ...
        if(m.action==='prop_hit'){const prop=r.props.get(String(m.propId||''));const now=Date.now();if(now<(r.matchStart||0)||!prop||prop.respawnAt)return reply({ok:false});const dmg=Math.max(1,Math.min(100,Math.round(Number(m.damage)||0)));prop.hp=Math.max(0,prop.hp-dmg);if(prop.hp<=0){prop.respawnAt=now+12000;for(const pl of r.players.values())send(pl.ws,{type:'prop',data:{id:String(m.propId),hp:0,active:false,respawnMs:12000}});}return reply({ok:true,id:String(m.propId),hp:prop.hp,active:!prop.respawnAt});}
        if(m.action==='cat_hit'){const now=Date.now();if(now<(r.matchStart||0)||!r.cat?.active)return reply({ok:false,hp:r.cat?.hp||0,active:false,respawnMs:Math.max(0,(r.cat?.nextAt||0)-now)});const dmg=Math.max(1,Math.min(100,Math.round(Number(m.damage)||0)));r.cat.hp=Math.max(0,r.cat.hp-dmg);if(r.cat.hp<=0){r.cat.active=false;r.cat.nextAt=(r.cat.appearances||0)<(r.cat.maxAppearances||3)?now+75000:0;for(const pl of r.players.values())send(pl.ws,{type:'cat',data:{hp:0,active:false,respawnMs:r.cat.nextAt?75000:0}});if(r.reward&&!r.reward.active&&!r.reward.respawnAt){r.reward.active=true;for(const pl of r.players.values())send(pl.ws,{type:'reward',data:{active:true,respawnMs:0,x:r.reward.x,z:r.reward.z,message:'O GATO DEIXOU O TINTEIRO DOURADO!'}});}}return reply({ok:true,hp:r.cat.hp,active:r.cat.active===true,respawnMs:r.cat.nextAt?Math.max(0,r.cat.nextAt-now):0});}
        ...
        if(m.action==='hazardhit'){const now=Date.now();if(now<(r.matchStart||0)||p.health<=0||now<(p.invulnerableUntil||0)||!r.cat?.active||now-(p.lastHazardHit||0)<1200)return reply({ok:false});p.lastHazardHit=now;let healthDamage=26;if(p.armor>0){const a=Math.min(p.armor,10);p.armor-=a;healthDamage-=a;}p.health=Math.max(0,p.health-healthDamage);const eliminated=p.health===0;if(eliminated){p.deaths++;p.respawnAt=now+4000;for(const pl of r.players.values())send(pl.ws,{type:'kill',data:{killer:'Criatura',victim:p.nickname,self:false,team:'neutro',weapon:'cheshire',headshot:false}});}return reply({ok:true,health:p.health,armor:p.armor,eliminated});}
        if(m.action==='rabbit_hit'){const now=Date.now();if(now<(r.matchStart||0)||p.health<=0||now<(p.invulnerableUntil||0)||now-(p.lastRabbitHit||0)<1200)return reply({ok:false});p.lastRabbitHit=now;let damage=6;if(p.armor>0){const a=Math.min(p.armor,3);p.armor-=a;damage-=a;}p.health=Math.max(0,p.health-damage);const eliminated=p.health===0;if(eliminated){p.deaths++;p.respawnAt=now+4000;for(const pl of r.players.values())send(pl.ws,{type:'kill',data:{killer:'Coelho',victim:p.nickname,self:false,team:'neutro',weapon:'rabbit',headshot:false}});}return reply({ok:true,health:p.health,armor:p.armor,eliminated});}
        ...
        if(m.action==='hit'){
          ...
          if(now<(r.matchStart||0)||!target||target===p||target.team===p.team||p.health<=0||target.health<=0||now<(target.invulnerableUntil||0)||now-p.lastHit<w.delay||Math.hypot(p.x-target.x,p.z-target.z)>w.range+2)return reply({ok:false});
...
if(process.argv[1]===fileURLToPath(import.meta.url)){const game=createGameServer();game.server.listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Submundo V13 Beta Polimento pronto'));process.on('SIGTERM',async()=>{await game.close();process.exit(0);});}
