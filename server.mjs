...
export function createGameServer(){
  const rooms=new Map();
  const weapons={rifle:{damage:38,delay:100,range:100},smg:{damage:22,delay:65,range:75},shotgun:{damage:100,delay:650,range:15},launcher:{damage:100,delay:900,range:100},sniper:{damage:100,delay:900,range:150},knife:{damage:100,delay:350,range:3}};
  ...
  const wss=new WebSocketServer({server,path:'/realtime',maxPayload:4096,perMessageDeflate:false});
  ...
  const pose=p=>({token:p.id,nickname:p.nickname,team:p.team,x:p.x,y:p.y,z:p.z,yaw:p.yaw,pitch:p.pitch,weapon:p.weapon,alive:p.health>0,health:p.health,kills:p.kills,deaths:p.deaths,seq:p.seq});
  ...
  const spawn=p=>{p.x=0;p.y=1.7;p.z=p.team==='azul'?30:-30;p.yaw=p.team==='azul'?Math.PI:0;p.pitch=0;p.health=100;p.respawnAt=0;p.seq++;};
    ...
    ws.on('message',raw=>{
      ...
      try{
        ...
        if(m.action==='hit'){
          ...
          p.lastHit=now;const damage=Math.min(w.damage,Math.round(amount));target.health=Math.max(0,target.health-damage);const eliminated=target.health===0;
          if(eliminated){target.deaths++;target.respawnAt=now+4000;p.kills++;r.scores[p.team]++;}
          return reply({ok:true,damage,health:target.health,eliminated});
        }
...
if(process.argv[1]===fileURLToPath(import.meta.url)){const game=createGameServer();game.server.listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Submundo V71 pronto'));process.on('SIGTERM',async()=>{await game.close();process.exit(0);});}
